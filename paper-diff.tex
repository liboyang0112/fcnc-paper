...
%DIF DEL ../fcnc-paper_v1.1/ANA-TOPQ-2019-17-PAPER.tex   Fri Mar 18 21:18:24 2022
%DIF ADD ANA-TOPQ-2019-17-PAPER.tex                      Tue Apr 12 22:27:29 2022
...
%DIF 17c17
%DIF < \documentclass[PAPER, coverpage, atlasdraft=true, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc}
%DIF -------
%\documentclass[PAPER, coverpage, atlasdraft=true, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc} %DIF > 
%DIF -------
%\documentclass[PAPER, atlasdraft=true, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc}
%DIF 19c19
%DIF < %\documentclass[CONF, atlasdraft=false, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc}
%DIF -------
\documentclass[CONF, atlasdraft=false, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc} %DIF > 
%DIF -------
...
%DIF 105c105
%DIF < \AtlasVersion{1.1}
%DIF -------
\AtlasVersion{1.2} %DIF > 
%DIF -------
...
%DIF 128-129c128-130
%DIF < \AtlasRefCode{TOPQ-2019-17}
%DIF < %\AtlasRefCode{ATLAS-CONF-2018-049}
%DIF -------
%\AtlasRefCode{CONF-TOPQ-2022-06} %DIF > 
%\AtlasRefCode{TOPQ-2019-17} %DIF > 
\AtlasRefCode{ATLAS-CONF-2022-014} %DIF > 
%DIF -------
...
However, these branching ratios can be as high as $\BR(t\to qH) \sim 10^{-5}$ \DIFdelbegin \DIFdel{which incorporate }\DIFdelend \DIFaddbegin \DIFadd{when considering }\DIFaddend beyond the SM processes. Examples of these \DIFaddbegin \DIFadd{processes }\DIFaddend include: quark-singlet models~\cite{AguilarSaavedra:2002kr}, two-Higgs-doublet models (2HDM)~\cite{ Branco:2hdm2012} with or without flavour violation,
...
The $tH$ associated production in the SM is expected to have a cross section of $92^{+7}_{-12}$ fb at \DIFaddbegin \DIFadd{a }\DIFaddend centre-of-mass \DIFdelbegin \DIFdel{energies }\DIFdelend \DIFaddbegin \DIFadd{energy }\DIFaddend of $\sqrt{s}= 13~\tev$\DIFdelbegin \DIFdel{centre of mass energy}\DIFdelend ~\cite{deFlorian:2016spz}.
...
\DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend full Run 2 \DIFdelbegin \DIFdel{data and  set }\DIFdelend \DIFaddbegin \DIFadd{dataset. The }\DIFaddend observed (expected) upper limits on  \DIFdelbegin \DIFdel{$\mathcal{B}(t\to Hu)$ of $7.9\times10^{-4}(1.1\times10^{-3})$ and $\mathcal{B}(t\to Hc)$ of }\DIFdelend \DIFaddbegin \DIFadd{$\BR(t\to cH)$ are }\DIFaddend $9.4\times10^{-4}(8.6\times10^{-4})$
\DIFaddbegin \DIFadd{and are $7.9\times10^{-4}(1.1\times10^{-3})$ for $\BR{B}(t\to uH)$}\DIFaddend ~\cite{CMS:2021gfa}.
...
\begin{table}[t!]
\caption{\DIFdelbeginFL %DIFDELCMD < \small{Summary of 95\% CL upper limits on $\BR(t \to Hc)$ and $\BR(t \to Hu)$ obtained from ATLAS and CMS Collaborations with Run 2 data. Each limit is obtained assuming the other branching ratio is 0}%%%
\DIFdelendFL \DIFaddbeginFL \small{Summary of 95\% CL upper limits on $\BR(t \to cH)$ and $\BR(t \to uH)$ obtained from ATLAS and CMS Collaborations with Run 2 data. Each limit is obtained assuming the other branching ratio is null.}\DIFaddendFL }
\begin{center}
\small 
\begin{tabular}{ccccc}
\toprule\toprule
& &\multirow{2}{*}{$\mathcal{L}$ [fb$^{-1}$]} & \multicolumn{2}{c}{95\% CL observed upper limits}  \\
& & 										    & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{on $\BR(t \to Hc)$}            %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{on $\BR(t \to cH)$}            \DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{on $\BR(t \to Hu)$} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{on $\BR(t \to uH)$} \DIFaddendFL \\
\midrule
\multirow{5}{*}{ATLAS}
...
the  $W$ \DIFdelbegin \DIFdel{decay}\DIFdelend \DIFaddbegin \DIFadd{and tau decays}\DIFaddend . Top quark pair production events where the $W$ boson decays hadronically (leptonically) are referred to as $t_h$ ($t_{\ell}$).
The \DIFaddbegin \DIFadd{decays $\tau \to \ell \nu_{\ell} \nu_{\tau}$ are referred as $\lep$, while the decays $\tau \to \text{hadrons} + \nu_{\tau}$ are referred as $\thad$.
The }\DIFaddend contribution of 
...
either $\tlhad$ or $\thadhad$ final states when the top decays hadronically ($t_h$), but only \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend $\thadhad$ final state \DIFaddbegin \DIFadd{is considered
}\DIFaddend when the top decay leptonically ($t_{\ell}$) in order to avoid \DIFdelbegin \DIFdel{overlap with
multilepton analysis}\DIFdelend \DIFaddbegin \DIFadd{overlaps with
other ATLAS searches}\DIFaddend ~\cite{Aaboud:2018pob}. 
Events with two hadronically decaying $\tau$-leptons without electron or muon are named hadronic channels. Events with at least one \tauhad with an additional electron or muon \DIFaddbegin \DIFadd{(corresponding to $t_{\ell}$ or $\lep$ events) }\DIFaddend are named leptonic channels. More signal regions (with top quark decay leptonically) are exploited here compared to the previous FCNC $\Htautau$ search conducted using partial Run~2 data~\cite{fcnc36}.
In addition, an improved treatment of misidentified $\tau$-leptons in simulation and data-driven
fake estimations from multi-jet background \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend implemented.
% and more
...
A \DIFdelbegin \DIFdel{track-based }\DIFdelend \DIFaddbegin \DIFadd{track- and cluster-based }\DIFaddend lepton isolation criterion is defined by calculating the quantity $I_R = \sum \pt^{\textrm{trk}}$ and $E_R = \sum E^{\textrm{clst}}$, where
...
The \DIFaddbegin \DIFadd{reconstructed }\DIFaddend taus are required to have \DIFaddbegin \DIFadd{a }\DIFaddend \pt \DIFaddbegin \DIFadd{of }\DIFaddend 5 GeV higher than the trigger threshold, and $|\eta|<2.5$, excluding the EM calorimeter's transition region.
...
The events in the leptonic channel are recorded by \DIFaddbegin \DIFadd{a }\DIFaddend single-electron or single-muon trigger, required to have exactly one electron or muon that matches, \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{within }\DIFaddend $\Delta R < 0.15$, the lepton reconstructed by at least one of the possible triggers. Further requirements are defined targeting different signal decay modes:  
\begin{itemize}
\item $t_h\lephad$: Targeting into the $t_hH$ and $t_ht(qH)$ final states with \DIFaddbegin \DIFadd{a }\DIFaddend $H\to\lephad$ decay. Exactly one $\had$ with opposite-side charge to $\lep$ is required, including at least three jets with exactly one $b$-jet.
\item $t_{\ell}\hadhad$: Targeting $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with \DIFaddbegin \DIFadd{a }\DIFaddend $H\to\hadhad$ decay. Exactly one light lepton and two opposite-sign $\had$ are selected, including at least \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend jet with exactly one $b$-jet.
\item $t_{\ell}\had$: Also targeting $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with \DIFaddbegin \DIFadd{a }\DIFaddend $H\to\hadhad$ decay \DIFdelbegin \DIFdel{when one of the }\DIFdelend \DIFaddbegin \DIFadd{where one }\DIFaddend $\had$ fails the tau reconstruction or identification so that there is only one \DIFaddbegin \DIFadd{reconstructed }\DIFaddend $\had$ candidate\DIFdelbegin \DIFdel{reconstructed.
  Exactly }\DIFdelend \DIFaddbegin \DIFadd{.
  To reduce the SM backgrounds and avoid overlaps with the other selections, exactly }\DIFaddend one $\had$ with same-sign charge to the light lepton is required\DIFdelbegin \DIFdel{, including at least 2 jets with }\DIFdelend \DIFaddbegin \DIFadd{.
  In addition, at least two jets including }\DIFaddend exactly one $b$-jet \DIFdelbegin \DIFdel{, to
  reduce the SM backgrounds as well as orthogonal to other channels}\DIFdelend \DIFaddbegin \DIFadd{are required}\DIFaddend .
\end{itemize}

The events in the hadronic channel are \DIFdelbegin \DIFdel{recorded by }\DIFdelend \DIFaddbegin \DIFadd{selected by s }\DIFaddend di-tau trigger\DIFdelbegin \DIFdel{, required to have both leading and subleading $\had$ passed the trigger}\DIFdelend . Further requirements are\DIFdelbegin \DIFdel{defined targeting signal decay modes:
}\DIFdelend \DIFaddbegin \DIFadd{:
%DIF > recorded by di-tau trigger, required to have both leading and subleading $\had$ passed the trigger. Further requirements are defined targeting signal decay modes:
}\DIFaddend \begin{itemize}
\item $t_h\hadhad$: Targeting \DIFdelbegin \DIFdel{into }\DIFdelend the $t_hH$ and $t_ht(qH)$ final states with $H\to\hadhad$ decay. Exactly two $\had$ with opposite-sign charge \DIFdelbegin \DIFdel{are selected, including at least 3 jet with }\DIFdelend \DIFaddbegin \DIFadd{and at least three jets, including }\DIFaddend exactly one $b$-jet\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, are required.
  %DIF > are selected, including at least 3 jet with exactly one $b$-jet.
...
with the NNPDF3.0 NLO parton distribution function (PDF) set. The parton showers, hadronisation, and underlying event \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend modelled by {\pythia}~8.212 with the NNPDF2.3 LO PDF set
...
The contribution of triboson production is found to be \DIFdelbegin \DIFdel{small and }\DIFdelend negligible.
...
\footnotesize
\centering
\caption{Overview of the MC generators \DIFdelbeginFL \DIFdelFL{at which }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{used for }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{matrix-element calculation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{main signal and background samples. including the matrix element }\DIFaddendFL (ME)\DIFdelbeginFL \DIFdelFL{at the leading or the next-leading order (Order)}\DIFdelendFL , \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL parton shower \DIFdelbeginFL \DIFdelFL{model }\DIFdelendFL (PS), \DIFdelbeginFL \DIFdelFL{and the }\DIFdelendFL parton distribution function set (PDF)\DIFdelbeginFL \DIFdelFL{used for the main signal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{background samples}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cross-section calculation order (Order)}\DIFaddendFL .}
...
\DIFdelendFL $VH$ & {\powheg} & {\pythia}~8 & PDF4LHC15&CTEQ6L1 & AZNLO & NLO \\ \hline
$tH$ & {\amcatnlolong} & {\pythia}~8 & \multicolumn{2}{c|}{CT10PDF} & A14 & NLO \\ \hline
\DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL Diboson & \multicolumn{2}{c|}{{\sherpa}~2.2.1} & \multicolumn{2}{c|}{NNPDF30NNLO} & {\sherpa} & NLO/LO \\ \hline\hline
\end{tabular}
\label{mob}
\end{table}
...
\begin{table}
\centering
\caption{Overview of the signal regions (SR), validation region (VR), and \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL $t\bar{t}$ control regions (CRtt) used for \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL fake tau scale factor derivation in \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL leptonic channels. Leptons are required to have either same-sign (SS) or opposite-sign (OS) \DIFdelbeginFL \DIFdelFL{charges }\DIFdelendFL in each region.}
\label{tab:srcr}
\begin{tabular}[h]{c|c|c|c|c|c|c}
\hline \hline
...
The contribution of background to the signal regions of the $t_h\hadhad$ and $t_h\lephad$ channels is reduced by having a kinematic constraints on the di-tau mass and the \DIFdelbegin \DIFdel{missing transverse energy }\DIFdelend \DIFaddbegin \DIFadd{$\met$ }\DIFaddend in the event~\cite{Chen:2015nta}.
...
Events should contain 4 jets, with the one \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{having }\DIFaddend the smallest angular \DIFdelbegin \DIFdel{distance }\DIFdelend \DIFaddbegin \DIFadd{separation with respect }\DIFaddend to the visible \DIFdelbegin \DIFdel{decay of the }\DIFdelend di-tau \DIFaddbegin \DIFadd{system }\DIFaddend being labelled as  \DIFdelbegin \DIFdel{the q-jet }\DIFdelend \DIFaddbegin \DIFadd{$q$-jet }\DIFaddend since the FCNC top decay products are likely boosted closer together\DIFdelbegin \DIFdel{than other jets}\DIFdelend . 
If there are more than two jets besides the $q$- and $b$-jet, the jets from $W$ boson decay are chosen from the combination
with the invariant mass closest to the $W$ boson mass. There is the \DIFdelbegin \DIFdel{chance }\DIFdelend \DIFaddbegin \DIFadd{possibility }\DIFaddend that one of the jets fails the $\pt$ requirement and is not reconstructed.
These kinds of events will fall into \DIFaddbegin \DIFadd{the }\DIFaddend $t_hH$ final state.
...
are estimated using a kinematic fit and \DIFaddbegin \DIFadd{assuming a }\DIFaddend collinear approximation for the tau decay products.
The fit is done by minimising a $\chi^2$ function based on the Gaussian constraints on the Higgs boson mass and the
measured $\met$ within their expected resolutions. The resolution on the Higgs boson mass is estimated to be $20~\gev$ from \DIFdelbegin \DIFdel{the MC simulation
of the }\DIFdelend \DIFaddbegin \DIFadd{a fit of the mass
distribution of the simulated  }\DIFaddend $tqH$ \DIFdelbegin \DIFdel{signals}\DIFdelend \DIFaddbegin \DIFadd{signal events}\DIFaddend , while the resolution on the measured $\met$ is parameterised as a linear function of 
...
generically referred to as \DIFdelbegin \texttt{\DIFdel{fake}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{fake }\DIFaddend leptons, are estimated using data-driven methods. 
%DIF > \texttt{fake} 
...
The events with one light lepton, two same-sign $\had$, and one $b$-jet, are also selected\DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{, providing }\DIFaddend a validation region (VR) for the background estimation in the leptonic channels. 
%-------------------------------------------------------------------------------
...
The FFs are computed as the ratio of \DIFaddbegin \DIFadd{misidentified $\thad$ candidates that either pass or fail }\DIFaddend the \DIFdelbegin \DIFdel{Data-MC ($\mathrm{real~tau}$) yields in which the tau candidate is passed or failed the }\DIFdelend medium tau ID selection in \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{regions enriched in events originating from 
}\DIFaddend $W$+jets \DIFdelbegin \DIFdel{samples}\DIFdelend \DIFaddbegin \DIFadd{processes}\DIFaddend ~\cite{ATLAS-CONF-2021-044}. 
%DIF > the Data-MC ($\mathrm{real~tau}$) ratio of the yields of events in which the tau candidate is passing or failing the medium tau ID selection
%DIF > in the $W$+jets samples~\cite{ATLAS-CONF-2021-044}.
...
A closure test is made for the background estimations in the signal depleted low BDT ($<-0.6$) regions\DIFaddbegin \DIFadd{, defined in Section~\ref{sec:tmva}}\DIFaddend .
The data are in good agreement with the background prediction in the
$t_{\ell}\had$ channels\DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{, while }\DIFaddend the fake light leptons are \DIFdelbegin \DIFdel{important, while }\DIFdelend negligible in other leptonic channels. 
...
%%%%%%%%%%%%%%%
\begin{table*}[t!]
  \caption{\DIFdelbeginFL %DIFDELCMD < \small{The number of discriminating variables (n) used in the training of BDT in each SR. 
%DIFDELCMD < The rank of the discriminating variables relative to one another according to their importance in the training is reported from highest (1) to 
...
lowest (n). Variables whose ranking is missing are not included in the training of the corresponding SR. The description of each variable is provided in the text.}\DIFaddendFL }
...
The BDT discriminant is trained for each SR starting from a large list of variables. Then the \DIFdelbegin \DIFdel{variables are }\DIFdelend \DIFaddbegin \DIFadd{least important variable is }\DIFaddend removed one at \DIFdelbegin \DIFdel{time and retraining }\DIFdelend \DIFaddbegin \DIFadd{a time and the BDT is retrained }\DIFaddend until the
...
The final BDT input variables in each SR and their \DIFdelbegin \DIFdel{importances }\DIFdelend \DIFaddbegin \DIFadd{importance }\DIFaddend are listed in Table~\ref{tab:importance}. The discriminating variables used are:
\begin{itemize}
...
\item $\chi^2$ of the kinematic fit of the \DIFdelbegin \DIFdel{neutrino momentum }\DIFdelend \DIFaddbegin \DIFadd{momentum of the invisible decay products of the tau leptons}\DIFaddend .
...
A comparison between data and the predicted background for the leading $\had$ $\pt$  \DIFaddbegin \DIFadd{and visible di-tau mass }\DIFaddend distribution in the SRs, after applying fake factors, 
%some of these variables in three main decay final states($t_{\ell}\hadhad$, $t_h\hadhad$, and $t_h\lephad$)
is shown in Figure~\ref{fig:taupt_prefit} \DIFaddbegin \DIFadd{and Figure~\ref{fig:mtt}, respectively}\DIFaddend .
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg1l2tau1bnj_os.pdf} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l2tau1bnj_os_log_.pdf} \DIFaddendFL &
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg1l1tau1b1j_ss.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b1j_ss_log_.pdf}\DIFaddendFL &
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg1l1tau1b2j_ss.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b2j_ss_log_.pdf}\DIFaddendFL \\
(a) & (b) & (c) \\
%(a1) \pT(\tauhad) in $t_{\ell}\thadhad$ & (a2) \pT(\tauhad) in  $t_{\ell}\tauhad$-1j& (a3) \pT(\tauhad) in $t_{\ell}\tauhad$-2j\\
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg1l1tau1b2j_os.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b2j_os_log_.pdf}\DIFaddendFL &
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg1l1tau1b3j_os.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b3j_os_log_.pdf} &
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l2tau1bnj_ss_log_.pdf} \DIFaddendFL \\
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1,width=0.33\textwidth]{figures/new_pt/reg2mtau1b2jos_vetobtagwp70_highmet.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b2jos_log_.pdf}\DIFaddendFL &
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b3jos_log_.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b3jss_log_.pdf}\DIFaddendFL \\
...
\end{tabular}
\caption{\DIFdelbeginFL \DIFdelFL{The leading }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Leading }\DIFaddendFL $\thad$ \DIFdelbeginFL \DIFdelFL{$p_T$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\pt$  }\DIFaddendFL distributions \DIFdelbeginFL \DIFdelFL{are compared between }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{obtained before }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{fit to data (``Pre-Fit'') showing 
  the }\DIFaddendFL expected background and $tuH$ signals \DIFaddbeginFL \DIFaddFL{after applying fake factors }\DIFaddendFL in \DIFaddbeginFL \DIFaddFL{the following SRs}\DIFaddendFL : $t_{\ell}\thadhad$ (a),
 $t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$ same sign CR (f), }\DIFaddendFL $t_h\thadhad$-2j (\DIFdelbeginFL \DIFdelFL{f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{g}\DIFaddendFL ), \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_h\thadhad$-3j (\DIFdelbeginFL \DIFdelFL{g}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{h}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{before the fit to data }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and $t_h\thadhad$-3j same sign CR }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{'Pre-Fit'}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{i}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{after applying fake factors}\DIFdelendFL .
 The \DIFdelbeginFL \DIFdelFL{uncertainty band includes both }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{total size of }\DIFaddendFL the statistical and systematic uncertainties \DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL the background prediction \DIFaddbeginFL \DIFaddFL{is indicated by the hatched band}\DIFaddendFL .
 %DIF > The uncertainty band includes both the statistical and systematic uncertainties in the background prediction.
 Overflow \DIFdelbeginFL \DIFdelFL{bins }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{events }\DIFaddendFL are included \DIFdelbeginFL \DIFdelFL{respectively }\DIFdelendFL in the last bin. \DIFdelbeginFL \DIFdelFL{Others }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{``Other MC'' }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{``}\DIFaddendFL Rare\DIFaddbeginFL \DIFaddFL{''}\DIFaddendFL ) includes single top, and $V$+jets and other small backgrounds in the leptonic (hadronic) \DIFdelbeginFL \DIFdelFL{channels}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{channel}\DIFaddendFL . The $tuH$ signal \DIFdelbeginFL \DIFdelFL{shapes are also compared }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is scaled }\DIFaddendFL using a normalization factor \DIFdelbeginFL \DIFdelFL{(2-50).   
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of 2 to 50.   
...
The lower panels show the \DIFdelbeginFL \DIFdelFL{ratios of the }\DIFdelendFL data to \DIFdelbeginFL \DIFdelFL{the background }\DIFdelendFL prediction \DIFaddbeginFL \DIFaddFL{ratio}\DIFaddendFL .}
...
\centering
\begin{tabular}{@{}ccc@{}}
%DIF > \includegraphics[page=7,width=0.33\textwidth]{\FCNCFigures/tthML/showFake/faketau/postfit/NOMINAL_fancpaper/reg1l2tau1bnj_os/tau_pt_0.pdf} &
%DIF > \includegraphics[page=7,width=0.33\textwidth]{\FCNCFigures/tthML/showFake/faketau/postfit/NOMINAL_fancpaper/reg1l1tau1b1j_ss_vetobtagwp70_highmet/tau_pt_0.pdf}&
%DIF > \includegraphics[page=7,width=0.33\textwidth]{\FCNCFigures/tthML/showFake/faketau/postfit/NOMINAL_fancpaper/reg1l1tau1b2j_ss_vetobtagwp70_highmet/tau_pt_0.pdf}\\
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l2tau1bnj_os_log_mtt.pdf} &
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l1tau1b1j_ss_log_mtt.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l1tau1b2j_ss_log_mtt.pdf}\\
\DIFaddFL{(a) }& \DIFaddFL{(b) }& \DIFaddFL{(c) }\\
%DIF > (a1) \pT(\tauhad) in $t_{\ell}\thadhad$ & (a2) \pT(\tauhad) in  $t_{\ell}\tauhad$-1j& (a3) \pT(\tauhad) in $t_{\ell}\tauhad$-2j\\
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l1tau1b2j_os_log_mtt.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l1tau1b3j_os_log_mtt.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg1l2tau1bnj_ss_log_mtt.pdf}\\
\DIFaddFL{(d) }& \DIFaddFL{(e) }& \DIFaddFL{(f)}\\
%DIF > (b1) \pT(\tauhad) in $t_h\tlhad$-2j & (b2) \pT(\tauhad) in  $t_h\tlhad$-3j & (b3) \pT(\tauhad) in $t_h\thadhad$-2j \\
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg2mtau1b2jos_log_mtt.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg2mtau1b3jos_log_mtt.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/mtt/tuH_reg2mtau1b3jss_log_mtt.pdf}\\
\DIFaddFL{(g) }& \DIFaddFL{(h) }& \DIFaddFL{(i)}\\
%DIF > (c1) \pT(\tauhad) in $t_h\thadhad$-3j\\
\end{tabular}
\caption{\DIFaddFL{Visible di-tau mass  distributions obtained before the fit to data (``Pre-Fit'') showing 
  the expected background and $tuH$ signals after applying fake factors in the following SRs: $t_{\ell}\thadhad$ (a),
 $t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), $t_{\ell}\thadhad$ same sign CR (f), $t_h\thadhad$-2j (g), $t_h\thadhad$-3j (h) and $t_h\thadhad$-3j same sign CR (i).
...
%DIF > ( only subtau real refers to $t\bar{t}$ MC with leading tau faked by other object, subleading tau from real contribution. 
...
%DIF > (c1) \pT(\tauhad) in $t_h\thadhad$-3j\\
\end{tabular}
\caption{\DIFaddFL{Leading $\thad$ $\pt$  distributions obtained the fake $\thad$ calibration method after applying fake factors in the following CRs: 
...
Table~\ref{tab:had_sys_impact} shows a summary of the dominant systematics uncertainties on the measured \DIFdelbegin \DIFdel{$\BR(H\to qH)$ }\DIFdelend \DIFaddbegin \DIFadd{$\BR(t\to qH)$ }\DIFaddend branching ratio
...
%DIFDELCMD < %%%
\DIFdelend %The uncertainty on the pileup reweighing is evaluated by varying the pileup scale factors by 1$\sigma$ based on the reweighing of the average interactions per bunch crossing. However, this uncertainty is highly correlated with the luminosity uncertainty and may be an overestimate.
Scale factors are applied to reweight simulated events in order to obtain the pile-up distribution corresponding to data.
An uncertainty on these reweighting scale factors is \DIFdelbegin \DIFdel{considered based on the disagreement between the instantaneous luminosity in data and in simulation.
}\DIFdelend \DIFaddbegin \DIFadd{estimated by varying the reweighting to the profile in data within its uncertainties.
%DIF > considered based on the disagreement between the instantaneous luminosity in data and in simulation.
}\DIFaddend 
...
This is mainly \DIFdelbegin \DIFdel{limited by the LO }\DIFdelend \DIFaddbegin \DIFadd{driven by the disagreement between }\DIFaddend MC simulation in the $V$+jets \DIFdelbegin \DIFdel{high multiplicity regions}\DIFdelend \DIFaddbegin \DIFadd{regions with high multiplicity of jets}\DIFaddend .  
...
different fake tau sources and different $\pt$ slices. The uncertainties on the MC modelling of the various processes \DIFdelbegin \DIFdel{that used for
evaluating }\DIFdelend \DIFaddbegin \DIFadd{used for
evaluation of }\DIFaddend the fake factors in the \DIFdelbegin \DIFdel{CRs }\DIFdelend \DIFaddbegin \DIFadd{CRtt's }\DIFaddend are considered and treated \DIFdelbegin \DIFdel{fully correlated}\DIFdelend \DIFaddbegin \DIFadd{as fully correlated in the fit}\DIFaddend .
...
\DIFdelbegin \DIFdel{The uncertainties on the MC modelling of the various processes that used for
evaluating the fake factors in the CRs are treated fully correlated in the fit.
}\DIFdelend %DIF > The uncertainties on the MC modelling of the various processes used for
%DIF > evaluation of the fake factors in the CRtt's are treated as fully correlated in the fit.

\subsection{Signal modelling}
\label{sec:syst_sigmodeling}

Several normalisation and shape uncertainties are taken into account and are treated \DIFaddbegin \DIFadd{as }\DIFaddend fully correlated for the $\Hq$ and $pp\to tH$ signals.
...
\DIFaddbegin \DIFadd{The $pp\rightarrow tH$ production cross section is related to $\BR(t\rightarrow qH)$ based on their dimension-6 operators~\mbox{%DIFAUXCMD
\cite{fcnc_production_theory}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend Therefore, the expected total number of events in a given bin depends on $\mu$ and $\theta$. 
...
\DIFaddbegin \DIFadd{The background modelling in this signal region has also been checked using the VR in which both $\thad$ candidates have the same charge.
%DIF > The BDT distribution in this VR is shown in Fig.##,
...
}\DIFaddend %The fitted signal strength and its significance from individual channels and their combination are shown in Table~\ref{tab:limits_summary}.
...
\DIFaddbeginFL \includegraphics[width=0.3\textwidth]{figures/tuH_reg1l2tau1bnj_ss.pdf}\DIFaddendFL \\
...
\DIFaddbeginFL \includegraphics[width=0.3\textwidth]{figures/tuH_reg2mtau1b3jss.pdf}\DIFaddendFL \\
%(c1) BDT in$t_h\thadhad$-3j\\
%DIF < (g) & (h)  &  \\
\DIFdelbeginFL \DIFdelFL{(f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(g}\DIFaddendFL ) & (\DIFdelbeginFL \DIFdelFL{g}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{h}\DIFaddendFL )  & \DIFaddbeginFL \DIFaddFL{(i) }\DIFaddendFL \\
%DIF > (f) & (g)  &  \\
\end{tabular}
...
$t_{\ell}\thadhad$ (a),$t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$ same sign CR (f), }\DIFaddendFL $t_h\thadhad$-2j (\DIFdelbeginFL \DIFdelFL{f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{g}\DIFaddendFL ), \DIFaddbeginFL \DIFaddFL{$t_h\thadhad$-3j (h) }\DIFaddendFL and $t_h\thadhad$-3j \DIFaddbeginFL \DIFaddFL{same sign CR }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{g}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{i}\DIFaddendFL ).  
...
\centering
\begin{tabular}{@{}ccc@{}}
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/tthML/Limit/tuH_reg1l2tau1bnj_os_postFit.pdf}&
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/tthML/Limit/tuH_reg1l1tau1b1j_ss_postFit.pdf}&
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/tthML/Limit/tuH_reg1l1tau1b2j_ss_postFit.pdf}\\
%DIF < (a1) BDT discriminant in $t_{\ell}\thadhad$ & (a2) BDT discriminant in  $t_{\ell}\tauhad$-1j& (a3) BDT discriminant in $t_{\ell}\tauhad$-2j\\
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/tthML/Limit/tuH_reg1l1tau1b2j_os_postFit.pdf}&
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/tthML/Limit/tuH_reg1l1tau1b3j_os_postFit.pdf}&
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/xTFW/Limit/tuH_reg2mtau1b2jos_vetobtagwp70_highmet_postFit.pdf}\\
%DIF < (b1) BDT discriminant in $t_h\tlhad$-2j & (b2) BDT discriminant in  $t_h\tlhad$-3j & (b3) BDT discriminant in $t_h\thadhad$-2j \\
%DIF < \includegraphics[width=0.33\textwidth]{\FCNCFigures/xTFW/Limit/tuH_reg2mtau1b3jos_vetobtagwp70_highmet_postFit.pdf}&\\
...
%DIF <   $t_h\tlhad$-2j (b1), $t_h\tlhad$-3j (b2), $t_h\thadhad$-2j (b3), and $t_h\thadhad$-3j (c1). Statistical and systematic uncertainties are being shown.}
%DIF < \label{fig:asimov_postfitbdtHu}
\DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \includegraphics[width=0.3\textwidth]{figures/tcH_reg1l2tau1bnj_os.pdf}&
...
\DIFaddbeginFL \includegraphics[width=0.3\textwidth]{figures/tcH_reg1l2tau1bnj_ss.pdf}\DIFaddendFL \\
%\includegraphics[width=0.3\textwidth]{figures/tcH_reg1l2tau1bnj_ss.pdf}\\
%(b1) BDT in $t_h\tlhad$-2j & (b2) BDT in  $t_h\tlhad$-3j & (b3) BDT in $t_h\thadhad$-2j \\
%DIF < (d) & (e)  & (f) \\
(d) & (e)  \DIFaddbeginFL & \DIFaddFL{(f) }\DIFaddendFL \\
...
\DIFaddbeginFL \includegraphics[width=0.3\textwidth]{figures/tcH_reg2mtau1b3jss.pdf}\DIFaddendFL \\
%(c1) BDT in$t_h\thadhad$-3j\\
%DIF < (g) & (h)  &  \\
\DIFdelbeginFL \DIFdelFL{(f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(g}\DIFaddendFL ) & (\DIFdelbeginFL \DIFdelFL{g}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{h}\DIFaddendFL )  & \DIFaddbeginFL \DIFaddFL{(i) }\DIFaddendFL \\
%DIF > (f) & (g)  &  \\
...
\begin{table}[htbp]
\caption{
  Predicted and observed yields in each of the analysis regions considered. The background prediction is shown after a \DIFdelbeginFL \DIFdelFL{background only }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{background-only }\DIFaddendFL fit to data.
...
\DIFdelbegin \DIFdel{$\BR(t\to cH)<9.9 \times 10^{-4}\,(5.0 \times 10^{-4})$}\DIFdelend \DIFaddbegin \DIFadd{$\BR(t\to cH)<9.9 \times 10^{-4}\,(5.0^{+2.2}_{-1.4}\times10^{-4})$}\DIFaddend , assuming $\BR(t\to uH)=0$,and \DIFdelbegin \DIFdel{$\BR(t\to uH)<7.2 \times 10^{-4}\,(3.6 \times 10^{-4})$}\DIFdelend \DIFaddbegin \DIFadd{$\BR(t\to uH)<7.2 \times 10^{-4}\,(3.6^{+1.7}_{-1.0}\times10^{-4})$}\DIFaddend , assuming $\BR(t\to cH)=0$.
...
The main contributions to the total systematic uncertainty arise from  the \DIFaddbegin \DIFadd{size of the MC samples,
the }\DIFaddend uncertainties on the measured $\BR(H\to \tautau)$ branching ratio,
...
A summary of the upper limits, significance and best-fit values \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the branching ratios obtained by the individual searches, as well as their combination, is given  
...
\begin{table}[h!]
  \caption{\DIFdelbeginFL \DIFdelFL{List of absolute }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Absolute }\DIFaddendFL uncertainties on $\BR(t\to qH)$ ($q=u,c$) obtained from the combined fit to data. The uncertainties are symmetrised for
     \DIFdelbeginFL \DIFdelFL{\
 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL presentation \DIFaddbeginFL \DIFaddFL{purpose }\DIFaddendFL and grouped into the categories described in the \DIFdelbeginFL \DIFdelFL{text}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Section~\ref{sec:systematics}}\DIFaddendFL .}
\label{tab:had_sys_impact}
%DIF >  \begin{center}
%DIF >    \begin{tabular}{%
%DIF >        @{}l%
...
%DIF >      }
%DIF >      \toprule\toprule
%DIF >      \multirow{2}{*}{Source of uncertainty}      & \multicolumn{2}{c}{$\Delta\BR$ [$10^{-5}$]} \\
...
%DIF >      Signal modeling including Br($H\to\tau\tau$)            &5.3           &7.0         \\
%DIF >      $\tau$ ID                               & 3.3           &4.4         \\
%DIF >      Luminosity and Pileup                   & 0.9           &1.3         \\    
%DIF >      MC statistics              & 5.1           &7.0         \\\midrule
%DIF >      %Other MC modeling                       & 2.1           &2.9         \\
%DIF >      %JES and JER                             & 2.4           &3.2         \\
%DIF >      %Flavour tagging                         & 2.7           &3.7         \\
%DIF >      %$t\bar{t}$ modeling                     & 2.9           &4.3         \\
...
%DIF >      Data statistical uncertainty                           & 14.1          &19.6         \\\midrule
%DIF >      Total uncertainties                       & 18            &25        \\
%DIF >      %Total systematic  uncertainty            & 11.2          &15.5        \\
...
%DIF >  \end{table}
 \begin{center}
   \begin{tabular}{%
     ...
     & \multicolumn{1}{c}{$t\rightarrow uH$} & \multicolumn{1}{c}{$t\rightarrow cH$} \\\midrule
     Lepton ID                               & 0.6           &\DIFdelbeginFL \DIFdelFL{1.0         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.8         }\DIFaddendFL \\
     $\met$                                  & 0.7           &\DIFdelbeginFL \DIFdelFL{0.8         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.7         }\DIFaddendFL \\
     Fake lepton  modeling                   & \DIFdelbeginFL \DIFdelFL{0.9           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.2           }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{1.1         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.7         }\DIFaddendFL \\
     JES and JER                             & \DIFdelbeginFL \DIFdelFL{2.4           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.5           }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{3.2         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.3         }\DIFaddendFL \\
     Flavour tagging                         & 2.7           &3.7         \\
     $t\bar{t}$ modeling                     & \DIFdelbeginFL \DIFdelFL{2.9           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.6           }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{4.3         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.9         }\DIFaddendFL \\
     Other MC modeling                       & 2.1           &\DIFdelbeginFL \DIFdelFL{2.9         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.0         }\DIFaddendFL \\
     Fake $\tau$ modeling                    & \DIFdelbeginFL \DIFdelFL{3.2           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.3           }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{4.6         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.7         }\DIFaddendFL \\
     Signal modeling including Br($H\to\tau\tau$)&\DIFdelbeginFL \DIFdelFL{5.3           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.8        }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{7.0         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.5         }\DIFaddendFL \\
     $\tau$ ID                               & 3.3           &4.4         \\
     Luminosity and Pileup                   & \DIFdelbeginFL \DIFdelFL{0.9           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.7           }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{1.3         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.4         }\DIFaddendFL \\    
     MC statistics                           & 5.1           &\DIFdelbeginFL \DIFdelFL{7.0         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7.1         }\DIFaddendFL \\\midrule
     %DIF < Other MC modeling                       & 2.1           &2.9         \\
    %DIF < JES and JER                             & 2.4           &3.2         \\
    %DIF < Flavour tagging                         & 2.7           &3.7         \\
    %DIF < $t\bar{t}$ modeling                     & 2.9           &4.3         \\
    ...
    %DIF < $\tau$ ID                               & 3.3           &4.4         \\
    %DIF < Signal modeling including Br($H\to\tau\tau$)       & 5.3           &7.0         \\\midrule
    \DIFdelbeginFL \DIFdelFL{Sum systematics                           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Total systematic uncertainty            }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{11.2          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.1          }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{15.5        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{14.1        }\DIFaddendFL \\
     Data \DIFdelbeginFL \DIFdelFL{statistics                           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{statistical uncertainty            }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{14.1          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{14.9          }\DIFaddendFL &\DIFdelbeginFL \DIFdelFL{19.6         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{19.4        }\DIFaddendFL \\\midrule
     Total uncertainties                     & 18            &\DIFdelbeginFL \DIFdelFL{25        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{24          }\DIFaddendFL \\
     %DIF < Total systematic  uncertainty            & 11.2          &15.5        \\
    %DIF < Total statistical uncertainty            & 14.1          &19.6         \\\midrule
    %DIF < Total                                    & 18            &25        \\
    \bottomrule\bottomrule
...
%DIF >       \begin{table}[t!]
%DIF >         \caption{\small{Summary of 95\% CL upper limits on $\BR(t \to cH)$ and $\BR(t \to uH)$, significance and best-fit branching ratio in signal regions with a
%DIF >         benchmark branching ratio of $\BR(t \to qH)=0.1\%$}. Expected significance is obtained from Asimov fit with a signal injection corresponding to a branching ratio of 0.1\%. }
%DIF >       \begin{center}
%DIF >       \resizebox{\textwidth}{!}{
%DIF >       \begin{tabular}{lcccccc}
%DIF >       \toprule\toprule
%DIF >       
%DIF >       \multirow{3}{*}{Signal Regions} & \multicolumn{3}{c}{$t\to cH$}                                & \multicolumn{3}{c}{$t\to uH$}  \\
%DIF >                                       &  95\% CL upper limits[$10^{-3}$]  & Significance             &  $\BR[10^{-3}]$ &     95\% CL upper limits[$10^{-3}$]  & Significance   &  $\BR%[10^{-      3}]$  \\
%DIF >                                       &  \multicolumn{2}{c}{Observed (Expected)}                     &        &     \multicolumn{2}{c}{Observed (Expected)}& \\
%DIF >       \midrule
%DIF >       $t_h\thadhad$-2j                & $1.85$($2.80^{+1.30}_{-0.78}$)&$-0.96$($0.78$) & $-1.03^{+1.04}_{-1.04}$&$1.10$($1.65^{+0.79}_{-0.46}$)&$-0.90$($1.25$) &$-0.55^{+0.59}_{-0.59%}$ \\
%DIF >       $t_h\thadhad$-3j                & $1.18$($1.06^{+0.50}_{-0.30}$)&$0.34$($1.87$)  & $0.16^{+0.47}_{-0.47}$ &$1.00$($0.89^{+0.42}_{-0.25}$)&$0.36$($2.13$)  &$0.14^{+0.40}_{-0.40}%$  \\       \midrule
%DIF >       Hadronic Combination            & $1.04$($0.98^{+0.46}_{-0.28}$)&$0.26$ ($1.99$) & $0.11^{+0.43}_{-0.43}$ &$0.78$($0.78^{+0.37}_{-0.22}$)&$0.11$($2.33$)  &$0.04^{+0.34}_{-0.34}%$  \\
%DIF >       \midrule
%DIF >       $t_l\tauhad$-2j                 &$4.86$($4.32^{+1.89}_{-1.21}$) &$0.40$($0.48$)   &$0.81^{+2.04}_{-2.04}$  & $3.93$($3.55^{+1.56}_{-0.99}$) & $0.34$($0.58$)  &$0.57^{+1.66}_{-%1.66}$      \\
%DIF >       $t_l\tauhad$-1j                 &$3.94$($3.67^{+1.66}_{-1.03}$) &$0.24$($0.57$)   &$0.40^{+1.70}_{-1.70}$  & $3.10$($2.87^{+1.29}_{-0.80}$) & $0.24$($0.73$)  &$0.31^{+1.33}_{-%1.33}$      \\
%DIF >       $t_h\tlhad$-2j                  &$4.81$($5.85^{+2.90}_{-1.63}$) &$-0.52$($0.39$)  &$-1.36^{+2.56}_{-2.56}$ & $2.56$($3.05^{+1.38}_{-0.85}$) & $-0.48$($0.69$) &$-0.66^{+1.38}_{-%1.38}$      \\
%DIF >       $t_h\tlhad$-3j                  &$2.78$($2.79^{+1.36}_{-0.78}$) &$-0.04$($0.76$)  &$-0.04^{+1.26}_{-1.26}$ & $2.07$($2.09^{+0.94}_{-0.58}$) & $-0.05$($0.98$) &$-0.04^{+0.98}_{-%0.98}$      \\
%DIF >       $t_l\thadhad$                   &$1.41$($0.63^{+0.29}_{-0.18}$) &$2.64$($3.24$)   &$0.74^{+0.34}_{-0.34}$  & $1.01$($0.45^{+0.21}_{-0.13}$) & $2.64$($4.08$)  &$0.53^{+0.25}_{-%0.25}$      \\ \midrule
%DIF >       Leptonic Combination            &$1.29$($0.59^{+0.27}_{-0.17}$) &$2.59$($3.34$)   &$0.68^{+0.32}_{-0.32}$  & $0.92$($0.42^{+0.19}_{-0.12}$) & $2.59$($4.23$)  &$0.48^{+0.23}_{-%0.23}$      \\
%DIF >       \midrule
%DIF >       Combination                     &$0.99$ ($0.50^{+0.22}_{-0.14}$)&$2.34$($3.69$)   &$0.51^{+0.25}_{-0.25}$ & $0.72$ ($0.36^{+0.17}_{-0.10}$)& $2.31$($4.49$)&$0.37^{+0.18}_{-0.18%}$  \\
%DIF >       \bottomrule\bottomrule
%DIF >       \end{tabular}
%DIF >       }
...
%DIF > \begin{table}
%DIF > \caption{ Summary of fake tau scale factors derived in ttCRs. The numbers are shown as: nominal values,statistical errors and systematics erros. }
%DIF > \begin{center}
%DIF > \begin{tabular}{lcccccc}
%DIF > \toprule\toprule
%DIF > 
%DIF > \multirow{2}{*}{Fake Factor Types} & \multicolumn{3}{c}{1 prong}                                                        & \multicolumn{3}{c}{3 prong}  \\
%DIF >                                 &  $25-35$ GeV  & $35-45$ GeV  &  $45-$ GeV                                             &  $25-35$ GeV  & $35-45$ GeV       &  $45-$ GeV  \\
%DIF > \midrule
%DIF > Type-1                          &$0.71 \pm 0.01 \pm 0.03 $ &$0.61 \pm 0.02 \pm 0.04 $ &$0.38 \pm 0.02 \pm 0.05 $        & $1.01 \pm 0.03 \pm 0.04 $ & $1.09 \pm 0.04 \pm 0.05 $ & $0.30 \pm 0.05 \pm 0.07 $ \\
%DIF > % W fake OS       
%DIF > Type-2                          &$0.76 \pm 0.06 \pm 0.04 $ & $0.37 \pm 0.08 \pm 0.02$ & $0.74 \pm 0.08 \pm 0.02 $       & $0.93 \pm 0.10 \pm 0.04 $ & $1.05 \pm 0.09 \pm 0.03 $ & $0.79 \pm 0.09 \pm 0.04 $ \\
%DIF > % W fake SS          
%DIF > Type-3                          &$0.62 \pm 0.10 \pm 0.03 $ &$0.83 \pm 0.09 \pm 0.03 $ &$0.94 \pm 0.07 \pm 0.02 $        & $1.07 \pm 0.13 \pm 0.03 $ &$1.39 \pm 0.12 \pm 0.03 $ &$1.26 \pm 0.10 \pm 0.04 $  \\
%DIF > %  b fake       
%DIF > Type-4                          &$1.20 \pm 0.02 \pm 0.01 $ & $1.01 \pm 0.04 \pm 0.02 $ &$0.76 \pm 0.03 \pm 0.03 $       &$1.28 \pm 0.07 \pm 0.02 $ &$0.66 \pm 0.08 \pm 0.01 $ & $0.71 \pm 0.07 \pm 0.02 $ \\
%DIF > % other fake
%DIF > \bottomrule\bottomrule
%DIF > \end{tabular}
%DIF > \label{tab:ff_summary}
%DIF > \end{center}
%DIF > \end{table}
\DIFaddbegin 
...
\begin{table}
\caption{ \DIFaddFL{Summary of fake tau (1-prong) scale factors derived in ttCRs. The numbers are shown as: nominal values,statistical errors and systematics erros. }}
\begin{center}
\begin{tabular}{lcccccc}
\toprule\toprule

\multirow{2}{*}{Fake Factor Types} & \multicolumn{3}{c}{1 prong}  \\                                                
                                   &  \DIFaddFL{$25-35$ GeV  }& \DIFaddFL{$35-45$ GeV  }&  \DIFaddFL{$45-$ GeV            }\\                           
\midrule
\DIFaddFL{Type-1                          }&\DIFaddFL{$0.71 \pm 0.01 \pm 0.03 $ }&\DIFaddFL{$0.61 \pm 0.02 \pm 0.04 $ }&\DIFaddFL{$0.38 \pm 0.02 \pm 0.05 $  }\\
%DIF >  W fake OS       
\DIFaddFL{Type-2                          }&\DIFaddFL{$0.76 \pm 0.06 \pm 0.04 $ }& \DIFaddFL{$0.37 \pm 0.08 \pm 0.02$ }& \DIFaddFL{$0.74 \pm 0.08 \pm 0.02 $ }\\
%DIF >  W fake SS          
\DIFaddFL{Type-3                          }&\DIFaddFL{$0.62 \pm 0.10 \pm 0.03 $ }&\DIFaddFL{$0.83 \pm 0.09 \pm 0.03 $ }&\DIFaddFL{$0.94 \pm 0.07 \pm 0.02 $  }\\
%DIF >   b fake       
\DIFaddFL{Type-4                          }&\DIFaddFL{$1.20 \pm 0.02 \pm 0.01 $ }& \DIFaddFL{$1.01 \pm 0.04 \pm 0.02 $ }&\DIFaddFL{$0.76 \pm 0.03 \pm 0.03 $  }\\
%DIF >  other fake
\bottomrule\bottomrule
\end{tabular}
...
\begin{table}
\caption{ \DIFaddFL{Summary of fake tau (3-prong) scale factors derived in ttCRs. The numbers are shown as: nominal values,statistical errors and systematics erros. }}
\begin{center}
\begin{tabular}{lcccccc}
\toprule\toprule
...
\multirow{2}{*}{Fake Factor Types}        & \multicolumn{3}{c}{3 prong}  \\
                                          &  \DIFaddFL{$25-35$ GeV  }& \DIFaddFL{$35-45$ GeV       }&  \DIFaddFL{$45-$ GeV  }\\
\midrule
\DIFaddFL{Type-1                                    }& \DIFaddFL{$1.01 \pm 0.03 \pm 0.04 $ }& \DIFaddFL{$1.09 \pm 0.04 \pm 0.05 $ }& \DIFaddFL{$0.30 \pm 0.05 \pm 0.07 $ }\\
%DIF >  W fake OS       
\DIFaddFL{Type-2                                    }& \DIFaddFL{$0.93 \pm 0.10 \pm 0.04 $ }& \DIFaddFL{$1.05 \pm 0.09 \pm 0.03 $ }& \DIFaddFL{$0.79 \pm 0.09 \pm 0.04 $ }\\
%DIF >  W fake SS          
\DIFaddFL{Type-3                                    }& \DIFaddFL{$1.07 \pm 0.13 \pm 0.03 $ }&\DIFaddFL{$1.39 \pm 0.12 \pm 0.03 $ }&\DIFaddFL{$1.26 \pm 0.10 \pm 0.04 $  }\\
%DIF >   b fake       
\DIFaddFL{Type-4                                    }&\DIFaddFL{$1.28 \pm 0.07 \pm 0.02 $ }&\DIFaddFL{$0.66 \pm 0.08 \pm 0.01 $ }& \DIFaddFL{$0.71 \pm 0.07 \pm 0.02 $ }\\
%DIF >  other fake
\bottomrule\bottomrule
\end{tabular}
...
95\% CL upper limits are set on the \DIFdelbegin \DIFdel{$t\to Hq$ }\DIFdelend \DIFaddbegin \DIFadd{$t\to qH$ }\DIFaddend branching ratios and the corresponding
...
%DIF > These results improve significantly upon previous ATLAS and CMS limits on the $t\rightarrow Hq$ branching ratios.
These results improve significantly \DIFdelbegin \DIFdel{upon previous ATLAS and CMS limits on the $t\rightarrow Hq$ branching ratios.
}\DIFdelend \DIFaddbegin \DIFadd{previously published ATLAS studies in this channel and provide more stringent limit than the ATLAS  $t\rightarrow qH$ combination~\mbox{%DIFAUXCMD
...
%DIFAUXCMD
\DIFdelend %DIF > \section*{Acknowledgements}
%-------------------------------------------------------------------------------
%DIF <  Acknowledgements for papers with collision data
%DIF <  Version 24-Oct-2018
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Standard acknowledgements start here
%DIF < ----------------------------------------------
\DIFdel{We thank CERN for the very successful operation of the LHC, as well as the
support staff from our institutions without whom ATLAS could not be
operated efficiently.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We acknowledge the support of ANPCyT, Argentina; YerPhI, Armenia; ARC, Australia; BMWFW and FWF, Austria; ANAS, Azerbaijan; SSTC, Belarus; CNPq and FAPESP, Brazil; NSERC, NRC and CFI, Canada; CERN; CONICYT, Chile; CAS, MOST and NSFC, China; COLCIENCIAS, Colombia; MSMT CR, MPO CR and VSC CR, Czech Republic; DNRF and DNSRC, Denmark; IN2P3-CNRS, CEA-DRF/IRFU, France; SRNSFG, Georgia; BMBF, HGF, and MPG, Germany; GSRT, Greece; RGC, Hong Kong SAR, China; ISF and Benoziyo Center, Israel; INFN, Italy; MEXT and JSPS, Japan; CNRST, Morocco; NWO, Netherlands; RCN, Norway; MNiSW and NCN, Poland; FCT, Portugal; MNE/IFA, Romania; MES of Russia and NRC KI, Russian Federation; JINR; MESTD, Serbia; MSSR, Slovakia; ARRS and MIZ\v{S}, Slovenia; DST/NRF, South Africa; MINECO, Spain; SRC and Wallenberg Foundation, Sweden; SERI, SNSF and Cantons of Bern and Geneva, Switzerland; MOST, Taiwan; TAEK, Turkey; STFC, United Kingdom; DOE and NSF, United States of America. In addition, individual groups and members have received support from BCKDF, CANARIE, CRC and Compute Canada, Canada; COST, ERC, ERDF, Horizon 2020, and Marie Sk}%DIFDELCMD < {%%%
\DIFdel{\l}%DIFDELCMD < }%%%
...
%DIFDELCMD < %%%
\DIFdel{The crucial computing support from all WLCG partners is acknowledged gratefully, in particular from CERN, the ATLAS Tier-1 facilities at TRIUMF (Canada), NDGF (Denmark, Norway, Sweden), CC-IN2P3 (France), KIT/GridKA (Germany), INFN-CNAF (Italy), NL-T1 (Netherlands), PIC (Spain), ASGC (Taiwan), RAL (UK) and BNL (USA), the Tier-2 facilities worldwide and large non-WLCG resource providers. Major contributors of computing resources are listed in Ref.~\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend %DIF > \input{acknowledgements/Acknowledgements}
