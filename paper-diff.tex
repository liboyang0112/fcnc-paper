...
%DIF DEL ../fcnc-paper_circulation2_before_language/ANA-TOPQ-2019-17-PAPER.tex   Fri Jun  3 08:50:32 2022
%DIF ADD ./ANA-TOPQ-2019-17-PAPER.tex                                            Tue Jun  7 07:01:19 2022
...
%\documentclass[CONF, atlasdraft=false, texlive=2016, UKenglish]{\ATLASLATEXPATH atlasdoc}
...
%DIF 105c105
%DIF < \AtlasVersion{1.4}
%DIF -------
\AtlasVersion{2.1} %DIF > 
%DIF -------
% Abstract - % directly after { is important for correct indentation
\AtlasAbstract{%
%DIF 108c108
%DIF < A search for flavour-changing neutral current (FCNC) $tqH$ interactions involving a top quark, an up-type quark ($q=u, c$), and a
%DIF -------
A search for flavour-changing neutral current (FCNC) $tqH$ interactions involving a top quark, another up-type quark ($q=u,\, c$), and a %DIF > 
%DIF -------
Standard Model (SM) Higgs boson decaying into a $\tau$-lepton pair ($H\rightarrow \tau^+\tau^-$) is presented.
%DIF 110-111c110-111
%DIF < The search is based on a dataset of $pp$ collisions at $\sqrt{s}=13~\tev$ recorded with the ATLAS detector at the 
%DIF < Large Hadron Collider that corresponds to an integrated luminosity of 139 fb$^{-1}$.
%DIF -------
The search is based on a dataset of $pp$ collisions at $\sqrt{s}=13~\tev$ that corresponds to an integrated luminosity of 139 fb$^{-1}$ recorded with the ATLAS detector at the  %DIF > 
Large Hadron Collider. %DIF > 
%DIF -------
...
%DIF 115c115
%DIF < with an additional lepton (e,$\mu$),
%DIF -------
with an additional lepton ($e$, $\mu$), %DIF > 
%DIF -------
...
%DIF 117c117
%DIF < A slight excess of data is observed above the expected SM background with a significance of 2.3$\sigma$,
%DIF -------
A slight excess of data is observed with a significance of 2.3$\sigma$ above the expected SM background, %DIF > 
%DIF -------
and 95\% CL upper limits on the $t\to qH$ branching ratios are derived.
%DIF 119c119
%DIF < Observed (expected) 95\% CL upper limits are set on the $t\to cH$ and $t\to uH$ branching ratios of $9.4 \times 10^{-4}\, (4.8^{+2.2}_{-1.4}\times 10^{-4})$
%DIF -------
The observed (expected) 95\% CL upper limits set on the $t\to cH$ and $t\to uH$ branching ratios are $9.4 \times 10^{-4}\, (4.8^{+2.2}_{-1.4}\times 10^{-4})$ %DIF > 
%DIF -------
...
%DIF 122c122
%DIF < the effective $tqH$ couplings are $C_{c\phi} <1.35\, (0.97)$ and $C_{u\phi} <1.16\, (0.82)$, respectively.}%for the new physics scale $\Lambda$ at 1~TeV, respectively.} 
%DIF -------
the effective $tqH$ couplings are $C_{c\phi} <1.35\, (0.97)$ and $C_{u\phi} <1.16\, (0.82)$.}%for a new-physics scale $\Lambda$ of 1~$\tev$, respectively.}  %DIF > 
%DIF -------
...
%DIF 158c158
%DIF < \AtlasCoverSupportingNote{Search for flavor-changing neutral currents tHq interactions with, $H \to \tau^+\tau^-$}{https://cds.cern.ch/record/2687866}
%DIF -------
\AtlasCoverSupportingNote{Search for flavor-changing neutral currents $tqH$ interactions with, $H \to \tau^+\tau^-$}{https://cds.cern.ch/record/2687866} %DIF > 
%DIF -------
%
...
% Comment deadline
%DIF 166c166
%DIF < \AtlasCoverCommentsDeadline{9 May 2022}
%DIF -------
\AtlasCoverCommentsDeadline{14 June 2022} %DIF > 
%DIF -------
...
\label{sec:intro}
%-------------------------------------------------------------------------------
\DIFdelbegin \DIFdel{After }\DIFdelend \DIFaddbegin \DIFadd{Since }\DIFaddend the discovery of the Higgs boson in 2012 at the Large Hadron Collider (LHC) by the ATLAS~\cite{Aad:2012tfa} and 
...
However, the \DIFdelbegin \DIFdel{program }\DIFdelend \DIFaddbegin \DIFadd{programme }\DIFaddend is ongoing and precision measurements as well as searches for rare \DIFdelbegin \DIFdel{new physics }\DIFdelend \DIFaddbegin \DIFadd{new-physics }\DIFaddend processes beyond the Standard Model (BSM)
are underway. One such possibility is flavour-changing \DIFdelbegin \DIFdel{neutral-current }\DIFdelend \DIFaddbegin \DIFadd{neutral current }\DIFaddend (FCNC) interactions between the Higgs boson, 
the top quark, and an up-type quark, $tqH$ ($q=u, c$), which \DIFdelbegin \DIFdel{has been actively }\DIFdelend \DIFaddbegin \DIFadd{have been }\DIFaddend searched for by the ATLAS and CMS collaborations.  
...
In the Standard Model (SM), the FCNC interaction is forbidden at tree level and suppressed at higher orders \DIFdelbegin \DIFdel{due to the Glashow-Iliopoulos-Maiani }\DIFdelend \DIFaddbegin \DIFadd{through the Glashow--Iliopoulos--Maiani }\DIFaddend (GIM) mechanism~\cite{Glashow:1970gm}. \DIFdelbegin \DIFdel{Therefore, an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend observation of an enhanced rate of this decay would be \DIFdelbegin \DIFdel{a clear evidence for }\DIFdelend \DIFaddbegin \DIFadd{clear evidence of }\DIFaddend new physics.
Furthermore, if the $tqH$ interaction exists, the associated single-top and  Higgs  production process through this interaction would enhance
%becomes non-zero, enhancing
...
However, these branching ratios can be as high as $\BR(t\to qH) \sim 10^{-5}$ when \DIFdelbegin \DIFdel{considering }\DIFdelend \DIFaddbegin \DIFadd{processes }\DIFaddend beyond the SM \DIFdelbegin \DIFdel{processes}\DIFdelend \DIFaddbegin \DIFadd{are included}\DIFaddend . Examples of these processes include: quark-singlet models~\cite{AguilarSaavedra:2002kr}, two-Higgs-doublet models (\DIFdelbegin \DIFdel{2HDM}\DIFdelend \DIFaddbegin \DIFadd{2HDMs}\DIFaddend )~\cite{ Branco:2hdm2012} with or without flavour violation,
...
\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend warped extra dimensions models with SM fermions in the bulk~\cite{Azatov:2009na}. 
...
An even larger branching ratio of  $\BR(t\to cH) \sim 10^{-3}$ can be reached in \DIFdelbegin \DIFdel{2HDM }\DIFdelend \DIFaddbegin \DIFadd{2HDMs }\DIFaddend without explicit flavour conservation, since \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{in these models the }\DIFaddend tree-level FCNC coupling is no longer forbidden by any symmetry~\cite{Cheng:1987rs,Baum:2008qm,Chen:2013qta,Chiang:2015cba,Crivellin:2015hha,Botella:2015hoa, Gori:2017tvg,Chiang:2017fjr}. 
...
%the total
%production cross section of $pp\rightarrow tH$.
The study of $pp\rightarrow tH$ \DIFdelbegin \DIFdel{process }\DIFdelend \DIFaddbegin \DIFadd{processes }\DIFaddend will therefore also contribute to the FCNC interaction searches~\cite{Greljo:2014dka}.
\DIFdelbegin \DIFdel{The $tH$ associated production in the SM }\DIFdelend \DIFaddbegin \DIFadd{In the SM, associated production of $tH$ in $pp$ collisions }\DIFaddend is expected to have a cross section of \DIFdelbegin \DIFdel{$92^{+7}_{-12}$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_{tH}=92^{+7}_{-12}$ }\DIFaddend fb at a centre-of-mass energy of $\sqrt{s}= 13~\tev$~\cite{deFlorian:2016spz}.

Searches for $t \to qH$ decays have been performed by the ATLAS and CMS collaborations, taking advantage of the large samples
of top-quark pair ($\ttbar$) events collected in \DIFdelbegin \DIFdel{proton-proton }\DIFdelend \DIFaddbegin \DIFadd{proton--proton }\DIFaddend ($pp$) collisions at centre-of-mass energies of $\sqrt{s}=7~\tev$ and $8~\tev$~\cite{Aad:2014dya,Aad:2015pja,Khachatryan:2016atv} during Run~1 of the LHC, as well as at $\sqrt{s}=13~\tev$~\cite{fcnc36} using early Run~2 data.
In these searches, one of the top quarks is required to decay into $Wb$, while the other top quark decays into $qH$ with a small branching ratio 
 $\BR(t\to qH)$, \DIFdelbegin \DIFdel{denoted as }\DIFdelend \DIFaddbegin \DIFadd{a process denoted by }\DIFaddend $\ttbar \to WbHq$\DIFdelbegin \footnote{\DIFdel{In the following, $WbHq$ is used to denote both $W^+b H\bar{q}$ and its charge conjugate, $HqW^- \bar{b}$. Similarly, 
$WbWb$ is used to denote $W^+b W^- \bar{b}$.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{.}\footnote{\DIFadd{In the following, $WbHq$ is used to denote both $W^+b H\bar{q}$ and its charge conjugate, $HqW^- \bar{b}$. Similarly, 
$WbWb$ is used to denote $W^+b W^- \bar{b}$.}}  \DIFaddend The Higgs boson is assumed to have a mass of $m_H=125~\gev$ and to decay as predicted by
the SM.
...
Using 36.1~fb$^{-1}$ of data at $\sqrt{s}=13~\tev$, the ATLAS Collaboration has derived upper limits at 95\% confidence level (CL) \DIFdelbegin \DIFdel{of branching ratio}\DIFdelend \DIFaddbegin \DIFadd{on the 
$t\to cH$ branching ratio: }\DIFaddend $\BR(t\to cH)<0.22\%$ using $H\to \gamma\gamma$ decays~\cite{Aaboud:2017mfd} \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\BR(t\to cH)<0.16\%$ based on
%signatures with two same-charge light leptons (electrons or muons) or three light leptons 
\DIFdelbegin \DIFdel{multilepton }\DIFdelend \DIFaddbegin \DIFadd{multi-lepton }\DIFaddend (electron or muon) signatures resulting from 
$H \to  WW^*, ZZ^*$, \DIFdelbegin \DIFdel{$H\to \tau^+\tau^-$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau^+\tau^-$ }\DIFaddend in which both $\tau$-leptons decay leptonically~\cite{Aaboud:2018pob}\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
ATLAS also set upper limits of }\DIFaddend $\BR(t\to cH)<0.42\%$ using $H\to b\bar{b}$ decay~\cite{fcnc36} \DIFdelbegin \DIFdel{, and of }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\BR(t\to cH)<0.19\%$ using $H\to \tau^+\tau^-$ \DIFdelbegin \DIFdel{decay }\DIFdelend \DIFaddbegin \DIFadd{decays }\DIFaddend in which at least
one of \DIFaddbegin \DIFadd{the }\DIFaddend $\tau$-leptons decays hadronically~\cite{fcnc36}.  
These upper limits are derived assuming that the branching ratio \DIFdelbegin \DIFdel{$\BR(t\to uH)$ is null}\DIFdelend \DIFaddbegin \DIFadd{$\BR(t\to uH)=0$}\DIFaddend . Similar upper limits are obtained for $\BR(t\to uH)$ \DIFaddbegin \DIFadd{when }\DIFaddend assuming $\BR(t\to cH)=0$.
...
Combining all of the ATLAS searches using 36.1~fb$^{-1}$ \DIFaddbegin \DIFadd{of }\DIFaddend Run~2 data, upper limits at 95\% CL on the branching fractions are 
set at $\BR(t\to cH)<0.11\%$ assuming $\BR(t\to uH)=0$, and \DIFaddbegin \DIFadd{at }\DIFaddend $\BR(t\to uH)<0.12\%$ assuming $\BR(t\to cH)=0$~\cite{fcnc36}.
...
The search in Ref.~\cite{Sirunyan:2017uae} \DIFdelbegin \DIFdel{considers in addition }\DIFdelend \DIFaddbegin \DIFadd{also considers }\DIFaddend the contribution to the signal from 
$pp \to tH$ production~\cite{Greljo:2014dka}. 
CMS \DIFdelbegin \DIFdel{recently }\DIFdelend \DIFaddbegin \DIFadd{subsequently }\DIFaddend updated their search for $tqH$ in the $H\to b\bar{b}$ channel using
the full Run\DIFaddbegin \DIFadd{~}\DIFaddend 2 dataset\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{, obtaining }\DIFaddend observed (expected) upper limits \DIFdelbegin \DIFdel{on  $\BR(t\to cH)$ are $9.4\times10^{-4}\, (8.6\times10^{-4})$
and are $7.9\times10^{-4}\, (1.1\times10^{-3})$ for $\BR{B}(t\to uH)$}\DIFdelend \DIFaddbegin \DIFadd{of  $\BR(t\to cH) < 9.4\times10^{-4}\, (8.6\times10^{-4})$
and $\BR(t\to uH) < 7.9\times10^{-4}\, (1.1\times10^{-3})$}\DIFaddend ~\cite{CMS:2021gfa}.
...
\begin{table}[t!]
\caption{\DIFdelbeginFL %DIFDELCMD < \small{Summary of 95\% CL upper limits on $\BR(t \to cH)$ and $\BR(t \to uH)$ obtained from ATLAS and CMS Collaborations with Run 2 data. Each limit is obtained assuming the other branching ratio is null.}%%%
...
The analysis reported here targets Higgs boson decays \DIFdelbegin \DIFdel{to 
}\DIFdelend \DIFaddbegin \DIFadd{into 
}\DIFaddend %This paper is focused on the decay mode of the Higgs boson into
$\tau$-leptons \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the complete Run~2 dataset collected \DIFdelbegin \DIFdel{in 2015-2018}\DIFdelend \DIFaddbegin \DIFadd{by ATLAS in 2015--2018}\DIFaddend . The corresponding integrated luminosity is 139 fb$^{-1}$. Both $\ttbar \to WbHq$ \DIFdelbegin \DIFdel{decay }\DIFdelend \DIFaddbegin \DIFadd{decays }\DIFaddend and $pp \to tH$ production are \DIFdelbegin \DIFdel{targeted}\DIFdelend \DIFaddbegin \DIFadd{sought}\DIFaddend . The dataset is divided into several final states depending on the production mode and
the  $W$ \DIFdelbegin \DIFdel{and tau }\DIFdelend \DIFaddbegin \DIFadd{boson and $\tau$-lepton }\DIFaddend decays. Top quark pair production events where the $W$ boson decays hadronically (leptonically) are \DIFdelbegin \DIFdel{referred to as }\DIFdelend \DIFaddbegin \DIFadd{denoted by }\DIFaddend $t_h$ ($t_{\ell}$).
The decays $\tau \to \ell \nu_{\ell} \nu_{\tau}$ are \DIFdelbegin \DIFdel{referred as }\DIFdelend \DIFaddbegin \DIFadd{denoted by }\DIFaddend $\lep$, while the decays $\tau \to \text{hadrons} + \nu_{\tau}$ are \DIFdelbegin \DIFdel{referred as }\DIFdelend \DIFaddbegin \DIFadd{denoted by }\DIFaddend $\thad$.
The contribution of 
$W\rightarrow\tau\nu$ is included as $\lep$ in $t_{\ell}$ when the $\tau$\DIFaddbegin \DIFadd{-lepton }\DIFaddend decays into a light lepton (electron or muon) or as $\thad$ 
in $t_h$ when the $\tau$\DIFaddbegin \DIFadd{-lepton }\DIFaddend decays hadronically.
...
either \DIFaddbegin \DIFadd{the }\DIFaddend $\tlhad$ or $\thadhad$ final \DIFdelbegin \DIFdel{states }\DIFdelend \DIFaddbegin \DIFadd{state }\DIFaddend when the top \DIFaddbegin \DIFadd{quark }\DIFaddend decays hadronically ($t_h$), but only the $\thadhad$ final state is considered
when the top \DIFdelbegin \DIFdel{decay }\DIFdelend \DIFaddbegin \DIFadd{quark decays }\DIFaddend leptonically ($t_{\ell}$)\DIFaddbegin \DIFadd{, }\DIFaddend in order to avoid overlaps with
other ATLAS searches~\cite{Aaboud:2018pob}. 
\DIFdelbegin \DIFdel{Events }\DIFdelend \DIFaddbegin \DIFadd{In this search, events }\DIFaddend with two hadronically decaying $\tau$-leptons \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{and no }\DIFaddend electron or muon \DIFdelbegin \DIFdel{are named hadronic channels}\DIFdelend \DIFaddbegin \DIFadd{define the hadronic channel}\DIFaddend . Events with at least one \tauhad \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend an additional electron or muon (corresponding to $t_{\ell}$ or $\lep$ events) are \DIFdelbegin \DIFdel{named }\DIFdelend \DIFaddbegin \DIFadd{assigned to }\DIFaddend leptonic channels. More signal regions (with \DIFdelbegin \DIFdel{top quark decay }\DIFdelend \DIFaddbegin \DIFadd{the top quark decaying }\DIFaddend leptonically) are exploited here \DIFdelbegin \DIFdel{compared to }\DIFdelend \DIFaddbegin \DIFadd{than in }\DIFaddend the previous FCNC $\Htautau$ search\DIFdelbegin \DIFdel{conducted using }\DIFdelend \DIFaddbegin \DIFadd{, which was conducted using the }\DIFaddend partial Run~2 \DIFdelbegin \DIFdel{data}\DIFdelend \DIFaddbegin \DIFadd{dataset}\DIFaddend ~\cite{fcnc36}.
In addition, an improved treatment of misidentified $\tau$-leptons \DIFaddbegin \DIFadd{(`fakes') }\DIFaddend in simulation and \DIFaddbegin \DIFadd{in }\DIFaddend data-driven
\DIFdelbegin \DIFdel{fake estimations }\DIFdelend \DIFaddbegin \DIFadd{estimations of fakes }\DIFaddend from multi-jet background is implemented.
% and more
...
Finally, a multivariate technique based on boosted decision trees \DIFdelbegin \DIFdel{, }\DIFdelend is used to discriminate between the signal and the background on the basis of their different \DIFdelbegin \DIFdel{kinematics }\DIFdelend \DIFaddbegin \DIFadd{kinematical }\DIFaddend distributions. 
...
Electron candidates are required to have $|\eta_{\textrm{cluster}}| < 2.47$, and to satisfy \DIFdelbegin \DIFdel{tight }\DIFdelend \DIFaddbegin \DIFadd{`tight' }\DIFaddend likelihood-based identification 
...
Muon candidates are required to satisfy \DIFdelbegin \DIFdel{medium }\DIFdelend \DIFaddbegin \DIFadd{`medium' }\DIFaddend identification criteria~\cite{Aad:2016jkr}. 
...
To further reduce the background from non-prompt leptons, \DIFaddbegin \DIFadd{which originate mostly from heavy-flavour hadron decays, }\DIFaddend photon conversions and \DIFaddbegin \DIFadd{misidentified }\DIFaddend hadrons,
lepton candidates are also required to be isolated in the tracker and in the calorimeter.
A track- and cluster-based lepton isolation criterion is defined by \DIFdelbegin \DIFdel{calculating the quantity }\DIFdelend \DIFaddbegin \DIFadd{placing requirements on the quantities }\DIFaddend $I_R = \sum \pt^{\textrm{trk}}$ and $E_R = \sum E^{\textrm{clst}}$, where
the scalar sum includes all tracks\DIFdelbegin \DIFdel{and energy deposits }\DIFdelend \DIFaddbegin \DIFadd{' $\pt^{\textrm{trk}}$ or cluster energy deposits $E^{\textrm{clst}}$ }\DIFaddend (excluding the lepton candidate itself)
within the cone defined by \DIFdelbegin \DIFdel{$\Delta R<0.2 (0.3)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta R<0.2~(0.3)$ }\DIFaddend around the %axis
...
$I_R/\pt^\ell < 0.2~(0.3)$ and $E_R/\pt^\ell < 0.2~(0.3)$\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, where }\DIFaddend $\ell=e$ or $\mu$\DIFdelbegin \DIFdel{)}\DIFdelend .

\DIFdelbegin \DIFdel{Tight }\DIFdelend \DIFaddbegin \DIFadd{`Tight' }\DIFaddend isolation working points are also applied in some channels to reduce fake and non-prompt lepton contributions \DIFdelbegin \DIFdel{based on }\DIFdelend \DIFaddbegin \DIFadd{by using }\DIFaddend a trained isolation boosted decision tree \DIFaddbegin \DIFadd{(BDT)
}\DIFaddend \texttt{PromptLeptonVeto} (PLIV), which \DIFdelbegin \DIFdel{is used to identify }\DIFdelend \DIFaddbegin \DIFadd{identifies }\DIFaddend non-prompt light leptons \DIFaddbegin \DIFadd{by }\DIFaddend using lifetime information associated with a \DIFdelbegin \DIFdel{track jet }\DIFdelend \DIFaddbegin \DIFadd{track-jet }\DIFaddend that matches the selected light lepton~\cite{ATLAS-CONF-2019-045}. These additional reconstructed \DIFdelbegin \DIFdel{charged particle }\DIFdelend \DIFaddbegin \DIFadd{charged-particle }\DIFaddend tracks inside the jet can be
used to increase the \DIFdelbegin \DIFdel{accuracy }\DIFdelend \DIFaddbegin \DIFadd{efficiency }\DIFaddend for identifying the displaced decay \DIFdelbegin \DIFdel{vertex of heavy flavour }\DIFdelend \DIFaddbegin \DIFadd{vertices of heavy-flavour }\DIFaddend ($b$, $c$) hadrons that produced non-prompt leptons.
The \DIFdelbegin \DIFdel{tight }\DIFdelend \DIFaddbegin \DIFadd{`tight' }\DIFaddend working points are used for \DIFaddbegin \DIFadd{leptons with }\DIFaddend high $\pt$ \DIFdelbegin \DIFdel{lepton }\DIFdelend ($>20~\GeV$).
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Simulation-to-data }\DIFaddend scale factors for the efficiencies of the \DIFdelbegin \DIFdel{tight }\texttt{\DIFdel{PLIV}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{`tight' PLIV }\DIFaddend working points are measured using the \DIFdelbegin \DIFdel{tag and probe method}\DIFdelend \DIFaddbegin \DIFadd{tag-and-probe method~\mbox{%DIFAUXCMD
\cite{PERF-2016-01}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend with $Z\rightarrow \ell^+\ell^-$ events. These scale factors \DIFdelbegin \DIFdel{have also been checked for the electron or muon from the tau decay
}\DIFdelend \DIFaddbegin \DIFadd{were checked for electrons or muons from the $\tau$-lepton decays
}\DIFaddend using $Z\rightarrow\tau\tau\rightarrow e\mu4\nu$ samples, \DIFdelbegin \DIFdel{which is consistent at }\DIFdelend \DIFaddbegin \DIFadd{and are consistent at the }\DIFaddend 2\% level. To be conservative, an
additional uncertainty of $\pm 2\%$ is considered for the PLIV efficiency for the \DIFdelbegin \DIFdel{tau-lepton }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-lepton }\DIFaddend in the lepton+$\thad$ channels.
...
\DIFdelbegin \DIFdel{The calibratedjets used }\DIFdelend \DIFaddbegin \DIFadd{After being calibrated, the jets }\DIFaddend are required to have $\pt > 25~\gev$ and $|\eta| < 2.5$.
The four-momentum of \DIFdelbegin \DIFdel{the jets }\DIFdelend \DIFaddbegin \DIFadd{each jet }\DIFaddend is corrected for pile-up effects using the jet-area method~\cite{Cacciari:2008gn}.
...
Jets containing $b$-hadrons are identified ($b$-tagged) via the DL1r tagger~\cite{Aad:2019epj79,ATL-PHYS-PUB-2017-013}\DIFaddbegin \DIFadd{, }\DIFaddend %newref
...
Hadronically decaying $\tau$-lepton ($\had$) candidates are reconstructed from \DIFdelbegin \DIFdel{energy clusters in the calorimeters }\DIFdelend \DIFaddbegin \DIFadd{calorimeter energy clusters }\DIFaddend and
...
A \DIFdelbegin \DIFdel{Recurrent Neural Network }\DIFdelend \DIFaddbegin \DIFadd{recurrent neural network }\DIFaddend (RNN)~\cite{Graves:2012SCI}
...
Three working points\DIFdelbegin \DIFdel{labelled loose, medium }\DIFdelend \DIFaddbegin \DIFadd{, labelled `loose', `medium' and `tight', are defined }\DIFaddend and \DIFdelbegin \DIFdel{tight are defined, and }\DIFdelend correspond to different $\had$ identification efficiency values, with the efficiency designed to be independent of $\pt$. The $\Htautau$ search uses the \DIFdelbegin \DIFdel{medium
}\DIFdelend \DIFaddbegin \DIFadd{`medium'
}\DIFaddend working point for the \DIFdelbegin \DIFdel{nominal }\DIFdelend \DIFaddbegin \DIFadd{$\had$ }\DIFaddend selection.
%, while the loose working point is used for background estimation.
The \DIFdelbegin \DIFdel{medium }\DIFdelend \DIFaddbegin \DIFadd{`medium' }\DIFaddend working point has a combined reconstruction and identification efficiency of 75\% (60\%) for one-prong (three-prong) $\had$ 
decays, and an expected rejection factor \DIFdelbegin \DIFdel{against light-jets }\DIFdelend of 35 (240) \DIFaddbegin \DIFadd{against light jets}\DIFaddend ~\cite{ATL-PHYS-PUB-2019-033}. 
...
Only events recorded with a single-electron trigger, a single-muon trigger, or a \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$-lepton }\DIFaddend trigger~\cite{TRIG-2018-05,TRIG-2018-01,id_trigger,l1topo_trigger} under stable beam conditions 
and for which all detector subsystems were operational are considered for analysis. The events recorded by \DIFdelbegin \DIFdel{di-lepton triggers }\DIFdelend \DIFaddbegin \DIFadd{dilepton triggers that }\DIFaddend fall into the control regions are used for \DIFdelbegin \DIFdel{fake tau background
estimation }\DIFdelend \DIFaddbegin \DIFadd{fake-$\tau$-lepton background
estimation as discussed in Section~\ref{sec:background_model}}\DIFaddend . The number of $pp$ interactions per bunch crossing in this dataset ranges from about 8 to 45, with an average of 24.
...
with higher-threshold triggers \DIFdelbegin \DIFdel{but with }\DIFdelend \DIFaddbegin \DIFadd{that have }\DIFaddend a looser identification criterion and \DIFdelbegin \DIFdel{without any }\DIFdelend \DIFaddbegin \DIFadd{no }\DIFaddend isolation requirement.
The lowest $\pt$ threshold used for muons is 20 (26)~\gev\ in 2015 (\DIFdelbegin \DIFdel{2016-2018}\DIFdelend \DIFaddbegin \DIFadd{2016--2018}\DIFaddend ), while for electrons the threshold is 24 (26)~\gev.
...
For \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ }\DIFaddend triggers, the $\pt$ threshold \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the leading (subleading) $\had$ candidate is 35 (25)~\gev.
%Events satisfying the trigger selection are required to have at least one primary vertex candidate.
To reduce the impact of the trigger efficiency uncertainty around the threshold, the leptons are required to have a \pt \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{that is at least }\DIFaddend 1~\DIFdelbegin \DIFdel{GeV }\DIFdelend \DIFaddbegin \GeV\DIFadd{\ }\DIFaddend above the threshold. 
The reconstructed \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend are required to have a \pt \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{at least }\DIFaddend 5\DIFdelbegin \DIFdel{GeV }\DIFdelend \DIFaddbegin \DIFadd{~}\GeV\DIFadd{\ }\DIFaddend higher than the trigger threshold, and $|\eta|<2.5$, excluding the EM calorimeter's transition region.
...
The events in the leptonic \DIFdelbegin \DIFdel{channel }\DIFdelend \DIFaddbegin \DIFadd{channels }\DIFaddend are recorded by a single-electron or single-muon trigger, \DIFaddbegin \DIFadd{and are }\DIFaddend required to have exactly one electron or muon that matches, within $\Delta R < 0.15$, the lepton reconstructed by at least one of the possible triggers. \DIFdelbegin \DIFdel{Further requirements are defined targeting different signal decay modes:  
}\DIFdelend \DIFaddbegin \DIFadd{Requirements are placed on the following additional quantities.
%DIF > Further requirements are defined targeting different signal decay modes:  
}\DIFaddend \begin{itemize}
\item $t_h\lephad$: \DIFdelbegin \DIFdel{Targeting into }\DIFdelend \DIFaddbegin \DIFadd{To enhance selection of }\DIFaddend the $t_hH$ and $t_ht(qH)$ final states with a $H\to\lephad$ decay\DIFdelbegin \DIFdel{. Exactly }\DIFdelend \DIFaddbegin \DIFadd{, exactly }\DIFaddend one $\had$ with \DIFdelbegin \DIFdel{opposite-side }\DIFdelend \DIFaddbegin \DIFadd{opposite-sign }\DIFaddend charge to $\lep$ is required, \DIFdelbegin \DIFdel{including }\DIFdelend \DIFaddbegin \DIFadd{plus }\DIFaddend at least three jets with exactly one $b$-jet.
\item $t_{\ell}\hadhad$: \DIFdelbegin \DIFdel{Targeting }\DIFdelend \DIFaddbegin \DIFadd{To enhance selection of }\DIFaddend $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with a $H\to\hadhad$ decay\DIFdelbegin \DIFdel{. Exactly }\DIFdelend \DIFaddbegin \DIFadd{, exactly }\DIFaddend one light lepton and two opposite-sign $\had$ are \DIFdelbegin \DIFdel{selected,
  including at least one jet }\DIFdelend \DIFaddbegin \DIFadd{required,
  plus jets }\DIFaddend with exactly one $b$-jet.
\item $t_{\ell}\had$: \DIFdelbegin \DIFdel{Also targeting }\DIFdelend \DIFaddbegin \DIFadd{This channel enhances the selection of }\DIFaddend $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with a $H\to\hadhad$ decay where one $\had$ fails the \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{applied }\DIFaddend reconstruction or identification
  \DIFaddbegin \DIFadd{criterion }\DIFaddend so that there is only one reconstructed $\had$ candidate.
  To reduce the SM backgrounds and avoid overlaps with the \DIFaddbegin \DIFadd{final states used in }\DIFaddend other ATLAS searches, exactly one $\had$ with \DIFdelbegin \DIFdel{same-sign charge }\DIFdelend \DIFaddbegin \DIFadd{the same charge as that assigned }\DIFaddend to the light lepton is required.
  In addition, at least two jets including exactly one $b$-jet are required.
\end{itemize}

The events in the hadronic channel are selected by \DIFdelbegin \DIFdel{s di-tau }\DIFdelend \DIFaddbegin \DIFadd{a di-$\tau$ }\DIFaddend trigger. Further requirements are:
...
\begin{itemize}
\item $t_h\hadhad$: \DIFdelbegin \DIFdel{Targeting the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend $t_hH$ and $t_ht(qH)$ final states with $H\to\hadhad$ decay \DIFaddbegin \DIFadd{are targeted}\DIFaddend . Exactly two $\had$ with opposite-sign charge and at least three jets, including exactly one $b$-jet, are required.
...
%DIFDELCMD < The leading and subleading $\had$ candidates are denoted by $\tau_{\mathrm{had1}}$ and $\tau_{\mathrm{had2}}$ respectively.}%%%
...
\toprule\toprule
\multirow{2}{*}{Requirement} &  \DIFdelbeginFL %DIFDELCMD < \multicolumn{3}{c|}{leptonic channel}  %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{3}{c|}{Leptonic channels}  \DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c}{hadronic channel} %%%
...
An overview of the Monte Carlo (MC) generators used for the main signal and background samples is \DIFdelbegin \DIFdel{summarized }\DIFdelend \DIFaddbegin \DIFadd{summarised }\DIFaddend in Table~\ref{mob}.
...
is performed.} {\powheg}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend v2 \cite{Frixione:2007nw,Nason:2004rx,Frixione:2007vw,Alioli:2010xd}
%{\amcatnlolong}~2.4.3~\cite{Alwall:2014hca}  (referred to in the following as {\amcatnlo})
with the \DIFdelbegin \DIFdel{NNPDF3.0 NLO}\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nlo}}\DIFaddend ~\cite{Ball:2014uwa} parton distribution function (PDF) set and interfaced to {\pythia}\DIFaddbegin \DIFadd{\,}\DIFaddend 8.212~\cite{Sjostrand:2007gs} with the \DIFdelbegin \DIFdel{NNPDF2.3 LO}\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF2.3lo}}\DIFaddend ~\cite{Ball:2012cx} PDF set for the modelling of \DIFdelbegin \DIFdel{parton showering }\DIFdelend \DIFaddbegin \DIFadd{the parton showers }\DIFaddend (PS), hadronisation, and \DIFdelbegin \DIFdel{the }\DIFdelend underlying event. 
\DIFdelbegin \DIFdel{The A14~\mbox{%DIFAUXCMD
\cite{ATLASUETune4} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend set of tuned parameters \DIFaddbegin \DIFadd{called the A14 tune~\mbox{%DIFAUXCMD
\cite{ATLASUETune4} }\hspace{0pt}%DIFAUXCMD
was used }\DIFaddend in {\pythia} \DIFdelbegin \DIFdel{controlling the description of multiparton }\DIFdelend \DIFaddbegin \DIFadd{to control the modelling of multi-parton }\DIFaddend interactions and  
initial- and final-state radiation\DIFdelbegin \DIFdel{, referred to as the tune, was used}\DIFdelend .
The signal sample is normalised to the same total cross section as \DIFaddbegin \DIFadd{is }\DIFaddend used for the inclusive $t\bar{t}\to WbWb$ sample (see discussion below)
\DIFdelbegin \DIFdel{and
}\DIFdelend assuming a benchmark branching ratio of $\BR_{\mathrm{ref}}(t\to qH)=0.1\%$.
...
The $tH$ signal events were generated by {\amcatnlolong}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 2.6.2~\cite{Alwall:2014hca}  (referred to in the following as {\amcatnlo})
with the \DIFdelbegin \DIFdel{NNPDF3.0 NLO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nlo}} \DIFaddend parton distribution function (PDF) set. The parton showers, hadronisation, and underlying event \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend modelled by {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.212 with the \DIFdelbegin \DIFdel{NNPDF2.3 LO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF2.3lo}} \DIFaddend PDF set
...
Depending on \DIFdelbegin \DIFdel{either }\DIFdelend \DIFaddbegin \DIFadd{whether the }\DIFaddend up quark or \DIFdelbegin \DIFdel{charm quark }\DIFdelend \DIFaddbegin \DIFadd{the charm quark is }\DIFaddend involved in the FCNC production \DIFaddbegin \DIFadd{process}\DIFaddend , the effective Lagrangian of \DIFaddbegin \DIFadd{the }\DIFaddend $tqH$ interaction \DIFdelbegin \DIFdel{are parametrised using
dim-6 }\DIFdelend \DIFaddbegin \DIFadd{is parameterised using
dimension-6 }\DIFaddend operators~\cite{fcnc_production_theory}. \DIFdelbegin \DIFdel{We obtain $\sigma(ug\to tH)$ = 0.711 pb and $\sigma(cg\to tH)$ = 0.103 pb }\DIFdelend \DIFaddbegin \DIFadd{The cross sections $\sigma(ug\to tH) = 0.711$~pb and $\sigma(cg\to tH) = 0.103$~pb were obtained }\DIFaddend using $\BR_{\mathrm{ref}}(t\to qH)=0.1\%$ as \DIFaddbegin \DIFadd{the }\DIFaddend benchmark.   

The \DIFdelbegin \DIFdel{nominal }\DIFdelend sample used to model the $\ttbar$ background was generated with the NLO generator {\powheg}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend v2
%\cite{Frixione:2007nw,Nason:2004rx,Frixione:2007vw,Alioli:2010xd}
using the \DIFdelbegin \DIFdel{NNPDF3.0 NLO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nlo}} \DIFaddend PDF set. The {\powheg} model parameter $h_{\textrm{damp}}$, which controls 
...
The parton showers, hadronisation, and underlying event were modelled by {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.210 with the \DIFdelbegin \DIFdel{NNPDF2.3 LO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF2.3lo}} \DIFaddend PDF set in combination with the A14 tune.
...
The generated $\ttbar$ samples are normalised to a theoretical cross section of \DIFdelbegin \DIFdel{$832^{+46}_{-51}$}\DIFdelend \DIFaddbegin \DIFadd{$\sigma_{\ttbar}=832^{+46}_{-51}$}\DIFaddend ~pb, 
computed using \textsc{Top++}\DIFdelbegin \DIFdel{~v2.0~}\DIFdelend \DIFaddbegin \DIFadd{\,2.0~}\DIFaddend \cite{Czakon:2011xx} at next-to-next-to-leading order (NNLO), 
...
{\powheg}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend v2 \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Frederix:2012dh} }\hspace{0pt}%DIFAUXCMD
generator}\DIFdelend \DIFaddbegin \DIFadd{generator~\mbox{%DIFAUXCMD
\cite{Frederix:2012dh}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , using the \DIFdelbegin \DIFdel{4F }\DIFdelend \DIFaddbegin \DIFadd{four-flavour }\DIFaddend scheme  for the NLO matrix-element calculations
and the corresponding \DIFdelbegin \DIFdel{NNPDF3.0 NLO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nlo}} \DIFaddend set of PDFs.
Samples corresponding to the $tW$- and $s$-channel production mechanisms were generated 
with {\powheg}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend v2 using the five-flavour scheme. Overlaps between the $\ttbar$ and $tW$ final states were avoided by using 
the diagram removal scheme~\cite{Frixione:2005vw}.
The parton showers, hadronisation and \DIFdelbegin \DIFdel{the }\DIFdelend underlying event were modelled using {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.230~\cite{Sjostrand:2006za} \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the A14 tune and the \DIFdelbegin \DIFdel{NNPDF2.3 LO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF2.3lo}} \DIFaddend PDF set.
...
Samples of $W/Z$+jets events were generated with the {\sherpa}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 2.2.1~\cite{Gleisberg:2008ta} generator. 
...
\DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend merged with the {\sherpa} parton shower~\cite{Schumann:2007mg} using the \DIFdelbegin \DIFdel{ME+PS}\DIFdelend \DIFaddbegin \DIFadd{MEPS}\DIFaddend @NLO prescription~\cite{Hoeche:2012yf}. 
The PDF set used for the matrix-element calculation is \DIFdelbegin \DIFdel{NNPDF3.0 NNLO}\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nnlo}}\DIFaddend ~\cite{Ball:2014uwa} with a dedicated parton shower \DIFdelbegin \DIFdel{tuning }\DIFdelend \DIFaddbegin \DIFadd{tune }\DIFaddend developed for {\sherpa}. 
...
($W/Z$+light-jets), \DIFdelbegin \DIFdel{which are }\DIFdelend \DIFaddbegin \DIFadd{and were }\DIFaddend combined into the inclusive $W/Z$+jets samples.
\DIFdelbegin \DIFdel{Both the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend $W$+jets and $Z$+jets samples are normalised to their respective inclusive NNLO theoretical 
cross sections calculated with \textsc{FEWZ}~\cite{Anastasiou:2003ds}.

Samples of $WW/WZ/ZZ$+jets events were generated with {\sherpa}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 2.2.1 using the CT10 PDF set
...
Samples of $\ttbar V$ \DIFaddbegin \DIFadd{($V=W, Z$ boson) }\DIFaddend and $\ttbar H$ events were generated with {\amcatnlo}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 2.2.1, using NLO matrix elements and the \DIFdelbegin \DIFdel{NNPDF3.0 NLO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF3.0nlo}} \DIFaddend PDF set,
and interfaced to {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.210 with the \DIFdelbegin \DIFdel{NNPDF2.3 LO }\DIFdelend \DIFaddbegin \textsc{\DIFadd{NNPDF2.3lo}} \DIFaddend PDF set and the A14 tune. 
...
Samples of $WH$ and $ZH$, collectively referred to as $VH$\DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{, were }\DIFaddend generated using {\powheg}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend v2\DIFaddbegin \DIFadd{~}\DIFaddend \cite{Frixione:2007nw,Nason:2004rx,Frixione:2007vw,Alioli:2010xd}
and interfaced to {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.210 with the PDF4LHC15 PDF set and the AZNLO tune.
...
The sample \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend generated using {\amcatnlolong}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 2.6.2~\cite{Alwall:2014hca} and interfaced \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.210 with \DIFdelbegin \DIFdel{CT10PDF
and }\DIFdelend \DIFaddbegin \DIFadd{the CT10 PDF set
and the }\DIFaddend A14 tune. The MC predictions are normalised using the NLO cross section recommended in Ref.~\cite{deFlorian:2016spz}.
...
utilise \textsc{EvtGen}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 1.2.0~\cite{Lange:2001uf} to model the decays of heavy-flavour hadrons. 
...
The effects of multiple interactions in the same and nearby bunch crossings (pile-up) were modelled by overlaying minimum-bias events, simulated using the soft QCD processes of {\pythia}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend 8.186~\cite{Sjostrand:2006za} with the A3 \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{ATL-PHYS-PUB-2016-017} }\hspace{0pt}%DIFAUXCMD
set of tuned parameters and NNPDF2.3 LO }\DIFdelend \DIFaddbegin \DIFadd{tune~\mbox{%DIFAUXCMD
\cite{ATL-PHYS-PUB-2016-017} }\hspace{0pt}%DIFAUXCMD
and }\textsc{\DIFadd{NNPDF2.3lo}} \DIFaddend PDF set~\cite{Ball:2012cx}.
...
$W/Z$+jets & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c|}{{\sherpa}~2.2.1} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c|}{{\sherpa}\,2.2.1} \DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c|}{NNPDF30NNLO} %%%
...
$tH$ & {\amcatnlolong} & {\pythia}\DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\,}\DIFaddendFL 8 & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c|}{CT10PDF} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c|}{CT10} \DIFaddendFL & A14 & NLO \\ \hline
Diboson & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c|}{{\sherpa}~2.2.1} %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c|}{{\sherpa}\,2.2.1} \DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c|}{NNPDF30NNLO} %%%
...
The $tt(qH)$ and $tH$ signal being probed \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend characterised by the presence of $\tau$-leptons from the decay of 
...
In a significant fraction of the events, the \DIFdelbegin \DIFdel{lowest }\DIFdelend \DIFaddbegin \DIFadd{lowest-}\DIFaddend $\pt$ jet from the hadronic $W$ boson decay fails the minimum $\pt$ requirement of $25~\gev$,
...
In order to optimise the sensitivity of the search, the selected events are categorised into seven signal regions (SRs) based on the  \DIFdelbegin \DIFdel{number }\DIFdelend \DIFaddbegin \DIFadd{numbers }\DIFaddend of light leptons,
$\had$ candidates, and \DIFdelbegin \DIFdel{on the number of light flavoured }\DIFdelend \DIFaddbegin \DIFadd{light-flavour }\DIFaddend jets:
...
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \centering
\caption{Overview of the signal regions (SR), validation region (VR), and $t\bar{t}$ control regions (CRtt) used for the \DIFdelbeginFL \DIFdelFL{fake tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fake-$\tau$-lepton }\DIFaddendFL scale factor derivation in the leptonic channels. Leptons are required to have either same-sign (SS) or opposite-sign (OS) \DIFaddbeginFL \DIFaddFL{charges }\DIFaddendFL in each region.}
\label{tab:srcr}
\DIFaddbeginFL \begin{center}
\DIFaddendFL \begin{tabular}[h]{c|c|c|c|c|c|c}
\hline \hline
\multicolumn{2}{c|}{Regions} & $b$\DIFdelbeginFL \DIFdelFL{-jet }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-jets }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{light flavour }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Light-flavour }\DIFaddendFL jets        & \DIFdelbeginFL \DIFdelFL{lepton }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Leptons }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{hadronic taus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Hadronic $\tau$ decays }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{charge}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Charge}\DIFaddendFL \\ \hline
\multirow{7}{*}{SR}&$t_{\ell}\thadhad$     & 1     & $\ge0$\DIFaddbeginFL \DIFaddFL{~~~~                                }\DIFaddendFL & 1      & 2             & $\thadhad$ OS\\ \cline{2-7}
&$t_{\ell}\tauhad$-1j  & 1     & 1                                   & 1      & 1                     & $t_{\ell}\tauhad$ SS\\ \cline{2-7}
&$t_{\ell}\tauhad$-2j  & 1     & 2                                        & 1      & 1                     & $t_{\ell}\tauhad$ SS\\ \cline{2-7}
&$t_h\tlhad$-2j   & 1     & 2                           & 1      & 1             & $\tlhad$ OS\\ \cline{2-7}
&$t_h\tlhad$-3j   & 1     & $\ge3$\DIFaddbeginFL \DIFaddFL{~~~~                      }\DIFaddendFL & 1      & 1             & $\tlhad$ OS\\ \cline{2-7}
&$t_h\thadhad$-2j & 1     & 2                            & 0      & 2             & $\thadhad$ OS\\ \cline{2-7}
...
This event categorisation is primarily \DIFdelbegin \DIFdel{motivated by optimizing }\DIFdelend \DIFaddbegin \DIFadd{used to optimise }\DIFaddend the sensitivity in each signal region that targets either leptonic or hadronic top-quark
decays as well as the Higgs \DIFaddbegin \DIFadd{boson }\DIFaddend decays into either \DIFaddbegin \DIFadd{the }\DIFaddend $\lephad$ or $\hadhad$ final \DIFdelbegin \DIFdel{states}\DIFdelend \DIFaddbegin \DIFadd{state}\DIFaddend .   
...
The contribution of background to the signal regions of the $t_h\hadhad$ and $t_h\lephad$ channels is reduced by \DIFdelbegin \DIFdel{having a }\DIFdelend \DIFaddbegin \DIFadd{placing }\DIFaddend kinematic constraints on the \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ }\DIFaddend mass and the $\met$ in the event~\cite{Chen:2015nta}.

%based on the strategy used in Ref.~\cite{Chen:2015nta}, and is summarised below.

For the $t_ht(qH)$ events, the jet from $t\to qH$, \DIFdelbegin \DIFdel{denoted }\DIFdelend \DIFaddbegin \DIFadd{referred to }\DIFaddend as the FCNC jet ($q$-jet), should be a \DIFdelbegin \DIFdel{high }\DIFdelend \DIFaddbegin \DIFadd{high-}\DIFaddend $\pt$ jet from the
decay chain $t\to qH\to q\tau\tau$, with \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend reconstructed as $\lephad$ or $\hadhad$.
...
Events should contain \DIFdelbegin \DIFdel{4 }\DIFdelend \DIFaddbegin \DIFadd{four }\DIFaddend jets, with the one having the smallest angular separation \DIFdelbegin \DIFdel{with respect to the visible di-tau }\DIFdelend \DIFaddbegin \DIFadd{from the visible di-$\tau$ }\DIFaddend system being labelled \DIFdelbegin \DIFdel{as  }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend $q$-jet since the FCNC \DIFdelbegin \DIFdel{top }\DIFdelend \DIFaddbegin \DIFadd{top-quark }\DIFaddend decay products are likely \DIFaddbegin \DIFadd{to be }\DIFaddend boosted closer together. 
If there are more than two jets besides the $q$\DIFdelbegin \DIFdel{- }\DIFdelend \DIFaddbegin \DIFadd{-jet }\DIFaddend and $b$-jet, the jets from \DIFaddbegin \DIFadd{the }\DIFaddend $W$ boson decay are chosen \DIFaddbegin \DIFadd{to be those }\DIFaddend from the combination
with \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend invariant mass closest to the $W$ boson mass. \DIFdelbegin \DIFdel{There is the possibility }\DIFdelend \DIFaddbegin \DIFadd{It is possible }\DIFaddend that one of the jets fails the $\pt$ requirement and is not reconstructed.
\DIFdelbegin \DIFdel{These kinds of events will fall into }\DIFdelend \DIFaddbegin \DIFadd{Events of this kind produce }\DIFaddend the $t_hH$ final state.
...
\DIFdelbegin \DIFdel{There are three jets coming from top hadronic decay including the }\DIFdelend \DIFaddbegin \DIFadd{When both the $t_h$ and $H$ can be reconstructed, three jets come from the top quark's hadronic decay, including a }\DIFaddend $b$-jet\DIFaddbegin \DIFadd{, }\DIFaddend and a pair of opposite-sign $\had$ \DIFaddbegin \DIFadd{come }\DIFaddend from the Higgs boson decay\DIFdelbegin \DIFdel{, where
both $t_h$ and $H$ can be reconstructed}\DIFdelend .  
...
are estimated using a kinematic fit and assuming a collinear approximation for the \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend decay products.
The fit is done by minimising a $\chi^2$ function based on the Gaussian constraints \DIFaddbegin \DIFadd{placed }\DIFaddend on the Higgs boson mass (\DIFdelbegin \DIFdel{$m_{\text{H}}=125~\gev$}\DIFdelend \DIFaddbegin \DIFadd{$m_H=125~\gev$}\DIFaddend ) and the
measured \DIFdelbegin \DIFdel{$E_{\text{x,y}}^{\text{miss}}$  }\DIFdelend \DIFaddbegin \DIFadd{$E_{x,y}^{\text{miss}}$  }\DIFaddend within their expected resolutions (\DIFdelbegin \DIFdel{$\sigma_{E_{\text{x,y}}^{\text{miss}}}$}\DIFdelend \DIFaddbegin \DIFadd{$\sigma_{E_{x,y}^{\text{miss}}}$}\DIFaddend ), defined as
\begin{eqnarray}
\DIFdelbegin %DIFDELCMD < \begin{array}{ll}
%DIFDELCMD < \chi^2 =
%DIFDELCMD < \left( \frac{m_{\tau\tau,\text{fit}} - m_{\text{H}}}{\sigma_{\tau\tau}} \right)^2 +
%DIFDELCMD < \left( \frac{E_{\text{x,fit}}^{\text{miss}} - E_{\text{x}}^{\text{miss}}}{\sigma_{E_{\text{x}}^{\text{miss}}}} \right)^2
%DIFDELCMD <   +\left( \frac{E_{\text{y,fit}}^{\text{miss}} - E_{\text{y}}^{\text{miss}}}{\sigma_{E_{\text{y}}^{\text{miss}}}} \right)^2.
%DIFDELCMD < \end{array}%%%
...
The \DIFdelbegin \DIFdel{resolution on the }\DIFdelend Higgs boson mass \DIFaddbegin \DIFadd{resolution }\DIFaddend ($\sigma_{\tau\tau}$) is estimated to be $20~\gev$ from a fit of the mass
distribution of the simulated  $tqH$ signal events, while the \DIFdelbegin \DIFdel{resolution on the measured }\DIFdelend $\met$ \DIFaddbegin \DIFadd{measurement's resolution }\DIFaddend is parameterised as a linear function of 
$\sqrt{\sum E_{\text{T}}}$, where $\sum E_{\text{T}}$ is the scalar sum of the \DIFdelbegin \DIFdel{$\pt$ }\DIFdelend \DIFaddbegin \DIFadd{$E_{\text{T}}$ values }\DIFaddend of all physics objects contributing to the $\met$ reconstruction~\cite{Aaboud:2018tkc}.
After the $\chi^2$ minimisation, \DIFaddbegin \DIFadd{both }\DIFaddend the Higgs boson $\pt$ \DIFdelbegin \DIFdel{as well as }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the 
$\pt$ \DIFaddbegin \DIFadd{values }\DIFaddend of the parent top quarks are determined with better resolution in the signal events. 

For the  $t_{\ell}t(qH)$ and $t_{\ell}H$ events where \DIFaddbegin \DIFadd{the }\DIFaddend $W$ boson from $t\to W b$ decay decays leptonically,
%has a large momentum and the flying direction is unknown.
the kinematic fit is no longer feasible due to \DIFdelbegin \DIFdel{extra neutrino from }\DIFdelend \DIFaddbegin \DIFadd{the neutrino from the }\DIFaddend $W\rightarrow \ell\nu$ decay. The kinematic variables are calculated using the visible
objects only. After the event reconstruction, a number of \DIFaddbegin \DIFadd{these }\DIFaddend variables are used \DIFaddbegin \DIFadd{in a multivariate analysis }\DIFaddend to discriminate the signal from the background\DIFdelbegin \DIFdel{using a multivariate analysis }\DIFdelend \DIFaddbegin \DIFadd{, as }\DIFaddend described in Section~\ref{sec:tmva}.
...
and the Higgs boson is forced to decay \DIFdelbegin \DIFdel{into }\DIFdelend \DIFaddbegin \DIFadd{via }\DIFaddend $H\to \tau\tau$ with \DIFaddbegin \DIFadd{a }\DIFaddend branching ratio calculated using \textsc{Hdecay}~\cite{Djouadi:1997yw}.  
Backgrounds with non-prompt light leptons (electron or muon), with photons or jets misidentified as electrons, or with jets misidentified as \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-lepton }\DIFaddend candidates, 
generically referred to as fake leptons, are estimated using data-driven methods. 
%\texttt{fake} 
The background prediction is \DIFdelbegin \DIFdel{further }\DIFdelend improved during the statistical analysis by performing a likelihood 
...
%-------------------------------------------------------------------------------
\subsection{Backgrounds with fake \DIFdelbegin \DIFdel{tau leptons}\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons}\DIFaddend }
\label{sec:faketaus}
The background with one or more fake \DIFdelbegin \DIFdel{tau candidates mainly arises }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ candidates arises mainly }\DIFaddend from $\ttbar$ or
multi-jet production, depending on the search \DIFdelbegin \DIFdel{channels}\DIFdelend \DIFaddbegin \DIFadd{channel}\DIFaddend .
Studies based on simulation show that, for all the above processes, fake \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend candidates primarily result from the
misidentification of light jets and $b$-quark jets.
It is also found that the fake rate decreases for all jet flavours as the \DIFdelbegin \DIFdel{tau candidate}\DIFdelend \DIFaddbegin \DIFadd{$\tau$ candidate's }\DIFaddend $\pt$ increases.

In the leptonic channels, the events with \DIFaddbegin \DIFadd{a }\DIFaddend prompt electron or muon and fake \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend are modelled by calibrating \DIFdelbegin \DIFdel{MC }\DIFdelend \DIFaddbegin \DIFadd{the MC samples }\DIFaddend with scale factors (SF)
derived from the dedicated $t\bar t$
control regions (CRtt) using dileptonic decays of $t\bar t$ \DIFdelbegin \DIFdel{events }\DIFdelend \DIFaddbegin \DIFadd{pairs }\DIFaddend and semileptonic decays of $t\bar t$ \DIFdelbegin \DIFdel{events }\DIFdelend \DIFaddbegin \DIFadd{pairs }\DIFaddend with two
$b$-jets, \DIFdelbegin \DIFdel{summarized }\DIFdelend \DIFaddbegin \DIFadd{as is summarised }\DIFaddend in Table~\ref{tab:srcr}. \DIFaddbegin \DIFadd{The control regions are defined similarly to the signal regions but with an additional $b$-jet or light lepton. 	
}\DIFaddend %the SM $t\bar t$ decay of dilepton events and semileptonically double-btagged lepton-jet events, summarized in Table~\ref{tab:srcr},
%aimed at fake taus from different origins.
There are four kinds of fake \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend that need to be calibrated: Type-1 fake \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend from hadronic $W$ boson decay ($\tau_{W}$) %$W$-jets ($\tau_{W}$)
with opposite-sign (OS) \DIFdelbegin \DIFdel{of charge }\DIFdelend \DIFaddbegin \DIFadd{charge relative }\DIFaddend to the light lepton;
Type-2 $\tau_{W}$'s with same-sign (SS) charge \DIFaddbegin \DIFadd{relative }\DIFaddend to the light lepton; Type-3 fake \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend originating from $b$-hadron decays; \DIFaddbegin \DIFadd{and }\DIFaddend Type-4 fake \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend from light-flavour hadron decays.
\DIFdelbegin \DIFdel{The control regions are defined similar to the signal regions but with an additional $b$-jet or light lepton.
}\DIFdelend %DIF > The control regions are defined similar to the signal regions but with an additional $b$-jet or light lepton.
...
The dilepton regions ($t_{\ell}t_{\ell}1b\thad$ and $t_{\ell}t_{\ell}2b\thad$) are used to calibrate Type-3 and Type-4 fake \DIFdelbegin \DIFdel{taus}\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons}\DIFaddend . The semileptonic
regions ($t_{\ell}t_h2b\thad$-2jOS and $t_{\ell}t_h2b\thad$-3jOS)\DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{, where the }\DIFaddend $\thad$ and light lepton have opposite \DIFdelbegin \DIFdel{charge }\DIFdelend \DIFaddbegin \DIFadd{charges, }\DIFaddend are used to calibrate Type-1 fake \DIFdelbegin \DIFdel{taus}\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons}\DIFaddend .
Similarly for Type-2, the semileptonic regions ($t_{\ell}t_h2b\thad$-2jSS and $t_{\ell}t_h2b\thad$-3jSS)\DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{, where the }\DIFaddend $\thad$ and light lepton have same \DIFdelbegin \DIFdel{charge }\DIFdelend \DIFaddbegin \DIFadd{charges, }\DIFaddend are used.
A simultaneous fit to data is made to derive the scale factors for fake \DIFdelbegin \DIFdel{taus in MC, which }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons in the MC samples. These }\DIFaddend consist of a total of 24 parameters
depending on four types of fake \DIFdelbegin \DIFdel{taus}\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons}\DIFaddend , three $\pt$ bins, \DIFaddbegin \DIFadd{and }\DIFaddend two bins for 1- and 3-prong \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ decays }\DIFaddend separately. The post-fit distributions of the
leading $\thad$ $\pt$ and measured \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend scale factors are presented in Appendix~\ref{sec:tauFF_appendix}. 
The values of these scale factors range from 0.3 \DIFdelbegin \DIFdel{-1.28 are then }\DIFdelend \DIFaddbegin \DIFadd{to 1.28 and are }\DIFaddend used to correct the \DIFdelbegin \DIFdel{MC estimated }\DIFdelend \DIFaddbegin \DIFadd{MC-estimated }\DIFaddend fakes in the corresponding  signal regions with a single $b$-jet.
In the $t_{\ell}\thadhad$ channel, both \DIFdelbegin \DIFdel{taus }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ candidates }\DIFaddend can be misidentified, so the calibration is applied to each \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ candidate }\DIFaddend separately, following the same procedure used in the $\tlhad$ channel.
%using the lepton and fake tau charges, then the scale factors are multiplied together.
The \DIFdelbegin \DIFdel{nominal value of scale factors will }\DIFdelend \DIFaddbegin \DIFadd{central values of the scale factors }\DIFaddend vary according to their uncertainties in the final fit. A closure test is made for the \DIFdelbegin \DIFdel{fake tau estimations }\DIFdelend \DIFaddbegin \DIFadd{fake-$\tau$ estimates, }\DIFaddend using the same procedure \DIFaddbegin \DIFadd{as
}\DIFaddend in the $t_{\ell}\thadhad$-SS validation region, \DIFdelbegin \DIFdel{which shows a }\DIFdelend \DIFaddbegin \DIFadd{and it shows }\DIFaddend good agreement between data and the background prediction. 
...
In the hadronic channels, the contribution of fakes is estimated \DIFdelbegin \DIFdel{partially from data by defining }\DIFdelend \DIFaddbegin \DIFadd{from data using the fake-factor (FF) method to derive the transfer functions from the background-enriched }\DIFaddend control
regions (CR) \DIFdelbegin \DIFdel{enriched in misidentified
tau candidates via loosened tau requirements with fake factors and partially from MC}\DIFdelend \DIFaddbegin \DIFadd{to the SR}\DIFaddend ~\cite{ATLAS-CONF-2021-044}.
%DIF > by defining control regions (CR)
%DIF > enriched in misidentified
%DIF > tau candidates via loosened tau requirements with fake factors and partially from MC~\cite{ATLAS-CONF-2021-044}.
These CRs do not overlap with the main signal regions \DIFdelbegin \DIFdel{, }\DIFdelend discussed in Section~\ref{sec:strategy_Htautau}.
The CR selection requirements are analogous to those used to define signal regions, except
that the subleading \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend candidate is required to fail the \DIFdelbegin \DIFdel{medium tau identification , but it still passes }\DIFdelend \DIFaddbegin \DIFadd{`medium' $\tau$ identification while still passing }\DIFaddend a loose requirement.
The contribution of fakes with subleading \DIFdelbegin \DIFdel{tau candidate }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ candidates }\DIFaddend can be calculated by rescaling the templates \DIFdelbegin \DIFdel{of loosetaus }\DIFdelend \DIFaddbegin \DIFadd{for `loose' $\tau$ candidates }\DIFaddend in the CR
with \DIFdelbegin \DIFdel{fake factors (FF)}\DIFdelend \DIFaddbegin \DIFadd{the FFs}\DIFaddend .
The templates are produced by subtracting all MC background contributions with real subleading \DIFdelbegin \DIFdel{taus from }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons from the }\DIFaddend data.
The FFs are computed as the ratio of misidentified $\thad$ candidates that either pass or fail the \DIFdelbegin \DIFdel{medium tau }\DIFdelend \DIFaddbegin \DIFadd{`medium' $\tau$ }\DIFaddend ID selection in regions enriched in events originating from 
...
%We have compared
FFs obtained from the $W$+jets \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{events are compared with those }\DIFaddend from the same-sign $\thadhad$ control regions \DIFdelbegin \DIFdel{are compared }\DIFdelend and the differences are treated as \DIFdelbegin \DIFdel{a systematic uncertainty}\DIFdelend \DIFaddbegin \DIFadd{systematic uncertainties}\DIFaddend .

\subsection{Background with fake light leptons}
The background originating from non-prompt or misidentified light leptons is primarily from \DIFdelbegin \DIFdel{the }\DIFdelend multi-jet production.
...
The estimate is based on the \DIFdelbegin \DIFdel{number of tight and loose light-leptons }\DIFdelend \DIFaddbegin \DIFadd{numbers of `tight' and `loose' light leptons }\DIFaddend that passed and failed the PLIV cut in the low ($<25~\gev$) and high
($>25~\gev$) $\met$ regions. The number of
fake light leptons in the signal region is evaluated by scaling the number of \DIFdelbegin \DIFdel{loose }\DIFdelend \DIFaddbegin \DIFadd{`loose' }\DIFaddend leptons in the high $\met$ region by the ratio of \DIFdelbegin \DIFdel{tight and loose}\DIFdelend \DIFaddbegin \DIFadd{`tight' leptons to `loose' }\DIFaddend leptons in the
low $\met$ region. The contributions of prompt leptons and calibrated fake \DIFdelbegin \DIFdel{taus in the number of tight and loose }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons to the numbers of `tight' and `loose' }\DIFaddend leptons are subtracted using MC simulation.
A closure test is made for the background \DIFdelbegin \DIFdel{estimations in the signal depleted low BDT }\DIFdelend \DIFaddbegin \DIFadd{estimates in the signal-depleted low-BDT-score }\DIFaddend ($<-0.6$) regions, defined in Section~\ref{sec:tmva}.
...
%%%%%%%%%%%%%%%
\begin{table*}[t!]
  \caption{\DIFdelbeginFL %DIFDELCMD < \small{Discriminating variables (n) used in the training of the BDT of each SR. 
%DIFDELCMD < The ranking of the input variables according to their importance in the training is reported from highest (1) to 
%DIFDELCMD < lowest (n). Variables whose ranking is missing are not included in the training of the corresponding SR. The description of each variable is provided in the text.}%%%
...
%draft 1 version
In the training \DIFaddbegin \DIFadd{process}\DIFaddend , all signal events from $tt(qH)$ and $tH$ are \DIFdelbegin \DIFdel{merged together }\DIFdelend \DIFaddbegin \DIFadd{combined }\DIFaddend for $tuH$ and $tcH$. All background sources from SM processes
(including both \DIFaddbegin \DIFadd{the }\DIFaddend real and fake \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend contributions) are also used in the training.

\DIFdelbegin \DIFdel{A large set of potential variables are }\DIFdelend \DIFaddbegin \DIFadd{Many potential variables were }\DIFaddend investigated in each SR separately. The discrimination of a given variable is quantified by the \DIFdelbegin \DIFdel{"separation" (}\DIFdelend \DIFaddbegin \DIFadd{`separation' (which }\DIFaddend measures the degree of overlap between background and signal \DIFaddbegin \DIFadd{distribution }\DIFaddend shape) and \DIFdelbegin \DIFdel{"importance" (depicts }\DIFdelend \DIFaddbegin \DIFadd{`importance' (which ranks }\DIFaddend the power of the variable \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the classification of the events) provided by the TMVA package\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Hocker:2007ht}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > ~\cite{Hocker:2007ht}.
}\DIFaddend The BDT discriminant is trained for each SR starting from a large list of variables\DIFdelbegin \DIFdel{. Then }\DIFdelend \DIFaddbegin \DIFadd{; then }\DIFaddend the least important \DIFdelbegin \DIFdel{variable is removed one at a time }\DIFdelend \DIFaddbegin \DIFadd{variables are removed sequentially, }\DIFaddend and the BDT is retrained until the
\DIFdelbegin \DIFdel{receive }\DIFdelend \DIFaddbegin \DIFadd{receiver }\DIFaddend operating characteristic (ROC) score drops \DIFaddbegin \DIFadd{by }\DIFaddend more than 2\%.
...
The final BDT input variables in each SR and their importance ranking are listed in Table~\ref{tab:importance}. The discriminating variables \DIFdelbegin \DIFdel{used are }\DIFdelend \DIFaddbegin \DIFadd{are as follows}\DIFaddend :
\begin{itemize}
\item $\met$ is the missing transverse momentum.
\item ${\pt}_{\tau1} $ is the transverse momentum of the leading \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-lepton }\DIFaddend candidate.
\item \DIFdelbegin \DIFdel{${\pt}_{\tau2}$ is the transverse momentum of the subleading tau candidate.
}\DIFdelend \DIFaddbegin \DIFadd{\text{max}($\eta_{\tau}$) is the maximum $\eta$ of the $\tauhad$ candidate(s). 
%DIF > \item ${\pt}_{\tau2}$ is the transverse momentum of the subleading $\tau$-lepton candidate.
}\DIFaddend \item ${\pt}^{\ell}$ is the transverse momentum of the leading light lepton.
\item $\chi^2$ of the kinematic fit of the momentum of the invisible decay products of the \DIFdelbegin \DIFdel{tau leptons }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-leptons }\DIFaddend as defined in \DIFdelbegin \DIFdel{eq}\DIFdelend \DIFaddbegin \DIFadd{Eq}\DIFaddend .~(\ref{eq:eq1}).
\item \DIFdelbegin \DIFdel{$m_{\text{bjj},\text{fit}}$ is the fitted }\DIFdelend \DIFaddbegin \DIFadd{$m_{\text{bjj}}$ is the }\DIFaddend invariant mass of the $b$-jet and the two jets from the $W$ boson decay, and reflects the \DIFdelbegin \DIFdel{top }\DIFdelend \DIFaddbegin \DIFadd{top-quark }\DIFaddend mass in the decay $t\to Wb \to j_1j_2b$. This variable is only defined for the 4-jet $t_hH$ and $t_ht(qH)$ events.
\item $m^{W}_{\text{T}}$ is the transverse mass calculated from the lepton and $\met$ in the leptonic channels, defined as
\DIFdelbegin \begin{displaymath}
\DIFdel{m^{W}_{\text{T}} = \sqrt{2 {\pt}_{\ell} E_{\text{T}}^{\text{miss}} \left(1-\cos\Delta\phi_{\ell,\text{miss}} \right)},  
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
\DIFadd{m^{W}_{\text{T}} = \sqrt{2 {\pt}_{\ell} E_{\text{T}}^{\text{miss}} \left(1-\cos\Delta\phi_{\ell,\text{miss}} \right)},  
}\end{equation*}\DIFaddend 
where $\Delta\phi_{\ell,\text{miss}}$ is the \DIFdelbegin \DIFdel{azimuth }\DIFdelend \DIFaddbegin \DIFadd{azimuthal }\DIFaddend angle between the \DIFdelbegin \DIFdel{light-lepton }\DIFdelend \DIFaddbegin \DIFadd{light lepton }\DIFaddend and $\met$.  
\item $m_{\tau\tau,\text{fit}}$ is the fitted invariant mass of the \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend candidates and reconstructed neutrinos for the $t_hH$ and $t_ht(qH)$ events. 
\item $m_{\text{jj}}$ is the reconstructed invariant mass of two \DIFdelbegin \DIFdel{light-jets }\DIFdelend \DIFaddbegin \DIFadd{light jets }\DIFaddend from the $W$ decay with \DIFdelbegin \DIFdel{the mass closet to the }\DIFdelend \DIFaddbegin \DIFadd{a mass closest to the known }\DIFaddend $W$ \DIFaddbegin \DIFadd{boson }\DIFaddend mass.
\item $m_{\tau\tau\text{q},\text{fit}}$ is the fitted invariant mass of the FCNC-decaying top quark reconstructed from \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{the di-$\tau$ }\DIFaddend candidates, $q$-jet and reconstructed neutrinos.
\item $m_{\tau\tau}$ is the visible invariant mass of the \DIFdelbegin \DIFdel{di-tau system . %DIF < or the lepton and $\had$ candidate when there is only one $\had$.
}\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ system (including the light lepton when there is only one $\had$ candidate).
}\DIFaddend \item ${\pt}_{\tau\tau}$ is the visible $\pT$ of the \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ }\DIFaddend system.
\item $m_{\tau\tau,\text{q}}$ is the reconstructed visible mass of the FCNC-decaying top quark.
\item \DIFdelbegin \DIFdel{$m_{\text{bjj}}$ }\DIFdelend \DIFaddbegin \DIFadd{$m_{\ell \text{b}}$ }\DIFaddend is the invariant mass of the lepton and the $b$-jet, which reflects the visible \DIFdelbegin \DIFdel{top quark }\DIFdelend \DIFaddbegin \DIFadd{top-quark }\DIFaddend mass.
\item \text{min}($m_{\tau\tau \text{j}}$) is the minimum value of the visible mass of the \DIFdelbegin \DIFdel{di-tau candidates (include leptonic tau}\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ candidates (including leptonically decaying $\tau$ candidates}\DIFaddend ) and the light-flavour jet\DIFdelbegin \DIFdel{system,
  minimized by choosing different jet, }\DIFdelend \DIFaddbegin \DIFadd{,
  }\DIFaddend reflecting the invariant \DIFdelbegin \DIFdel{masss }\DIFdelend \DIFaddbegin \DIFadd{mass }\DIFaddend of the visible FCNC \DIFdelbegin \DIFdel{top decaying }\DIFdelend \DIFaddbegin \DIFadd{top-quark decay }\DIFaddend products.
\item \text{min}($m_{\text{jj}}$) is the minimum value of \DIFdelbegin \DIFdel{all }\DIFdelend the invariant mass of all the light-flavour jet pairs, reflecting the invariant mass of the $W$ boson candidate.
  %an alternative of $m_{\text{W}}$.
\item $\met$ centrality is a measure of how \DIFdelbegin \DIFdel{central }\DIFdelend \DIFaddbegin \DIFadd{centrally }\DIFaddend the $\met$ lies between the two \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend candidates in the transverse plane, and is defined as
\DIFdelbegin \begin{eqnarray*}
\DIFdel{\begin{array}{l}
...
\end{array}
%DIFDELCMD < \label{eq:eq3}%%%
}\end{eqnarray*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{eqnarray*}
\DIFadd{\begin{array}{l}
\met~\text{centrality} = {(x+y)}/{\sqrt{x^2+y^2}}, \\
\text{with}~x=\frac{\sin(\phi_{\text{miss}}-\phi_{\tau_1})}{\sin(\phi_{\tau_2}-\phi_{\tau_1})}, \quad  y=\frac{\sin(\phi_{\tau_2}-\phi_{\text{miss}})}{\sin(\phi_{\tau_2}-\phi_{\tau_1})} ,
\end{array}
\label{eq:eq3}
}\end{eqnarray*}\DIFaddend 
\item \DIFdelbegin \DIFdel{$E_{\tau\text{i}}/E_{\tau\text{i},\text{fit}}$ (\text{i}=1,2) }\DIFdelend \DIFaddbegin \DIFadd{$E_{\tau {i}}/E_{\tau {i},\text{fit}}$ $({i}=1,2)$ }\DIFaddend is the momentum fraction carried by the visible decay products from the leading and subleading \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$ }\DIFaddend decays. It is based on the \DIFdelbegin \DIFdel{best-fit of the 4-momentum }\DIFdelend \DIFaddbegin \DIFadd{best fit of the four-momentum }\DIFaddend of the neutrino(s) according to the event reconstruction algorithm in this section. For the $\tauhad$ decay mode, the visible decay products carry most of the \DIFdelbegin \DIFdel{tau }\DIFdelend \DIFaddbegin \DIFadd{$\tau$-lepton's }\DIFaddend energy since there is only a single neutrino in the final state.% which is evident in the excess around 1 in Figure \ref{fig:x12_fit}. 
...
\item $\Delta\phi(\tau\tau,\met)$ is the azimuthal angle between the $\met$ and \DIFdelbegin \DIFdel{di-tau system}\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ system's }\DIFaddend $\pT$.
\item $\Delta R(a,b)$ is the angular distance between \DIFdelbegin \DIFdel{\text{a} and \text{b} }\DIFdelend \DIFaddbegin \DIFadd{the $a$ and $b$ }\DIFaddend objects in the event. 
...
\DIFdelbegin \DIFdel{A comparison between }\DIFdelend \DIFaddbegin \DIFadd{Comparisons between the }\DIFaddend data and the predicted background for the \DIFaddbegin \DIFadd{distributions of }\DIFaddend leading $\had$ $\pt$  and \DIFdelbegin \DIFdel{the }\DIFdelend visible invariant mass of the \DIFdelbegin \DIFdel{di-tau }\DIFdelend \DIFaddbegin \DIFadd{di-$\tau$ }\DIFaddend system ($m_{\tau\tau}$)
\DIFdelbegin \DIFdel{distributions }\DIFdelend in SRs and VRs, after applying fake factors, 
%some of these variables in three main decay final states($t_{\ell}\hadhad$, $t_h\hadhad$, and $t_h\lephad$)
are shown in \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:taupt_prefit} and~\ref{fig:mtt}, respectively.
...
\DIFdelbegin \DIFdel{A good description of the data }\DIFdelend \DIFaddbegin \DIFadd{The data are well described }\DIFaddend by the background model \DIFdelbegin \DIFdel{is observed }\DIFdelend in all cases.
The final observable used to extract the signal contribution is the BDT distribution in each SR corresponding to either \DIFaddbegin \DIFadd{the }\DIFaddend $tuH$ or $tcH$ signal.
...
\end{tabular}
\caption{Leading $\thad$ $\pt$  distributions obtained before the fit to data (\DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Pre-Fit\DIFdelbeginFL \DIFdelFL{''}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}\DIFaddendFL ) showing 
  the expected background and $tuH$ signals after applying fake factors in the following regions: \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$ }\DIFdelendFL (a) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$}\DIFaddendFL ,
 \DIFdelbeginFL \DIFdelFL{$t_{\ell}\tauhad$-1j }\DIFdelendFL (b) \DIFdelbeginFL \DIFdelFL{,  }\DIFdelendFL $t_{\ell}\tauhad$\DIFdelbeginFL \DIFdelFL{-2j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1j,  }\DIFaddendFL (c) \DIFdelbeginFL \DIFdelFL{, $t_h\tlhad$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\ell}\tauhad$}\DIFaddendFL -2j\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (d) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\tlhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (e) \DIFaddbeginFL \DIFaddFL{$t_h\tlhad$-3j}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$-SS  }\DIFdelendFL (f) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$-SS}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_h\thadhad$-2j }\DIFdelendFL (g) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\thadhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (h) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_h\thadhad$-3j \DIFdelbeginFL \DIFdelFL{SS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (i) \DIFaddbeginFL \DIFaddFL{$t_h\thadhad$-3j SS}\DIFaddendFL .
 The total \DIFdelbeginFL \DIFdelFL{size of the }\DIFdelendFL statistical and systematic \DIFdelbeginFL \DIFdelFL{uncertainties }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty }\DIFaddendFL of the background prediction is indicated by the hatched band.
 %The uncertainty band includes both the statistical and systematic uncertainties in the background prediction.
 Overflow events are included in the last bin. \DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Other MC\DIFdelbeginFL \DIFdelFL{'' }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{' }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Rare\DIFdelbeginFL \DIFdelFL{''}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}\DIFaddendFL ) includes \DIFdelbeginFL \DIFdelFL{single top}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{single-top}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $V$+jets\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL and other small backgrounds in the leptonic (hadronic) channel. The $tuH$ signal is scaled \DIFdelbeginFL \DIFdelFL{using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by }\DIFaddendFL a
 \DIFdelbeginFL \DIFdelFL{normalization }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{normalisation }\DIFaddendFL factor of \DIFaddbeginFL \DIFaddFL{either }\DIFaddendFL 2\DIFdelbeginFL \DIFdelFL{to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, 10, or }\DIFaddendFL 50.   
%( only subtau real refers to $t\bar{t}$ MC with leading tau faked by other object, subleading tau from real contribution. 
The lower panels show the \DIFaddbeginFL \DIFaddFL{ratio of }\DIFaddendFL data to prediction\DIFdelbeginFL \DIFdelFL{ratio}\DIFdelendFL .}
...
%(c1) \pT(\tauhad) in $t_h\thadhad$-3j\\
\end{tabular}
\caption{\DIFdelbeginFL \DIFdelFL{Visible }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Distributions of the visible }\DIFaddendFL invariant mass of the \DIFdelbeginFL \DIFdelFL{di-tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{di-$\tau$ }\DIFaddendFL system ($m_{\tau\tau}$) \DIFdelbeginFL \DIFdelFL{distributions }\DIFdelendFL obtained before the fit to data (\DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Pre-Fit\DIFdelbeginFL \DIFdelFL{''}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}\DIFaddendFL ) showing 
  the expected background and $tuH$ signals after applying fake factors in the following regions: \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$ }\DIFdelendFL (a) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$}\DIFaddendFL ,
 \DIFdelbeginFL \DIFdelFL{$t_{\ell}\tauhad$-1j }\DIFdelendFL (b) \DIFdelbeginFL \DIFdelFL{,  }\DIFdelendFL $t_{\ell}\tauhad$\DIFdelbeginFL \DIFdelFL{-2j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1j,  }\DIFaddendFL (c) \DIFdelbeginFL \DIFdelFL{, $t_h\tlhad$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\ell}\tauhad$}\DIFaddendFL -2j\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (d) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\tlhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (e) \DIFaddbeginFL \DIFaddFL{$t_h\tlhad$-3j}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$-SS }\DIFdelendFL (f) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$-SS}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_h\thadhad$-2j }\DIFdelendFL (g) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\thadhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (h) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_h\thadhad$-3j \DIFdelbeginFL \DIFdelFL{SS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (i) \DIFaddbeginFL \DIFaddFL{$t_h\thadhad$-3j SS}\DIFaddendFL .
 The total \DIFdelbeginFL \DIFdelFL{size of the }\DIFdelendFL statistical and systematic \DIFdelbeginFL \DIFdelFL{uncertainties }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty }\DIFaddendFL of the background prediction is indicated by the hatched band.
 %The uncertainty band includes both the statistical and systematic uncertainties in the background prediction.
 Overflow events are included in the last bin. \DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Other MC\DIFdelbeginFL \DIFdelFL{'' }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{' }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Rare\DIFdelbeginFL \DIFdelFL{''}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}\DIFaddendFL ) includes \DIFdelbeginFL \DIFdelFL{single top}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{single-top}\DIFaddendFL , and $V$+jets and other small backgrounds in the leptonic (hadronic) channel. The $tuH$ signal is scaled \DIFdelbeginFL \DIFdelFL{using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{normalization }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{normalisation }\DIFaddendFL factor of \DIFaddbeginFL \DIFaddFL{either }\DIFaddendFL 2\DIFdelbeginFL \DIFdelFL{to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, 10, or }\DIFaddendFL 50.   
%( only subtau real refers to $t\bar{t}$ MC with leading tau faked by other object, subleading tau from real contribution. 
The lower panels show the \DIFaddbeginFL \DIFaddFL{ratio of }\DIFaddendFL data to prediction\DIFdelbeginFL \DIFdelFL{ratio}\DIFdelendFL .}
\label{fig:mtt}
\end{figure}
...
Table~\ref{tab:had_sys_impact} shows a summary of the dominant \DIFdelbegin \DIFdel{systematics uncertainties on }\DIFdelend \DIFaddbegin \DIFadd{systematic uncertainties in }\DIFaddend the measured $\BR(t\to qH)$ branching ratio
...
Uncertainties associated with jets arise from the jet energy scale \DIFdelbegin \DIFdel{and resolution}\DIFdelend \DIFaddbegin \DIFadd{(JES)
and resolution (JER)}\DIFaddend , and the efficiency to pass the JVT requirements. 
The largest contribution \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{comes }\DIFaddend from the jet energy scale, whose uncertainty dependence on jet $\pt$ and $\eta$, jet flavour, and pile-up treatment
\DIFdelbegin \DIFdel{, 
}\DIFdelend is split into 43 uncorrelated components that are treated independently~\cite{Aaboud:2017jcu}. The total JES uncertainty is
below 5\% for most jets and below 1\% for central jets with $\pt$ between 300\DIFdelbegin \DIFdel{GeV }\DIFdelend \DIFaddbegin \DIFadd{~}\GeV\DIFadd{\ }\DIFaddend and 2\DIFdelbegin \DIFdel{TeV}\DIFdelend \DIFaddbegin \DIFadd{~}\TeV\DIFaddend . The difference between the JER \DIFaddbegin \DIFadd{values
}\DIFaddend in data and MC \DIFaddbegin \DIFadd{events }\DIFaddend is represented by one \DIFdelbegin \DIFdel{NP}\DIFdelend \DIFaddbegin \DIFadd{nuisance parameter (NP)}\DIFaddend . It is applied \DIFdelbegin \DIFdel{on the MC }\DIFdelend \DIFaddbegin \DIFadd{to the MC events }\DIFaddend by smearing the jet $\pt$ within the prescribed uncertainty.
...
Efficiencies to tag $b$-jets and $c$-jets in the simulation are corrected \DIFaddbegin \DIFadd{by $\pt$-dependent factors }\DIFaddend to match the efficiencies in data\DIFdelbegin \DIFdel{by $\pt$-dependent factors,
whereas }\DIFdelend \DIFaddbegin \DIFadd{,
while }\DIFaddend the light-jet efficiency is scaled by $\pt$- and $\eta$-dependent factors.
...
The uncertainties in these scale factors \DIFdelbegin \DIFdel{include }\DIFdelend \DIFaddbegin \DIFadd{come from }\DIFaddend a total of 44 independent sources affecting $b$-jets, 19 \DIFdelbegin \DIFdel{source }\DIFdelend \DIFaddbegin \DIFadd{sources }\DIFaddend affecting $c$-jets, and 19 sources affecting \DIFdelbegin \DIFdel{light-jets}\DIFdelend \DIFaddbegin \DIFadd{light jets}\DIFaddend . 
These systematic uncertainties are taken \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{to be }\DIFaddend uncorrelated between $b$-jets, $c$-jets, and \DIFdelbegin \DIFdel{light-jets}\DIFdelend \DIFaddbegin \DIFadd{light jets}\DIFaddend .
...
Scale factors are applied to reweight simulated events in order to obtain the pile-up distribution corresponding to \DIFaddbegin \DIFadd{the }\DIFaddend data.
An uncertainty \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend these reweighting scale factors is estimated by \DIFdelbegin \DIFdel{varying the reweighting to }\DIFdelend \DIFaddbegin \DIFadd{reweighting }\DIFaddend the profile in data \DIFaddbegin \DIFadd{while keeping it }\DIFaddend within its uncertainties.
...
A number of sources of systematic uncertainty affecting the modelling of $t\bar{t}$+jets are considered: the choice of \DIFdelbegin \DIFdel{the }\DIFdelend renormalisation and factorisation scale in the matrix-element calculation, the choice of \DIFdelbegin \DIFdel{the }\DIFdelend matching scale when matching the matrix elements to the parton \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{shower }\DIFaddend generator, the uncertainty in the value of \DIFdelbegin \DIFdel{$\alpha_s$ when modeling }\DIFdelend \DIFaddbegin \DIFadd{$\alpha_\text{s}$ when modelling }\DIFaddend initial-state radiation (ISR), \DIFaddbegin \DIFadd{and
}\DIFaddend the choice of \DIFdelbegin \DIFdel{the }\DIFdelend renormalisation scale when \DIFdelbegin \DIFdel{modeling }\DIFdelend \DIFaddbegin \DIFadd{modelling }\DIFaddend final-state radiation (FSR).
...
The $h_{\mathrm{damp}}$ parameter\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend which controls the amount of radiation produced by the parton shower in
\DIFdelbegin \DIFdel{POWHEG-BOX }\DIFdelend \DIFaddbegin \powheg\DIFadd{\,}\DIFaddend v2\DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend is set to 1.5$m_t$ in the \DIFdelbegin \DIFdel{nominal case. Alternative samples are generated with $h_{\mathrm{damp}}$=3$m_t$}\DIFdelend \DIFaddbegin \DIFadd{$t\bar t$ sample. An alternative sample was generated with $h_{\mathrm{damp}}=3m_t$}\DIFaddend . The
difference between \DIFaddbegin \DIFadd{the }\DIFaddend two samples is treated as a systematic \DIFdelbegin \DIFdel{named as ``}\DIFdelend \DIFaddbegin \DIFadd{uncertainty called `}\DIFaddend $t\bar{t}$ $h_{\mathrm{damp}}$\DIFdelbegin \DIFdel{''}\DIFdelend \DIFaddbegin \DIFadd{'}\DIFaddend . 
...
by comparing the predictions from {\powheg} interfaced either to \DIFdelbegin %DIFDELCMD < {\pythiaeight} %%%
\DIFdel{or }%DIFDELCMD < {\herwig7}%%%
\DIFdelend \DIFaddbegin \textsc{\DIFadd{Pythia\,8}} \DIFadd{or }\textsc{\DIFadd{Herwig\,7}}\DIFaddend .
The latter uses the \DIFdelbegin \DIFdel{MMHT2014 LO}\DIFdelend \DIFaddbegin \textsc{\DIFadd{MMHT2014lo}}\DIFaddend ~\cite{Harland-Lang:2014zoa} PDF set in combination with the H7UE tune~\cite{Bellm:2015jjp}.
The uncertainty in the modelling of additional radiation from the PS \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend assessed by
...
Another significant background in \DIFaddbegin \DIFadd{the }\DIFaddend hadronic channel stems from the $Z\rightarrow \tau\tau$ samples\DIFdelbegin \DIFdel{, several }\DIFdelend \DIFaddbegin \DIFadd{. Several }\DIFaddend sources of uncertainty are
considered for these samples: the PDF variation\DIFaddbegin \DIFadd{, which is evaluated by }\DIFaddend considering the standard deviation
\DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{produced by the event weights from the }\DIFaddend 100 NNPDF replicas \DIFdelbegin \DIFdel{event weights
of NNPDF3.0nnlo}\DIFdelend \DIFaddbegin \DIFadd{for the }\textsc{\DIFadd{NNPDF3.0nnlo}}\DIFaddend ~\cite{Ball:2015NNPDF} PDF set used in \DIFdelbegin \DIFdel{Sherpa,
renormalisation ($\mu_{R}$}\DIFdelend \DIFaddbegin \sherpa\DIFadd{,
the renormalisation ($\mu_\text{r}$}\DIFaddend ) and factorisation \DIFdelbegin \DIFdel{scales ($\mu_{F}$) , }\DIFdelend \DIFaddbegin \DIFadd{($\mu_\text{f}$) scales, the }\DIFaddend jet-to-parton matching uncertainty, \DIFaddbegin \DIFadd{the }\DIFaddend resummation scale uncertainty,
\DIFaddbegin \DIFadd{the }\DIFaddend variation in the choice of \DIFdelbegin \DIFdel{$\alpha_{S}$, alternative PDF variation evaluated }\DIFdelend \DIFaddbegin \DIFadd{$\alpha_\text{s}$, and the use of alternative PDFs, which is evaluated by }\DIFaddend comparing predictions from \DIFdelbegin \DIFdel{NNPDF3.0nnlo }\DIFdelend \DIFaddbegin \DIFadd{the }\textsc{\DIFadd{NNPDF3.0nnlo}} \DIFaddend PDF set (nominal)
with \DIFdelbegin \DIFdel{MMHT2014nnlo68cl and CT14nnlo}\DIFdelend \DIFaddbegin \DIFadd{those from the }\textsc{\DIFadd{MMHT2014nnlo68cl}} \DIFadd{and }\textsc{\DIFadd{CT14nnlo}}\DIFaddend ~\cite{Lai:2010vv,Gao:2013xoa} PDF sets.
...
difference between the data and \DIFaddbegin \DIFadd{the }\DIFaddend total background prediction in the different analysis regions considered, but requiring exactly zero, one, and two $b$-jets.
This is \DIFdelbegin \DIFdel{mainly driven by the disagreement between MC simulation }\DIFdelend \DIFaddbegin \DIFadd{driven mainly by differences between MC simulations }\DIFaddend in the $V$+jets regions with \DIFaddbegin \DIFadd{a }\DIFaddend high multiplicity of jets.  
...
$+5\%$/$-4\%$ uncertainty \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the total cross section\DIFaddbegin \DIFadd{, which is }\DIFaddend estimated as a weighted average 
...
Additional uncertainties associated with the parton shower, hadronisation and ISR/FSR \DIFdelbegin \DIFdel{radiations }\DIFdelend \DIFaddbegin \DIFadd{radiation }\DIFaddend are also considered \DIFaddbegin \DIFadd{by }\DIFaddend using the same procedure
as \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $t\bar t$. Uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the diboson background normalisation include \DIFaddbegin \DIFadd{those estimated from }\DIFaddend variations of the \DIFdelbegin \DIFdel{renormalization and factorization scale,  NNPDF3.0nnlo variations and $\alpha_{S}$ }\DIFdelend \DIFaddbegin \DIFadd{renormalisation and factorisation scales,  }\textsc{\DIFadd{NNPDF3.0nnlo}} \DIFadd{variations and $\alpha_\text{s}$ }\DIFaddend variations. Uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the $\ttbar V$ and $\ttbar H$ cross sections are \DIFaddbegin \DIFadd{estimated to be }\DIFaddend 12\% and \DIFdelbegin \DIFdel{$+5.8\%-9.2\%$}\DIFdelend \DIFaddbegin [\DIFadd{$+5.8\%/-9.2\%$}]\DIFaddend , respectively,
from the uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend their respective NLO theoretical cross sections~\cite{ttZRun2}. 
...
The statistical uncertainties \DIFdelbegin \DIFdel{on the fake tau background }\DIFdelend \DIFaddbegin \DIFadd{of the fake-$\tau$-background }\DIFaddend calibration in the leptonic \DIFdelbegin \DIFdel{channel }\DIFdelend \DIFaddbegin \DIFadd{channels }\DIFaddend are applied with uncorrelated uncertainties for
different \DIFdelbegin \DIFdel{fake tau sources }\DIFdelend \DIFaddbegin \DIFadd{sources of fake $\tau$-leptons }\DIFaddend and different $\pt$ slices. The uncertainties \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the MC modelling of the various processes used for
evaluation of the fake factors in the CRtt \DIFdelbegin \DIFdel{'s }\DIFdelend are treated as fully correlated in the fit.
The uncertainty \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the ABCD method is applied \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the normalisation factors for \DIFdelbegin \DIFdel{muon and electron respectively including both
,
}\DIFdelend \DIFaddbegin \DIFadd{muons and electrons, including both
}\DIFaddend its statistical fluctuation and the differences \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend the normalisation factors \DIFdelbegin \DIFdel{between the }\DIFdelend \DIFaddbegin \DIFadd{for the various }\DIFaddend leptonic channels.
The uncertainties in the \DIFdelbegin \DIFdel{fake factor method applied in }\DIFdelend \DIFaddbegin \DIFadd{fake-factor method applied to the }\DIFaddend hadronic channel includes \DIFdelbegin \DIFdel{statistical uncertainties }\DIFdelend \DIFaddbegin \DIFadd{the statistical uncertainty }\DIFaddend of each fake factor and
\DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{differences between the }\DIFaddend sets of fake factors derived from different \DIFdelbegin \DIFdel{signal depleted }\DIFdelend \DIFaddbegin \DIFadd{signal-depleted }\DIFaddend CRs.
...
Uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the Higgs boson branching ratios are taken
%from the measured values in Ref.~\cite{Zyla:2020zbs}.
into account \DIFaddbegin \DIFadd{by }\DIFaddend following the recommendation in Ref.~\cite{deFlorian:2016spz}.
The \DIFdelbegin \DIFdel{uncertainty of }\DIFdelend \DIFaddbegin \DIFadd{uncertainties due to }\DIFaddend ISR, FSR, \DIFdelbegin \DIFdel{scale and PDF }\DIFdelend \DIFaddbegin \DIFadd{the scales, and the PDFs, }\DIFaddend are considered and are treated \DIFaddbegin \DIFadd{as }\DIFaddend fully correlated in all SRs. The parton shower uncertainties are estimated by comparing
the nominal \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{sample with }\DIFaddend an alternative sample interfaced with \DIFdelbegin %DIFDELCMD < {\herwig7}%%%
\DIFdelend \DIFaddbegin \textsc{\DIFadd{Herwig\,7}}\DIFaddend .
...
The final discriminant distributions are the BDT outputs \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend all the considered analysis regions. They are jointly analysed \DIFdelbegin \DIFdel{to test }\DIFdelend for the 
presence of a signal. The BDT distributions are binned to \DIFdelbegin \DIFdel{maximize the signalsensitivity}\DIFdelend \DIFaddbegin \DIFadd{maximise sensitivity to the signal}\DIFaddend .
...
and \DIFaddbegin \DIFadd{on }\DIFaddend $\theta$, a set of nuisance parameters that encode the effect of systematic uncertainties on the signal and background expectations.
The $pp\rightarrow tH$ production cross section is related to $\BR(t\rightarrow qH)$ \DIFdelbegin \DIFdel{based on }\DIFdelend \DIFaddbegin \DIFadd{through }\DIFaddend their dimension-6 operators~\cite{fcnc_production_theory}.
\DIFdelbegin \DIFdel{Therefore, }\DIFdelend \DIFaddbegin \DIFadd{It follows that }\DIFaddend the expected total number of events in a given bin depends on $\mu$ and $\theta$. 
All nuisance parameters are subject to Gaussian constraints in the likelihood \DIFaddbegin \DIFadd{function}\DIFaddend .
For a given value of $\mu$, the nuisance parameters $\theta$ allow variations of the expectations for signal and background
\DIFdelbegin \DIFdel{according to }\DIFdelend \DIFaddbegin \DIFadd{consistent with }\DIFaddend the corresponding systematic uncertainties, and their fitted values result in the deviations from
the nominal expectations that globally provide the best fit to the data.
This procedure \DIFdelbegin \DIFdel{allows a reduction of }\DIFdelend \DIFaddbegin \DIFadd{reduces }\DIFaddend the impact of systematic uncertainties on 
the search\DIFaddbegin \DIFadd{'s }\DIFaddend sensitivity by taking advantage of the highly populated background-dominated bins included in the likelihood fit.
...
Statistical uncertainties in each bin of the predicted final discriminant distributions are taken into account \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{through }\DIFaddend dedicated parameters in the fit.     
The best-fit $\BR(t\to qH)$ is obtained by performing a binned likelihood fit to the data under the signal-plus-background
hypothesis, \DIFaddbegin \DIFadd{and }\DIFaddend maximising the likelihood function ${\cal L}(\mu,\theta)$ over $\mu$ and $\theta$.

The fitting procedure was initially validated through extensive studies using \DIFdelbegin \DIFdel{pseudo data, }\DIFdelend \DIFaddbegin \DIFadd{pseudo-data (which is }\DIFaddend defined as the sum of all predicted backgrounds 
plus an injected signal of variable strength\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend as well as by performing fits to real data where bins of the final \DIFdelbegin \DIFdel{discriminant }\DIFdelend \DIFaddbegin \DIFadd{discriminating }\DIFaddend variable with 
a signal contamination above 10\% are excluded (referred to as \DIFdelbegin \DIFdel{blinding }\DIFdelend \DIFaddbegin \DIFadd{`blinding' }\DIFaddend requirements).
In both cases, the robustness of the model \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{with respect to }\DIFaddend systematic uncertainties is established by verifying the stability of the fitted background 
...
\DIFdelbegin \DIFdel{$q_\mu = -2\ln({\cal L}(\mu,{\hat{\theta}}_\mu)/{\cal L}(\hat{\mu},\hat{\theta}))$}\DIFdelend \DIFaddbegin \DIFadd{$q_\mu = -2\ln({\cal L}(\mu,{\hat{\hat{\theta}}}_\mu)/{\cal L}(\hat{\mu},\hat{\theta}))$}\DIFaddend ,
where $\hat{\mu}$ and $\hat{\theta}$ are the values of the parameters that
maximise the likelihood function (subject to the constraint $0\leq \hat{\mu} \leq \mu$), and \DIFdelbegin \DIFdel{${\hat{\theta}}_\mu$ }\DIFdelend \DIFaddbegin \DIFadd{the ${\hat{\hat{\theta}}}_\mu$ }\DIFaddend are the values of the
...
For a given signal scenario, values of \DIFdelbegin \DIFdel{the }\DIFdelend $\BR(t\to qH)$ yielding CL$_{\textrm{s}} < 0.05$,
where CL$_{\textrm{s}}$ is computed using the asymptotic approximation~\cite{Cowan:2010js}, are excluded \DIFdelbegin \DIFdel{at $\geq 95\%$ CL}\DIFdelend \DIFaddbegin \DIFadd{with at least 95\% confidence}\DIFaddend .
...
\DIFdelbegin \DIFdel{due to the anti-correlations among sources of systematic uncertainty }\DIFdelend %DIF > due to the anti-correlations among sources of systematic uncertainty
...
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:asimov_postfitbdtHu} and~\ref{fig:asimov_postfitbdtHc} show \DIFdelbegin \DIFdel{a post-fit with signal plus background (S+B) }\DIFdelend \DIFaddbegin \DIFadd{the BDT output distributions after the signal-plus-background fit }\DIFaddend to the 
data for the $tcH$ and $tuH$ \DIFdelbegin \DIFdel{search separately}\DIFdelend \DIFaddbegin \DIFadd{searches, respectively}\DIFaddend .
...
The observed and predicted yields after \DIFaddbegin \DIFadd{a background-only }\DIFaddend fit to the data \DIFdelbegin \DIFdel{with background only are summarized }\DIFdelend \DIFaddbegin \DIFadd{are summarised }\DIFaddend in Table~\ref{tab:HtautauPostfitYieldsUnblind}.
%pre-fit and post-fit yields can be found in Appendix~\ref{sec:prepostfit_yields_Htautau_appendix}.
A slight excess \DIFdelbegin \DIFdel{in data is observed above the expected background }\DIFdelend \DIFaddbegin \DIFadd{of data events, }\DIFaddend with a significance of 2.3$\sigma$, \DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{is observed above the expected background. This is }\DIFaddend mainly in the high \DIFdelbegin \DIFdel{BDT score }\DIFdelend \DIFaddbegin \DIFadd{BDT-score }\DIFaddend region of the most sensitive \DIFdelbegin \DIFdel{$t_{\ell}\thadhad$ channel, }\DIFdelend \DIFaddbegin \DIFadd{channel, $t_{\ell}\thadhad$, }\DIFaddend as shown in
Table~\ref{tab:limits_summary}.
The kinematic distributions for the observed excess in
the high \DIFdelbegin \DIFdel{BDT region are }\DIFdelend \DIFaddbegin \DIFadd{BDT-score region were }\DIFaddend checked. Within the large statistical uncertainty, the observed distributions are compatible with the background shapes, but also with a small signal contribution. There is no indication that the excess is from a specific data period.
The background modelling in this signal region \DIFdelbegin \DIFdel{has also been }\DIFdelend \DIFaddbegin \DIFadd{was also }\DIFaddend checked using the VR in which both $\thad$ candidates have the same charge.
...
\DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$ }\DIFdelendFL (a) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\tauhad$-1j }\DIFdelendFL (b) \DIFdelbeginFL \DIFdelFL{,  }\DIFdelendFL $t_{\ell}\tauhad$\DIFdelbeginFL \DIFdelFL{-2j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1j,  }\DIFaddendFL (c) \DIFdelbeginFL \DIFdelFL{, $t_h\tlhad$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\ell}\tauhad$}\DIFaddendFL -2j\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (d) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\tlhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (e) \DIFaddbeginFL \DIFaddFL{$t_h\tlhad$-3j}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$-SS }\DIFdelendFL (f) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$-SS}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_h\thadhad$-2j }\DIFdelendFL (g) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\thadhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (h) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_h\thadhad$-3j \DIFdelbeginFL \DIFdelFL{SS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (i) \DIFaddbeginFL \DIFaddFL{$t_h\thadhad$-3j SS}\DIFaddendFL .  
The total \DIFdelbeginFL \DIFdelFL{size of the }\DIFdelendFL statistical and systematic \DIFdelbeginFL \DIFdelFL{uncertainties }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty }\DIFaddendFL is indicated by the hatched band. The signal shapes of $tt(uH)$, $tH$, and their sum are also shown using a normalisation of $2 \times\BR(t\to uH)$ of 0.1\%. 
}
...
%$t_{\ell}\thadhad$ (a),  $t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), $t_{\ell}\thadhad$ SS (f), $t_h\thadhad$-2j (g), and $t_h\thadhad$-3j (h), where $t_{\ell}\thadhad$ SS is used for the background validation.
\DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$ }\DIFdelendFL (a) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\tauhad$-1j }\DIFdelendFL (b) \DIFdelbeginFL \DIFdelFL{,  }\DIFdelendFL $t_{\ell}\tauhad$\DIFdelbeginFL \DIFdelFL{-2j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1j,  }\DIFaddendFL (c) \DIFdelbeginFL \DIFdelFL{, $t_h\tlhad$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\ell}\tauhad$}\DIFaddendFL -2j\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (d) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\tlhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (e) \DIFaddbeginFL \DIFaddFL{$t_h\tlhad$-3j}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_{\ell}\thadhad$-SS }\DIFdelendFL (f) \DIFaddbeginFL \DIFaddFL{$t_{\ell}\thadhad$-SS}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$t_h\thadhad$-2j }\DIFdelendFL (g) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_h\thadhad$\DIFdelbeginFL \DIFdelFL{-3j }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2j, }\DIFaddendFL (h) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_h\thadhad$-3j \DIFdelbeginFL \DIFdelFL{SS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (i) \DIFaddbeginFL \DIFaddFL{$t_h\thadhad$-3j SS}\DIFaddendFL . 
The total \DIFdelbeginFL \DIFdelFL{size of the }\DIFdelendFL statistical and systematic
\DIFdelbeginFL \DIFdelFL{uncertainties }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty }\DIFaddendFL is indicated by the hatched band. The signal shapes of $tt(cH)$, $tH$, and their sum are also shown using a normalisation of $2 \times\BR(t\to cH)$ of 0.1\%.
}
\label{fig:asimov_postfitbdtHc}
\end{figure}
...
\begin{table}[htbp]
\caption{
  Predicted and observed yields in each of the analysis regions considered. The background prediction is shown after a background-only fit \DIFaddbeginFL \DIFaddFL{is applied }\DIFaddendFL to \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL data.
  Also shown are the \DIFdelbeginFL \DIFdelFL{signal expectations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{expected signals }\DIFaddendFL for $\Hc$ and
  ...
  diboson, $\ttbar V$, $\ttbar H$, single-top-quark, and other small backgrounds are combined into a single background source referred to as \DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Other MC\DIFdelbeginFL \DIFdelFL{'' }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{' }\DIFaddendFL in the leptonic channels,
  whereas single-top-quark and the small contributions are combined into \DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Rare\DIFdelbeginFL \DIFdelFL{'' }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{' }\DIFaddendFL in the hadronic channels.
  The quoted uncertainties are the sum in quadrature of \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL statistical and systematic uncertainties of the yields.}
\small
\centering
...
Upper limits \DIFaddbegin \DIFadd{on $\BR(t\to cH)$ and $\BR(t\to uH)$ }\DIFaddend are derived using the CL$_{\textrm{s}}$ method~\cite{Junk:1999kv,Read:2002hq}, and  
\DIFaddbegin \DIFadd{the }\DIFaddend observed (expected) 95\% CL limits are 
\DIFdelbegin \DIFdel{set on $\BR(t\to cH)$ and $\BR(t\to uH)$:
}\DIFdelend $\BR(t\to cH)<9.4 \times 10^{-4}\,(4.8^{+2.2}_{-1.4}\times10^{-4})$, assuming $\BR(t\to uH)=0$, and $\BR(t\to uH)<6.9 \times 10^{-4}\,(3.5^{+1.5}_{-1.0}\times10^{-4})$, assuming $\BR(t\to cH)=0$.
These results are dominated by the leptonic channels, \DIFdelbegin \DIFdel{which have a sensitivity }\DIFdelend \DIFaddbegin \DIFadd{whose sensitivity is }\DIFaddend a factor of two better than that of the hadronic channels.
The expected sensitivity \DIFdelbegin \DIFdel{has been improved upon }\DIFdelend \DIFaddbegin \DIFadd{is a factor of five better than that of }\DIFaddend the previous ATLAS \DIFdelbegin \DIFdel{result }\DIFdelend \DIFaddbegin \DIFadd{search, which was }\DIFaddend based on 36 fb$^{-1}$ of data \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{and used }\DIFaddend $H\to \tau\tau$ \DIFdelbegin \DIFdel{decay~\mbox{%DIFAUXCMD
\cite{fcnc36}}\hspace{0pt}%DIFAUXCMD
by a factor of~5. }\DIFdelend \DIFaddbegin \DIFadd{decays~\mbox{%DIFAUXCMD
\cite{fcnc36}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend A factor of~2 improvement in sensitivity comes from the larger dataset, and a further factor of~2.5 comes from including
...
the uncertainties \DIFdelbegin \DIFdel{on the measured $\BR(H\to \tautau)$ }\DIFdelend \DIFaddbegin \DIFadd{in the measured $H\to \tautau$ }\DIFaddend branching ratio,
the \DIFdelbegin \DIFdel{normalization and factorization scales, }\DIFdelend \DIFaddbegin \DIFadd{normalisation and factorisation scales, the }\DIFaddend $b$-tagging \DIFaddbegin \DIFadd{efficiency}\DIFaddend , the choice of parton shower and \DIFdelbegin \DIFdel{hadronization }\DIFdelend \DIFaddbegin \DIFadd{hadronisation schemes }\DIFaddend for $t\bar t$ modelling, and
the \DIFdelbegin \DIFdel{fake }\DIFdelend \DIFaddbegin \DIFadd{fake-}\DIFaddend $\had$ background estimation in the hadronic channels. Their absolute impacts on the signal strength are \DIFdelbegin \DIFdel{summarized }\DIFdelend \DIFaddbegin \DIFadd{summarised }\DIFaddend in Table~\ref{tab:had_sys_impact}.
...
\begin{table}[h!]
  \caption{Absolute uncertainties \DIFdelbeginFL \DIFdelFL{on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL $\BR(t\to qH)$ ($q=u,c$) obtained from the combined fit to \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL data. The uncertainties are symmetrised
    \DIFdelbeginFL \DIFdelFL{for
     a presentation purpose }\DIFdelendFL and grouped into the categories described in \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL Section~\ref{sec:systematics}.}
\label{tab:had_sys_impact}
% \begin{center}
%   \begin{tabular}{%
%       @{}l%
...
% \end{table}
 \begin{center}
   \begin{tabular}{%
     ...
     Lepton ID                               & 0.6           &0.8         \\
     $\met$                                  & 0.7           &0.7         \\
     \DIFdelbeginFL \DIFdelFL{Fake lepton  modeling                   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fake-lepton  modelling                   }\DIFaddendFL & 1.2           &1.7         \\
     JES and JER                             & 2.5           &3.3         \\
     Flavour tagging                         & 2.7           &3.7         \\
     $t\bar{t}$ \DIFdelbeginFL \DIFdelFL{modeling                     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{modelling                     }\DIFaddendFL & 2.6           &3.9         \\
     Other MC \DIFdelbeginFL \DIFdelFL{modeling                       }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{modelling                       }\DIFaddendFL & 2.1           &3.0         \\
     \DIFdelbeginFL \DIFdelFL{Fake }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fake-}\DIFaddendFL $\tau$ \DIFdelbeginFL \DIFdelFL{modeling                    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{modelling                    }\DIFaddendFL & 3.3           &4.7         \\
     Signal \DIFdelbeginFL \DIFdelFL{modeling including Br($H\to\tau\tau$)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{modelling including $\BR(H\to\tau\tau)$}\DIFaddendFL &1.8        &1.5         \\
     $\tau$ ID                               & 3.3           &4.4         \\
     Luminosity and \DIFdelbeginFL \DIFdelFL{Pileup                   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pile-up                   }\DIFaddendFL & 1.7           &2.4         \\    
...
Upper limits on the branching ratios $\BR(t\to qH)$ (\DIFdelbegin \DIFdel{$q=u,c$}\DIFdelend \DIFaddbegin \DIFadd{$q=u,\, c$}\DIFaddend ) can be translated into upper limits on the dimension-6 (D6) operator Wilson coefficients \DIFaddbegin \DIFadd{($C^{i3}_{u\phi}$,
$C^{3i}_{u\phi}$) }\DIFaddend appearing in the effective field theory Lagrangian for the $tqH$ interaction~\cite{fcnc_production_theory}:
%
\DIFdelbegin \begin{displaymath}
  \DIFdel{\mathcal{L}_{EFT} = \frac{C^{i3}_{u\phi}}{\Lambda^{2}}(\phi^{\dagger}\phi)(\bar{q_{i}}t)\tilde{\phi} + \frac{C^{3i}_{u\phi}}{\Lambda^{2}}(\phi^{\dagger}\phi)(\bar{t}q_{i})\tilde{\phi}
  %DIFDELCMD < \label{eq:eq01}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
  \DIFadd{\mathcal{L}_{EFT} = \frac{C^{i3}_{u\phi}}{\Lambda^{2}}(\phi^{\dagger}\phi)(\bar{q_{i}}t)\tilde{\phi} + \frac{C^{3i}_{u\phi}}{\Lambda^{2}}(\phi^{\dagger}\phi)(\bar{t}q_{i})\tilde{\phi}
  \label{eq:eq01}
}\end{equation*}\DIFaddend 
%
where the subscript \DIFdelbegin \DIFdel{i= 1, 2 }\DIFdelend \DIFaddbegin \DIFadd{$i=1,\, 2$ }\DIFaddend represents the generation of the \DIFdelbegin \DIFdel{light quark fields ($q=u, c$}\DIFdelend \DIFaddbegin \DIFadd{light-quark fields ($q=u,\, c$}\DIFaddend ).
The branching ratio $\BR(t\to qH)$ is estimated as the ratio of its partial width to the SM $t \to Wb$ partial width including next-to-leading-order QCD corrections\DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend coefficients can be extracted as $C_{q\phi} = \sqrt{1946.6~\BR(t\to qH)}$~\cite{fcnc_production_theory}. The $C_{q\phi}$ coefficient corresponds to the sum in quadrature of the coefficients relative to the two possible chirality combinations of the quark fields,
$C_{q\phi} =\sqrt{(C^{i3}_{q\phi})^2 + (C^{3i}_{q\phi})^2}$~\cite{fcnc_production_theory}. The observed (expected) upper limits on the D6 Wilson coefficients from the combination of the \DIFdelbegin \DIFdel{searches }\DIFdelend \DIFaddbegin \DIFadd{search
results }\DIFaddend are $C_{c\phi}<1.35\,(0.97)$ and $C_{u\phi}<1.16\,(0.82)$ for a \DIFdelbegin \DIFdel{new physics }\DIFdelend \DIFaddbegin \DIFadd{new-physics }\DIFaddend scale $\Lambda=1$~\DIFdelbegin \DIFdel{TeV}\DIFdelend \DIFaddbegin \TeV\DIFaddend . 
...
\begin{table}[t!]
  \caption{\DIFdelbeginFL %DIFDELCMD < \small{Summary of 95\% CL upper limits on $\BR(t \to cH)$ and $\BR(t \to uH)$, significance and best-fit branching ratio in signal regions with a
%DIFDELCMD <   benchmark branching ratio of $\BR(t \to qH)=0.1\%$}%%%
\DIFdelendFL \DIFaddbeginFL \small{Summary of 95\% CL upper limits on $\BR(t \to cH)$ and $\BR(t \to uH)$, significance and best-fit branching ratio in the signal regions with a
  benchmark branching ratio of $\BR(t \to qH)=0.1\%$}\DIFaddendFL . The expected significance is obtained from \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL Asimov fit\DIFaddbeginFL \DIFaddFL{~\mbox{%DIFAUXCMD
\cite{Cowan:2010js} }\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL with a signal injection corresponding to a branching ratio of 0.1\%. }
\begin{center}
...
%DIFDELCMD <                                 &  95\% CL upper limits[$10^{-3}$]  & Significance             &  $\BR[10^{-3}]$ &     95\% CL upper limits[$10^{-3}$]  & Significance   &  $\BR[10^{-3}]$  \\
%DIFDELCMD <                                 &  \multicolumn{2}{c}{Observed (Expected)}                     &        &     \multicolumn{2}{c}{Observed (Expected)}& \\
%DIFDELCMD < \midrule
%DIFDELCMD < $t_h\thadhad$-2j                & $1.80$($2.72^{+1.18}_{-0.76}$)&$-0.96$($0.78$) & $-1.03^{+1.03}_{-1.03}$&$1.07$($1.60^{+0.71}_{-0.45}$)&$-0.90$($1.31$) &$-0.55^{+0.58}_{-0.58}$ \\
%DIFDELCMD < $t_h\thadhad$-3j                & $1.14$($1.02^{+0.45}_{-0.29}$)&$0.34$($1.87$)  & $0.16^{+0.47}_{-0.47}$ &$0.97$($0.86^{+0.38}_{-0.24}$)&$0.36$($2.25$)  &$0.14^{+0.40}_{-0.40}$  \\ \midrule
%DIFDELCMD < Hadronic Combination            & $1.00$($0.95^{+0.42}_{-0.27}$)&$0.26$ ($1.99$) & $0.11^{+0.43}_{-0.43}$ &$0.76$($0.76^{+0.33}_{-0.21}$)&$0.12$($2.52$)  &$0.04^{+0.34}_{-0.34}$  \\
%DIFDELCMD < 

%DIFDELCMD < \midrule
%DIFDELCMD < $t_l\tauhad$-2j                 &$4.77$($4.23^{+1.72}_{-1.18}$) &$0.41$($0.47$)   &$0.85^{+2.06}_{-2.06}$  & $3.84$($3.48^{+1.42}_{-0.97}$) & $0.36$($0.58$)  &$0.61^{+1.68}_{-1.68}$\\
%DIFDELCMD < $t_l\tauhad$-1j                 &$3.80$($3.56^{+1.51}_{-0.99}$) &$0.22$($0.58$)   &$0.36^{+1.70}_{-1.70}$  & $2.98$($2.78^{+1.17}_{-0.78}$) & $0.22$($0.73$)  &$0.29^{+1.33}_{-1.33}$\\
%DIFDELCMD < $t_h\tlhad$-2j                  &$4.71$($5.71^{+2.68}_{-1.60}$) &$-0.52$($0.38$)  &$-1.36^{+2.56}_{-2.56}$ & $2.50$($2.97^{+1.25}_{-0.83}$) & $-0.47$($0.70$) &$-0.66^{+1.38}_{-1.38}$\\
%DIFDELCMD < $t_h\tlhad$-3j                  &$2.71$($2.71^{+1.25}_{-0.76}$) &$-0.03$($0.77$)  &$-0.03^{+1.26}_{-1.26}$ & $2.02$($2.03^{+0.86}_{-0.57}$) & $-0.05$($0.99$) &$-0.03^{+0.98}_{-0.98}$\\
%DIFDELCMD < $t_l\thadhad$                   &$1.35$($0.61^{+0.27}_{-0.17}$) &$2.64$($3.31$)   &$0.74^{+0.33}_{-0.33}$  & $0.97$($0.44^{+0.19}_{-0.12}$) & $2.64$($4.38$)  &$0.53^{+0.24}_{-0.24}$\\ \midrule
%DIFDELCMD < Leptonic Combination            &$1.25$($0.58^{+0.25}_{-0.16}$) &$2.61$($3.46$)   &$0.69^{+0.31}_{-0.31}$  & $0.88$($0.41^{+0.18}_{-0.11}$) & $2.60$($4.62$)  &$0.49^{+0.22}_{-0.22}$\\
%DIFDELCMD < \midrule
%DIFDELCMD < Combination                     &$0.94$($0.48^{+0.20}_{-0.14}$)&$2.34$($4.02$)   &$0.51^{+0.24}_{-0.24}$ & $0.69$($0.35^{+0.15}_{-0.10}$)& $2.31$($5.18$)&$0.37^{+0.18}_{-0.18}$  \\
...
\end{tabular}
\caption{\small {\DIFaddbeginFL \DIFaddFL{(a) This figure shows }\DIFaddendFL 95\% CL upper limits on $\BR(t\to cH)$ \DIFdelbeginFL \DIFdelFL{(a) }\DIFdelendFL for the individual searches as well as their
combination, assuming $\BR(t\to uH)=0$. \DIFaddbeginFL \DIFaddFL{(b) This figure shows }\DIFaddendFL 95\% CL upper limits on $\BR(t\to uH)$ \DIFdelbeginFL \DIFdelFL{(b) }\DIFdelendFL for the individual searches as well as their
...
The corresponding upper limits on the D6 Wilson \DIFdelbegin \DIFdel{coefficients }\DIFdelend \DIFaddbegin \DIFadd{coefficient }\DIFaddend couplings in the $C_{u\phi}$ versus $C_{c\phi}$ plane are shown in Figure~\ref{fig:limits_combo_2D}(b).
...
\caption{\small {95\% CL upper limits (a) \DIFdelbeginFL \DIFdelFL{on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{plane of }\DIFdelendFL $\BR(t\to cH)$ versus $\BR(t\to uH)$ \DIFaddbeginFL \DIFaddFL{plane }\DIFaddendFL and (b) \DIFdelbeginFL \DIFdelFL{on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL the 
\DIFdelbeginFL \DIFdelFL{plane 
of }\DIFdelendFL $C_{c\phi}$ versus $C_{u\phi}$ \DIFaddbeginFL \DIFaddFL{plane }\DIFaddendFL for the combination of the searches. The observed limits (solid lines) are compared with the expected (median) limits under the background-only hypothesis (dotted lines). The surrounding shaded bands correspond to the 68\% and 95\% CL intervals around the expected limits, 
denoted by $\pm 1\sigma$ and $\pm 2\sigma$ \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL respectively.}}
%%=======
...
%-------------------------------------------------------------------------------
A search for flavour-changing neutral current processes involving a top quark, \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{another }\DIFaddend up-type quark ($q=u, c$), and a SM Higgs boson is presented. 
\DIFaddbegin \DIFadd{The search uses 139 fb$^{-1}$ of 13~}\TeV\DIFadd{\ $pp$ collisions recorded by the ATLAS detector at the LHC.
Evidence of }\DIFaddend FCNC $tqH$ \DIFdelbegin \DIFdel{interaction is searched for both in }\DIFdelend \DIFaddbegin \DIFadd{interactions is sought in both }\DIFaddend the $t\bar{t}$ decay mode, where one top quark decays \DIFdelbegin \DIFdel{according to the }\DIFdelend \DIFaddbegin \DIFadd{via }\DIFaddend SM processes and the other one decays through $t\rightarrow qH$, and \DIFdelbegin \DIFdel{in }\DIFdelend the production mode ($pp\rightarrow tH$)\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{, where }\DIFaddend a single top quark produced via \DIFdelbegin \DIFdel{FCNC interaction where }\DIFdelend \DIFaddbegin \DIFadd{the
FCNC interaction decays as }\DIFaddend $t\to Wb$.
\DIFdelbegin \DIFdel{The search is based on a dataset of $pp$ collisions at $\sqrt{s}=13~\tev$ recorded with the ATLAS detector at the CERN Large Hadron Collider and corresponding to an integrated luminosity of 139 fb$^{-1}$. The search selects events with two hadronically decaying $\tau$-lepton candidates ($\tauhad$) or at least one $\tauhad$ with an additional lepton (e,$\mu$), a $b$-jet as well as multiple jets. Multivariate techniques based on BDT are used to distinguish the signal from the background on the basis of their different kinematics.
}\DIFdelend %DIF > The search is based on a dataset of $pp$ collisions at $\sqrt{s}=13~\tev$ recorded with the ATLAS detector at the CERN Large Hadron Collider and corresponding to an integrated luminosity of 139 fb$^{-1}$. The search selects events with two hadronically decaying $\tau$-lepton candidates ($\tauhad$) or at least one $\tauhad$ with an additional lepton (e,$\mu$), a $b$-jet as well as multiple jets. Multivariate techniques based on BDT are used to distinguish the signal from the background on the basis of their different kinematics.
...
A slight excess of data is observed above background\DIFaddbegin \DIFadd{, }\DIFaddend with a significance of 2.3$\sigma$. \DIFaddbegin \DIFadd{Upper limits at the
}\DIFaddend 95\% \DIFdelbegin \DIFdel{CL upper limits }\DIFdelend \DIFaddbegin \DIFadd{confidence level }\DIFaddend are set on the $t\to qH$ branching ratios and the corresponding
...
\DIFdelbegin \DIFdel{Observed }\DIFdelend \DIFaddbegin \DIFadd{The observed }\DIFaddend (expected) 95\% CL upper limits \DIFdelbegin \DIFdel{are }\DIFdelend set on the $t\to cH$ and $t\to uH$ branching ratios \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend $9.4\times10^{-4}$ ($4.8^{+2.2}_{-1.4}\times10^{-4}$) and $6.9\times10^{-4}$ ($3.5^{+1.5}_{-1.0}\times10^{-4}$)\DIFaddbegin \DIFadd{, respectively}\DIFaddend .
...
The corresponding combined observed (expected) upper limits on the dimension-6 operator Wilson coefficients in
the effective $tqH$ couplings\DIFdelbegin \DIFdel{for a new physics }\DIFdelend \DIFaddbegin \DIFadd{, for a new-physics }\DIFaddend scale $\Lambda$ of 1~\DIFdelbegin \DIFdel{TeV }\DIFdelend \DIFaddbegin \TeV\DIFadd{, }\DIFaddend are $C_{c\phi} <1.35\, (0.97)$ and $C_{u\phi} <1.16\, (0.82)$\DIFdelbegin \DIFdel{, respectively}\DIFdelend .
%These results improve significantly upon previous ATLAS and CMS limits on the $t\rightarrow Hq$ branching ratios.
These results improve significantly \DIFaddbegin \DIFadd{upon the }\DIFaddend previously published ATLAS studies in this channel and provide more stringent \DIFdelbegin \DIFdel{limit than the }\DIFdelend \DIFaddbegin \DIFadd{limits than the previous combination of }\DIFaddend ATLAS  $t\rightarrow qH$ \DIFdelbegin \DIFdel{combination~\mbox{%DIFAUXCMD
\cite{fcnc36}}\hspace{0pt}%DIFAUXCMD
...
%DIF <  Version 24-Oct-2018
%DIF >  Version 2022-01-18

% Standard acknowledgements start here
%----------------------------------------------
\DIFaddbegin 

\DIFaddend We thank CERN for the very successful operation of the LHC, as well as the
...
CERN; 
\DIFdelbegin \DIFdel{CONICYT}\DIFdelend \DIFaddbegin \DIFadd{ANID}\DIFaddend , Chile; 
CAS, MOST and NSFC, China; 
\DIFdelbegin \DIFdel{COLCIENCIAS}\DIFdelend \DIFaddbegin \DIFadd{Minciencias}\DIFaddend , Colombia; 
\DIFdelbegin \DIFdel{MSMT CR, MPO CR and VSC CR, }\DIFdelend \DIFaddbegin \DIFadd{MEYS CR, }\DIFaddend Czech Republic; 
DNRF and DNSRC, Denmark; 
IN2P3-CNRS \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend CEA-DRF/IRFU, France; 
SRNSFG, Georgia; 
BMBF, HGF \DIFdelbegin \DIFdel{, }\DIFdelend and MPG, Germany; 
\DIFdelbegin \DIFdel{GSRT}\DIFdelend \DIFaddbegin \DIFadd{GSRI}\DIFaddend , Greece; 
RGC \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend Hong Kong SAR, China; 
...
NWO, Netherlands; 
RCN, Norway; 
\DIFdelbegin \DIFdel{MNiSW and NCN}\DIFdelend \DIFaddbegin \DIFadd{MEiN}\DIFaddend , Poland; 
FCT, Portugal; 
MNE/IFA, Romania; 
\DIFaddbegin \DIFadd{JINR; 
}\DIFaddend MES of Russia and NRC KI, Russian Federation; 
\DIFdelbegin \DIFdel{JINR; }\DIFdelend MESTD, Serbia; 
MSSR, Slovakia; 
ARRS and MIZ\v{S}, Slovenia; 
\DIFdelbegin \DIFdel{DST}\DIFdelend \DIFaddbegin \DIFadd{DSI}\DIFaddend /NRF, South Africa; 
\DIFdelbegin \DIFdel{MINECO}\DIFdelend \DIFaddbegin \DIFadd{MICINN}\DIFaddend , Spain; 
...
BCKDF, CANARIE, \DIFdelbegin \DIFdel{CRC and Compute Canada }\DIFdelend \DIFaddbegin \DIFadd{Compute Canada and CRC}\DIFaddend , Canada; 
COST, ERC, ERDF, Horizon 2020 \DIFdelbegin \DIFdel{, }\DIFdelend and Marie Sk{\l}odowska-Curie Actions, European Union; 
Investissements d'Avenir Labex\DIFdelbegin \DIFdel{and Idex, }\DIFdelend \DIFaddbegin \DIFadd{, Investissements d'Avenir Idex and }\DIFaddend ANR, France; 
...
\DIFaddbegin \DIFadd{Norwegian Financial Mechanism 2014-2021, Norway; 
NCN and NAWA, Poland; 
La Caixa Banking Foundation, }\DIFaddend CERCA Programme Generalitat de Catalunya \DIFaddbegin \DIFadd{and PROMETEO and GenT Programmes Generalitat Valenciana}\DIFaddend , Spain; 
\DIFaddbegin \DIFadd{G\"{o}ran Gustafssons Stiftelse, Sweden; 
}\DIFaddend The Royal Society and Leverhulme Trust, United Kingdom.

The crucial computing support from all WLCG partners is acknowledged gratefully, in particular from CERN, the ATLAS Tier-1 facilities at TRIUMF (Canada), NDGF (Denmark, Norway, Sweden), CC-IN2P3 (France), KIT/GridKA (Germany), INFN-CNAF (Italy), NL-T1 (Netherlands), PIC (Spain), ASGC (Taiwan), RAL (UK) and BNL (USA), the Tier-2 facilities worldwide and large non-WLCG resource providers. Major contributors of computing resources are listed in Ref.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ATL-GEN-PUB-2016-002}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ATL-SOFT-PUB-2021-003}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend %----------------------------------------------
%DIF >  Created with Glance <Atlas.Glance@cern.ch>
...
\DIFaddbegin \addcontentsline{toc}{part}{\DIFadd{Appendix}}
\DIFaddend %\addcontentsline{toc}{part}{Auxiliary Material}
%-------------------------------------------------------------------------------
%In a paper, an appendix is used for technical details that would otherwise disturb the flow of the paper.
%Such an appendix should be printed before the Bibliography.
\section{\DIFdelbegin \DIFdel{Tau fake }\DIFdelend \DIFaddbegin \DIFadd{Fake-$\thad$ }\DIFaddend scale factor calibration in the \DIFdelbegin \DIFdel{ttCRs}\DIFdelend \DIFaddbegin \DIFadd{CRtt}\DIFaddend }
\label{sec:tauFF_appendix}

Figure~\ref{fig:ttCR_mtt} presents the post-fit distributions of the leading $\thad$ $\pt$ in the \DIFdelbegin \DIFdel{ttCRs, which shows a }\DIFdelend \DIFaddbegin \DIFadd{CRtt. These show }\DIFaddend good agreement between the data and the fitted background model.
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tables}\DIFaddend ~\ref{tab:ff1_summary} and~\ref{tab:ff2_summary} \DIFdelbegin \DIFdel{summarize the fake scale factors of }\DIFdelend \DIFaddbegin \DIFadd{summarise the scale factors for }\DIFaddend 1-prong and 3-prong \DIFdelbegin \DIFdel{taus
}\DIFdelend \DIFaddbegin \DIFadd{fake $\tau$ decays
}\DIFaddend derived from the \DIFdelbegin \DIFdel{ttCRs including both }\DIFdelend \DIFaddbegin \DIFadd{CRtt including both the }\DIFaddend statistical and systematic uncertainties. 
...
(d) & (e) & (f)\\
\end{tabular}
\caption{Leading $\thad$ $\pt$  distributions obtained after the fit to data (\DIFdelbeginFL \DIFdelFL{``}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`}\DIFaddendFL Post-Fit\DIFdelbeginFL \DIFdelFL{''}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}\DIFaddendFL ) for the \DIFaddbeginFL \DIFaddFL{fake-}\DIFaddendFL $\thad$ \DIFdelbeginFL \DIFdelFL{fake }\DIFdelendFL scale factors in the following \DIFdelbeginFL \DIFdelFL{ttCRs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CRtt}\DIFaddendFL :
  \DIFaddbeginFL \DIFaddFL{(a) }\DIFaddendFL $t_{\ell}t_{h}$2b$\thad$-2j OS\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{a}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{b}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_{\ell}t_{h}$2b$\thad$-2j SS\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{c}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_{\ell}t_{h}$2b$\thad$-3j OS\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{c}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{d}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL $t_{\ell}t_{h}$2b$\thad$-3j SS\DIFaddbeginFL \DIFaddFL{,
  }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{d}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{e}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{,
  }\DIFdelendFL $t_{\ell}t_{\ell}$1b$\thad$-nj \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{e}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{f}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL $t_{\ell}t_{\ell}$2b$\thad$-nj\DIFdelbeginFL \DIFdelFL{(f)}\DIFdelendFL .
  The total \DIFdelbeginFL \DIFdelFL{size of the }\DIFdelendFL statistical and systematic \DIFdelbeginFL \DIFdelFL{uncertainties }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty }\DIFaddendFL of the background prediction is indicated by the hatched band.
  Overflow events are included in the last bin. The lower panels show the \DIFaddbeginFL \DIFaddFL{ratio of }\DIFaddendFL data to prediction\DIFdelbeginFL \DIFdelFL{ratio}\DIFdelendFL .}
\label{fig:ttCR_mtt}
\end{figure}

\begin{table}
\caption{ Summary of \DIFdelbeginFL \DIFdelFL{fake tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fake-$\tau$ }\DIFaddendFL (1-prong) scale factors derived in the \DIFdelbeginFL \DIFdelFL{ttCRs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CRtt}\DIFaddendFL . The numbers are shown as \DIFdelbeginFL \DIFdelFL{: nominal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{central }\DIFaddendFL values, statistical \DIFaddbeginFL \DIFaddFL{uncertainties}\DIFaddendFL , and systematics uncertainties. }
\begin{center}
\begin{tabular}{lcccccc}
\toprule\toprule

\DIFdelbeginFL \DIFdelFL{Fake tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fake-$\tau$ }\DIFaddendFL types & \DIFdelbeginFL %DIFDELCMD < \multicolumn{3}{c}{1-prong tau}  %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{3}{c}{1-prong $\tau$ decay}  \DIFaddendFL \\ \midrule
...
\begin{table}
\caption{ Summary of \DIFdelbeginFL \DIFdelFL{fake tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fake-$\tau$ }\DIFaddendFL (3-prong) scale factors derived in the \DIFdelbeginFL \DIFdelFL{ttCRs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CRtt}\DIFaddendFL . The numbers are shown as \DIFdelbeginFL \DIFdelFL{: nominal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{central }\DIFaddendFL values, statistical \DIFaddbeginFL \DIFaddFL{uncertainties}\DIFaddendFL , and systematics \DIFdelbeginFL \DIFdelFL{erros}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainties}\DIFaddendFL . }
\begin{center}
\begin{tabular}{lcccccc}
\toprule\toprule

\DIFdelbeginFL \DIFdelFL{Fake tau }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fake-$\tau$ }\DIFaddendFL types        & \DIFdelbeginFL %DIFDELCMD < \multicolumn{3}{c}{3-prong tau}  %%%
