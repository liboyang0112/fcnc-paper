...
%DIF ADD ANA-TOPQ-2019-17-PAPER.tex                      Fri Mar 18 21:20:21 2022
...
\DIFdelendFL \DIFaddbeginFL \small{Summary of 95\% CL upper limits on $\BR(t \to Hc)$ and $\BR(t \to Hu)$ obtained from ATLAS and CMS Collaborations with Run 2 data. Each limit is obtained assuming the other branching ratio is null.}\DIFaddendFL }
\begin{center}
\small 
\begin{tabular}{ccccc}
\toprule\toprule
...
The contribution of background to the signal regions of the $t_h\hadhad$ and $t_h\lephad$ channels is reduced by having a kinematic constraints on the di-tau mass and the missing transverse energy in the event~\cite{Chen:2015nta}.
...
The events with one light lepton, two same-sign $\had$, and one $b$-jet, are also selected\DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{, enter in }\DIFaddend a validation region (VR) for the background estimation in the leptonic channels. 
%-------------------------------------------------------------------------------
...
The FFs are computed as the ratio of \DIFaddbegin \DIFadd{misidentified $\thad$ candidate that either pass or fail }\DIFaddend the \DIFdelbegin \DIFdel{Data-MC ($\mathrm{real~tau}$) yields in which the tau candidate is passed or failed the }\DIFdelend medium tau ID selection in the
$W$+jets \DIFdelbegin \DIFdel{samples}\DIFdelend \DIFaddbegin \DIFadd{processes}\DIFaddend ~\cite{ATLAS-CONF-2021-044}. 
...
The BDT discriminant is trained for each SR starting from a large list of variables. Then the variables are removed one at time and \DIFdelbegin \DIFdel{retraining }\DIFdelend \DIFaddbegin \DIFadd{the BDT is retrained }\DIFaddend until the
...
\item $\chi^2$ of the kinematic fit of the \DIFdelbegin \DIFdel{neutrino momentum}\DIFdelend \DIFaddbegin \DIFadd{invisible decay products of the tau leptons}\DIFaddend .
...
\caption{\DIFdelbeginFL \DIFdelFL{The leading }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Leading }\DIFaddendFL $\thad$ \DIFdelbeginFL \DIFdelFL{$p_T$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\pt$  }\DIFaddendFL distributions \DIFdelbeginFL \DIFdelFL{are compared between }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{obtained before }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{fit to data (``Pre-Fit'') showing 
  ...
  $t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), $t_h\thadhad$-2j (f), and $t_h\thadhad$-3j (g)\DIFdelbeginFL \DIFdelFL{before the fit to data ('Pre-Fit') after applying fake factors}\DIFdelendFL .
...
The uncertainties in the fake factor method applied in hadronic channel includes statistical uncertainties of each fake factor and different sets of fake \DIFdelbegin \DIFdel{factors derived }\DIFdelend \DIFaddbegin \DIFadd{factorsderived }\DIFaddend from different signal depleted CRs.
...
\begin{center}
  \begin{tabular}{%
    ...
    %$\tau$ ID                               & 3.3           &4.4         \\
    %Signal modeling including Br($H\to\tau\tau$)       & 5.3           &7.0         \\\midrule
    \DIFdelbeginFL \DIFdelFL{Sum systematics                           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Total systematic uncertainty                            }\DIFaddendFL & 11.2          &15.5        \\
    Data \DIFdelbeginFL \DIFdelFL{statistics                           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{statistical uncertainty                           }\DIFaddendFL & 14.1          &19.6         \\\midrule
...
These results improve significantly \DIFdelbegin \DIFdel{upon previous ATLAS and CMS limits on the }\DIFdelend \DIFaddbegin \DIFadd{previously published ATLAS studies in this channel and provide more stringent limit than the ATLAS  }\DIFaddend $t\rightarrow Hq$ \DIFdelbegin \DIFdel{branching ratios}\DIFdelend \DIFaddbegin \DIFadd{combination~\mbox{%DIFAUXCMD
