...
%DIF ADD ANA-TOPQ-2019-17-PAPER.tex                                              Sun Apr 17 20:47:59 2022
...
Observed (expected) 95\% CL upper limits are set on the $t\to cH$ and $t\to uH$ branching ratios of $9.4 \times 10^{-4}\, (4.8^{+2.2}_{-1.4}\times 10^{-4})$
...
the effective $tqH$ couplings are $C_{c\phi} <1.35\, (0.97)$ and $C_{u\phi} <1.16\, (0.82)$, respectively.}%for the new physics scale $\Lambda$ at 1~TeV, respectively.} 
...
%DIF -------
\AtlasCoverCommentsDeadline{9 June 2022} %DIF > 
%DIF -------
...
In the Standard Model (SM), the FCNC interaction is forbidden at tree level and suppressed at higher orders due to the Glashow-Iliopoulos-Maiani (GIM) mechanism~\cite{Glashow:1970gm}. \DIFdelbegin \DIFdel{Therefore, an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend observation of an enhanced rate of this decay would be \DIFdelbegin \DIFdel{a }\DIFdelend clear evidence for new physics.
Furthermore, if the $tqH$ interaction exists, the associated single-top and  Higgs  production process through this interaction would enhance
%becomes non-zero, enhancing
...
In these searches, one of the top quarks is required to decay into $Wb$, while the other top quark decays into $qH$ with a small branching ratio 
 $\BR(t\to qH)$, denoted\DIFaddbegin \DIFadd{~}\footnote{\DIFadd{In the following, $WbHq$ is used to denote both $W^+b H\bar{q}$ and its charge conjugate, $HqW^- \bar{b}$. Similarly, 
$WbWb$ is used to denote $W^+b W^- \bar{b}$.}} \DIFaddend as $\ttbar \to WbHq$\DIFdelbegin \footnote{\DIFdel{In the following, $WbHq$ is used to denote both $W^+b H\bar{q}$ and its charge conjugate, $HqW^- \bar{b}$. Similarly, 
$WbWb$ is used to denote $W^+b W^- \bar{b}$.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend .  The Higgs boson is assumed to have a mass of $m_H=125~\gev$ and to decay as predicted by
the SM.
...
Using 36.1~fb$^{-1}$ of data at $\sqrt{s}=13~\tev$, the ATLAS Collaboration has derived upper limits at 95\% confidence level (CL) \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on the }\DIFaddend branching ratio
$\BR(t\to cH)<0.22\%$ using $H\to \gamma\gamma$ decays~\cite{Aaboud:2017mfd} \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\BR(t\to cH)<0.16\%$ based on
...
The upper limits are set on }\DIFaddend $\BR(t\to cH)<0.42\%$ using $H\to b\bar{b}$ decay~\cite{fcnc36} \DIFdelbegin \DIFdel{, and of }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\BR(t\to cH)<0.19\%$ using \DIFaddbegin \DIFadd{the }\DIFaddend $H\to \tau^+\tau^-$ decay in which at least
...
the full Run 2 dataset\DIFdelbegin \DIFdel{. The observed }\DIFdelend \DIFaddbegin \DIFadd{; observing }\DIFaddend (expected) upper limits on  $\BR(t\to cH)$ \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $9.4\times10^{-4}\, (8.6\times10^{-4})$
and \DIFdelbegin \DIFdel{are $7.9\times10^{-4}\, (1.1\times10^{-3})$ for }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend $\BR{B}(t\to uH)$ \DIFaddbegin \DIFadd{of $7.9\times10^{-4}\, (1.1\times10^{-3})$}\DIFaddend ~\cite{CMS:2021gfa}.
...
$\tau$-leptons \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the complete Run~2 dataset collected in 2015-2018. The corresponding integrated luminosity is 139 fb$^{-1}$. Both $\ttbar \to WbHq$ \DIFdelbegin \DIFdel{decay }\DIFdelend \DIFaddbegin \DIFadd{decays }\DIFaddend and $pp \to tH$ production are \DIFdelbegin \DIFdel{targeted}\DIFdelend \DIFaddbegin \DIFadd{sought}\DIFaddend . The dataset is divided into several final states depending on the production mode and
...
To further reduce the background from non-prompt leptons \DIFaddbegin \DIFadd{mostly originating from heavy-flavour hadron decays}\DIFaddend , photon conversions and \DIFaddbegin \DIFadd{misidentified }\DIFaddend hadrons,
lepton candidates are also required to be isolated in the tracker and in the calorimeter.
A track- and cluster-based lepton isolation criterion is defined by calculating the \DIFdelbegin \DIFdel{quantity }\DIFdelend \DIFaddbegin \DIFadd{quantities }\DIFaddend $I_R = \sum \pt^{\textrm{trk}}$ and $E_R = \sum E^{\textrm{clst}}$, where
the scalar sum includes all tracks \DIFdelbegin \DIFdel{and energy deposits }\DIFdelend \DIFaddbegin \DIFadd{$\pt^{\textrm{trk}}$ and cluster energy deposits $E^{\textrm{clst}}$ }\DIFaddend (excluding the lepton candidate itself)
...
Tight isolation working points are also applied in some channels to reduce fake and non-prompt lepton contributions based on a trained isolation boosted decision tree \DIFaddbegin \DIFadd{(BDT)
}\DIFaddend \texttt{PromptLeptonVeto} (PLIV), which is used to identify non-prompt light leptons using lifetime information associated with a track jet that matches the selected light lepton~\cite{ATLAS-CONF-2019-045}. These additional reconstructed charged particle tracks inside the jet can be
used to increase the \DIFdelbegin \DIFdel{accuracy }\DIFdelend \DIFaddbegin \DIFadd{efficiency }\DIFaddend for identifying the displaced decay vertex of heavy flavour ($b$, $c$) hadrons that produced non-prompt leptons.
The tight working points are used for high $\pt$ \DIFdelbegin \DIFdel{lepton }\DIFdelend \DIFaddbegin \DIFadd{leptons }\DIFaddend ($>20~\GeV$).
The scale factors for the efficiencies of the tight \texttt{PLIV} working points are measured using the tag and probe method\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{PERF-2016-01}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend with $Z\rightarrow \ell^+\ell^-$ events. These scale factors have also been checked for the electron or muon from the tau decay
using $Z\rightarrow\tau\tau\rightarrow e\mu4\nu$ samples, which \DIFdelbegin \DIFdel{is consistent at }\DIFdelend \DIFaddbegin \DIFadd{are consistent at the }\DIFaddend 2\% level. To be conservative, an
...
The events in the leptonic channel are recorded by a single-electron or single-muon trigger, \DIFaddbegin \DIFadd{and are }\DIFaddend required to have exactly one electron or muon that matches, within $\Delta R < 0.15$, the lepton reconstructed by at least one of the possible triggers. \DIFdelbegin \DIFdel{Further requirements are defined targeting different signal decay modes:  
...
\item $t_h\lephad$: \DIFdelbegin \DIFdel{Targeting into }\DIFdelend \DIFaddbegin \DIFadd{To enhance selection of }\DIFaddend the $t_hH$ and $t_ht(qH)$ final states with a $H\to\lephad$ decay\DIFdelbegin \DIFdel{. Exactly }\DIFdelend \DIFaddbegin \DIFadd{, exactly }\DIFaddend one $\had$ with opposite-side charge to $\lep$ is required, including at least three jets with exactly one $b$-jet.
\item $t_{\ell}\hadhad$: \DIFdelbegin \DIFdel{Targeting }\DIFdelend \DIFaddbegin \DIFadd{To enhance selection of }\DIFaddend $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with a $H\to\hadhad$ decay\DIFdelbegin \DIFdel{. Exactly }\DIFdelend \DIFaddbegin \DIFadd{, exactly }\DIFaddend one light lepton and two opposite-sign $\had$ are selected,
  including \DIFdelbegin \DIFdel{at least one jet }\DIFdelend \DIFaddbegin \DIFadd{jets }\DIFaddend with exactly one $b$-jet.
\item $t_{\ell}\had$: \DIFdelbegin \DIFdel{Also targeting }\DIFdelend \DIFaddbegin \DIFadd{This channel is also to enhance }\DIFaddend $t_{\ell}H$ and $t_{\ell}t(qH)$ final states with a $H\to\hadhad$ decay where one $\had$ fails the tau reconstruction or identification
  \DIFaddbegin \DIFadd{criterion }\DIFaddend so that there is only one reconstructed $\had$ candidate.
  To reduce the SM backgrounds and avoid overlaps with the \DIFaddbegin \DIFadd{final states of }\DIFaddend other ATLAS searches, exactly one $\had$ with \DIFdelbegin \DIFdel{same-sign charge }\DIFdelend \DIFaddbegin \DIFadd{charge the same as that assigned }\DIFaddend to the light lepton is required.
  In addition, at least two jets including exactly one $b$-jet are required.
\end{itemize}

The events in the hadronic channel are selected by \DIFdelbegin \DIFdel{s }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend di-tau trigger. Further requirements are:
...
Depending on \DIFdelbegin \DIFdel{either }\DIFdelend \DIFaddbegin \DIFadd{whether the }\DIFaddend up quark or \DIFdelbegin \DIFdel{charm quark }\DIFdelend \DIFaddbegin \DIFadd{the charm quark is }\DIFaddend involved in the FCNC production, the effective Lagrangian of \DIFaddbegin \DIFadd{the }\DIFaddend $tqH$ interaction \DIFdelbegin \DIFdel{are parametrised using
dim-6 }\DIFdelend \DIFaddbegin \DIFadd{is parametrised using
dimension-6 }\DIFaddend operators~\cite{fcnc_production_theory}. We obtain $\sigma(ug\to tH)$ = 0.711 pb and $\sigma(cg\to tH)$ = 0.103 pb using $\BR_{\mathrm{ref}}(t\to qH)=0.1\%$ as \DIFaddbegin \DIFadd{the }\DIFaddend benchmark.   
...
}\DIFaddend {\powheg}~v2 \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
...
\begin{table}
\centering
\caption{Overview of the signal regions (SR), validation region (VR), and $t\bar{t}$ control regions (CRtt) used for the fake tau scale factor derivation in the leptonic channels. Leptons are required to have either \DIFdelbeginFL \DIFdelFL{same-sign }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same sign }\DIFaddendFL (SS) or \DIFdelbeginFL \DIFdelFL{opposite-sign }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{opposite sign }\DIFaddendFL (OS) in each region.}
\label{tab:srcr}
\begin{tabular}[h]{c|c|c|c|c|c|c}
\hline \hline
...
This event categorisation is primarily \DIFdelbegin \DIFdel{motivated by optimizing }\DIFdelend \DIFaddbegin \DIFadd{used to optimize }\DIFaddend the sensitivity in each signal region that targets either leptonic or hadronic top-quark
decays as well as the Higgs decays into either \DIFaddbegin \DIFadd{the }\DIFaddend $\lephad$ or $\hadhad$ final states.   
...
The contribution of background to the signal regions of the $t_h\hadhad$ and $t_h\lephad$ channels is reduced by having \DIFdelbegin \DIFdel{a }\DIFdelend kinematic constraints on the di-tau mass and the $\met$ in the event~\cite{Chen:2015nta}.
...
Events should contain 4 jets, with the one having the smallest angular separation with respect to the visible di-tau system being labelled \DIFdelbegin \DIFdel{as  }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend $q$-jet since the FCNC top decay products are likely boosted closer together. 
If there are more than two jets besides the $q$- and $b$-jet, the jets from \DIFaddbegin \DIFadd{the }\DIFaddend $W$ boson decay are chosen from the combination
...
These kinds of events will \DIFdelbegin \DIFdel{fall into }\DIFdelend \DIFaddbegin \DIFadd{produce }\DIFaddend the $t_hH$ final state.
...
both \DIFaddbegin \DIFadd{the }\DIFaddend $t_h$ and $H$ can be reconstructed.  
...
The fit is done by minimising a $\chi^2$ function based on the Gaussian constraints on the Higgs boson mass ($m_{\text{H}}=125~\gev$) and the
measured $E_{\text{x,y}}^{\text{miss}}$  within their expected resolutions ($\sigma_{E_{\text{x,y}}^{\text{miss}}}$), defined as
\begin{eqnarray}
\begin{array}{ll}
\chi^2 =
\left( \frac{m_{\tau\tau,\text{fit}} - m_{\text{H}}}{\sigma_{\tau\tau}} \right)^2 +
\left( \frac{E_{\text{x,fit}}^{\text{miss}} - E_{\text{x}}^{\text{miss}}}{\sigma_{E_{\text{x}}^{\text{miss}}}} \right)^2
  +\left( \frac{E_{\text{y,fit}}^{\text{miss}} - E_{\text{y}}^{\text{miss}}}{\sigma_{E_{\text{y}}^{\text{miss}}}} \right)^2.
\end{array}
\label{eq:eq1}
\end{eqnarray}

The resolution on the Higgs boson mass ($\sigma_{\tau\tau}$) is estimated to be $20~\gev$ from a fit of the mass
...
In the leptonic channels, the events with \DIFaddbegin \DIFadd{a }\DIFaddend prompt electron or muon and fake taus are modelled by calibrating \DIFaddbegin \DIFadd{the }\DIFaddend MC with scale factors (SF)
...
$b$-jets, \DIFaddbegin \DIFadd{as is }\DIFaddend summarized in Table~\ref{tab:srcr}. \DIFaddbegin \DIFadd{The control regions are defined similarly to the signal regions but with an additional $b$-jet or light lepton. 	
...
Type-2 $\tau_{W}$'s with same-sign (SS) charge \DIFaddbegin \DIFadd{relative }\DIFaddend to the light lepton; Type-3 fake taus originating from $b$-hadron decays; Type-4 fake taus from light-flavour hadron decays.
...
regions ($t_{\ell}t_h2b\thad$-2jOS and $t_{\ell}t_h2b\thad$-3jOS)\DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{, where the }\DIFaddend $\thad$ and light lepton have opposite \DIFdelbegin \DIFdel{charge }\DIFdelend \DIFaddbegin \DIFadd{charges, }\DIFaddend are used to calibrate Type-1 fake taus.
...
A simultaneous fit to data is made to derive the scale factors for fake taus in \DIFdelbegin \DIFdel{MC, which }\DIFdelend \DIFaddbegin \DIFadd{the MC. These }\DIFaddend consist of a total of 24 parameters
depending on four types of fake taus, three $\pt$ bins, \DIFaddbegin \DIFadd{and }\DIFaddend two bins for 1- and 3-prong taus separately. The post-fit distributions of the
leading $\thad$ $\pt$ and measured tau scale factors are presented in Appendix~\ref{sec:tauFF_appendix}. 
The values of these scale factors range from 0.3 \DIFdelbegin \DIFdel{-1.28 }\DIFdelend \DIFaddbegin \DIFadd{to 1.28 and }\DIFaddend are then used to correct the \DIFdelbegin \DIFdel{MC estimated }\DIFdelend \DIFaddbegin \DIFadd{MC-estimated }\DIFaddend fakes in the corresponding  signal regions with a single $b$-jet.
...
The \DIFdelbegin \DIFdel{nominal }\DIFdelend \DIFaddbegin \DIFadd{central }\DIFaddend value of scale factors \DIFdelbegin \DIFdel{will }\DIFdelend vary according to their uncertainties in the final fit. A closure test is made for the fake tau \DIFdelbegin \DIFdel{estimations }\DIFdelend \DIFaddbegin \DIFadd{estimates }\DIFaddend using the same procedure
in the $t_{\ell}\thadhad$-SS validation region, \DIFdelbegin \DIFdel{which shows a }\DIFdelend \DIFaddbegin \DIFadd{and it shows }\DIFaddend good agreement between data and the background prediction. 
...
that the subleading tau candidate is required to fail the medium tau identification \DIFdelbegin \DIFdel{, but it still passes }\DIFdelend \DIFaddbegin \DIFadd{while still passing }\DIFaddend a loose requirement.
The contribution of fakes with subleading tau \DIFdelbegin \DIFdel{candidate }\DIFdelend \DIFaddbegin \DIFadd{candidates }\DIFaddend can be calculated by rescaling the templates of loose taus in the CR
with \DIFdelbegin \DIFdel{fake factors (FF)}\DIFdelend \DIFaddbegin \DIFadd{the FFs}\DIFaddend .
The templates are produced by subtracting all MC background contributions with real subleading taus from \DIFaddbegin \DIFadd{the }\DIFaddend data.
...
%We have compared
FFs obtained from the $W$+jets and from the same-sign $\thadhad$ control regions are compared and the \DIFdelbegin \DIFdel{differences are }\DIFdelend \DIFaddbegin \DIFadd{difference is }\DIFaddend treated as a systematic uncertainty.
...
\DIFdelbegin \DIFdel{A large set of }\DIFdelend \DIFaddbegin \DIFadd{Many }\DIFaddend potential variables are investigated in each SR separately. The discrimination of a given variable is quantified by the "separation" (\DIFaddbegin \DIFadd{which }\DIFaddend measures the degree of overlap between background and signal shape) and "importance" (\DIFaddbegin \DIFadd{which }\DIFaddend depicts the power of the variable \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the classification of the events) provided by the TMVA package\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
...
The final BDT input variables in each SR and their importance ranking are listed in Table~\ref{tab:importance}. The discriminating variables \DIFdelbegin \DIFdel{used are :
...
\item $\chi^2$ of the kinematic fit of the momentum of the invisible decay products of the tau leptons as defined in \DIFdelbegin \DIFdel{eq}\DIFdelend \DIFaddbegin \DIFadd{Eq}\DIFaddend .~(\ref{eq:eq1}).
...
\end{equation}
where $\Delta\phi_{\ell,\text{miss}}$ is the \DIFdelbegin \DIFdel{azimuth }\DIFdelend \DIFaddbegin \DIFadd{azimuthal }\DIFaddend angle between the light-lepton and $\met$.  
...
\item $m_{\text{jj}}$ is the reconstructed invariant mass of two light-jets from the $W$ decay with the mass \DIFdelbegin \DIFdel{closet }\DIFdelend \DIFaddbegin \DIFadd{closest }\DIFaddend to the $W$ mass.
\item $m_{\tau\tau\text{q},\text{fit}}$ is the fitted invariant mass of the FCNC-decaying top quark reconstructed from \DIFaddbegin \DIFadd{the }\DIFaddend di-tau candidates, $q$-jet and reconstructed neutrinos.
...
\item \text{min}($m_{\tau\tau \text{j}}$) is the minimum value of the visible mass of the di-tau candidates (\DIFdelbegin \DIFdel{include leptonic }\DIFdelend \DIFaddbegin \DIFadd{including leptonically decaying }\DIFaddend tau) and the light-flavour jet system,
  \DIFdelbegin \DIFdel{minimized by choosing different jet, }\DIFdelend reflecting the invariant \DIFdelbegin \DIFdel{masss }\DIFdelend \DIFaddbegin \DIFadd{mass }\DIFaddend of the visible FCNC top decaying products.
...
\item $\met$ centrality is a measure of how \DIFdelbegin \DIFdel{central }\DIFdelend \DIFaddbegin \DIFadd{centrally }\DIFaddend the $\met$ lies between the two tau candidates in the transverse plane, and is defined as
\begin{eqnarray}
\begin{array}{l}
...
\item $\Delta R(a,b)$ is the angular distance between \DIFaddbegin \DIFadd{the }\DIFaddend \text{a} and \text{b} objects in the event. 
...
\DIFdelbegin \DIFdel{A comparison between }\DIFdelend \DIFaddbegin \DIFadd{Comparisons between the }\DIFaddend data and the predicted background for the leading $\had$ $\pt$  and the visible invariant mass of the di-tau system ($m_{\tau\tau}$)
...
The final observable used to extract the signal contribution is the BDT distribution in each SR corresponding to either \DIFaddbegin \DIFadd{the }\DIFaddend $tuH$ or \DIFaddbegin \DIFadd{the }\DIFaddend $tcH$ signal.
...
Uncertainties associated with jets arise from the jet energy scale \DIFaddbegin \DIFadd{(JES)
and resolution (JER) }\DIFaddend and \DIFdelbegin \DIFdel{resolution, and }\DIFdelend the efficiency to pass the JVT requirements. 
...
in data and \DIFaddbegin \DIFadd{in }\DIFaddend MC is represented by one \DIFdelbegin \DIFdel{NP}\DIFdelend \DIFaddbegin \DIFadd{nuisance parameter (NP)}\DIFaddend . It is applied \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the MC by smearing the jet $\pt$ within the prescribed uncertainty.
...
\DIFdelbegin \DIFdel{whereas }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend the light-jet efficiency is scaled by $\pt$- and $\eta$-dependent factors.
...
The uncertainties in these scale factors include a total of 44 independent sources affecting $b$-jets, 19 \DIFdelbegin \DIFdel{source }\DIFdelend \DIFaddbegin \DIFadd{sources }\DIFaddend affecting $c$-jets, and 19 sources affecting light-jets. 
...
An uncertainty on these reweighting scale factors is estimated by varying the reweighting \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{upon }\DIFaddend the profile in data within its uncertainties.
...
A number of sources of systematic uncertainty affecting the modelling of $t\bar{t}$+jets are considered: the choice of the renormalisation and factorisation scale in the matrix-element calculation, the choice of the matching scale when matching the matrix elements to the parton \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{shower }\DIFaddend generator, the uncertainty \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the value of $\alpha_s$ when modeling initial-state radiation (ISR), \DIFaddbegin \DIFadd{and
}\DIFaddend the choice of the renormalisation scale when modeling final-state radiation (FSR).
...
\DIFdelbegin \DIFdel{POWHEG-BOX }\DIFdelend \DIFaddbegin \powheg\DIFadd{~}\DIFaddend v2\DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend is set to 1.5$m_t$ in the \DIFdelbegin \DIFdel{nominal case. Alternative samples are }\DIFdelend \DIFaddbegin \DIFadd{$t\bar t$ sample. Alternative sample is }\DIFaddend generated with $h_{\mathrm{damp}}$=3$m_t$. The
difference between two samples is treated as a systematic \DIFdelbegin \DIFdel{named as }\DIFdelend \DIFaddbegin \DIFadd{called }\DIFaddend ``$t\bar{t}$ $h_{\mathrm{damp}}$''. 
...
The uncertainty \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the modelling of additional radiation from the PS \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend assessed by
...
Another significant background \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to the }\DIFaddend hadronic channel stems from the $Z\rightarrow \tau\tau$ samples\DIFdelbegin \DIFdel{, several }\DIFdelend \DIFaddbegin \DIFadd{. Several }\DIFaddend sources of uncertainty are
...
\DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{produced by }\DIFaddend 100 NNPDF replicas event weights of NNPDF3.0nnlo~\cite{Ball:2015NNPDF} PDF set used in Sherpa,
renormalisation ($\mu_{R}$) and factorisation scales ($\mu_{F}$), jet-to-parton matching uncertainty, resummation scale uncertainty,
variation in the choice of $\alpha_{S}$, \DIFdelbegin \DIFdel{alternative PDF variation evaluated }\DIFdelend \DIFaddbegin \DIFadd{and alternative PDF use, which is evaluated by }\DIFaddend comparing predictions from NNPDF3.0nnlo PDF set (nominal)
with \DIFaddbegin \DIFadd{those of }\DIFaddend MMHT2014nnlo68cl and CT14nnlo~\cite{Lai:2010vv,Gao:2013xoa} PDF sets.
...
This is mainly driven by the disagreement between MC \DIFdelbegin \DIFdel{simulation }\DIFdelend \DIFaddbegin \DIFadd{simulations }\DIFaddend in the $V$+jets regions with \DIFaddbegin \DIFadd{a }\DIFaddend high multiplicity of jets.  
...
$+5\%$/$-4\%$ uncertainty \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the total cross section\DIFaddbegin \DIFadd{, which is }\DIFaddend estimated as a weighted average 
...
Additional uncertainties associated with the parton shower, hadronisation and ISR/FSR radiations are also considered \DIFaddbegin \DIFadd{by }\DIFaddend using the same procedure
as \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $t\bar t$. Uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the diboson background normalisation include variations of the renormalization and factorization \DIFdelbegin \DIFdel{scale}\DIFdelend \DIFaddbegin \DIFadd{scales}\DIFaddend , NNPDF3.0nnlo variations and $\alpha_{S}$ variations. Uncertainties of the $\ttbar V$ and $\ttbar H$ cross sections are 12\% and \DIFdelbegin \DIFdel{$+5.8\%-9.2\%$}\DIFdelend \DIFaddbegin [\DIFadd{$+5.8\%/-9.2\%$}]\DIFaddend , respectively,
from the uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend their respective NLO theoretical cross sections~\cite{ttZRun2}. 
...
The uncertainty \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the ABCD method is applied \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the normalisation factors for \DIFdelbegin \DIFdel{muon and electron respectively including both
,
}\DIFdelend \DIFaddbegin \DIFadd{muons and electrons,including both
}\DIFaddend its statistical fluctuation and the differences in the normalisation factors between the leptonic channels.
The uncertainties \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the fake factor method applied \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to the }\DIFaddend hadronic channel includes statistical uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend each fake factor and
different sets of fake factors derived from different \DIFdelbegin \DIFdel{signal depleted }\DIFdelend \DIFaddbegin \DIFadd{signal-depleted }\DIFaddend CRs.
...
Uncertainties \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the Higgs boson branching ratios are taken
...
The uncertainty \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{due to }\DIFaddend ISR, FSR, scale and PDF are considered and are treated \DIFaddbegin \DIFadd{as }\DIFaddend fully correlated in all SRs. The parton shower uncertainties are estimated by comparing
the nominal \DIFaddbegin \DIFadd{sample }\DIFaddend to an alternative sample interfaced with {\herwig7}.
...
The final discriminant distributions are the BDT outputs of all the considered analysis regions. They are jointly analysed \DIFdelbegin \DIFdel{to test }\DIFdelend for the 
presence of a signal. The BDT distributions are binned to maximize \DIFdelbegin \DIFdel{the signal sensitivity}\DIFdelend \DIFaddbegin \DIFadd{sensitivity to the signal}\DIFaddend .
...
}\DIFaddend and $\theta$, a set of nuisance parameters that encode the effect of systematic uncertainties on the signal and background expectations.
...
All nuisance parameters are subject to Gaussian constraints \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the likelihood.
...
The expected sensitivity has been improved \DIFdelbegin \DIFdel{upon }\DIFdelend \DIFaddbegin \DIFadd{by a factor of~5 upon that of }\DIFaddend the previous ATLAS result\DIFaddbegin \DIFadd{, which was }\DIFaddend based on 36 fb$^{-1}$ of data using \DIFaddbegin \DIFadd{the }\DIFaddend $H\to \tau\tau$ decay~\cite{fcnc36}\DIFdelbegin \DIFdel{by a factor of~5. }\DIFdelend \DIFaddbegin \DIFadd{.
...
the normalization and factorization scales, \DIFaddbegin \DIFadd{the }\DIFaddend $b$-tagging \DIFaddbegin \DIFadd{efficiency}\DIFaddend , the choice of parton shower and hadronization \DIFaddbegin \DIFadd{schemes }\DIFaddend for $t\bar t$ modelling, and
...
\begin{table}[h!]
  \caption{Absolute uncertainties on $\BR(t\to qH)$ ($q=u,c$) obtained from the combined fit to \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL data. The uncertainties are symmetrised
    \DIFdelbeginFL \DIFdelFL{for
     a presentation purpose }\DIFdelendFL and grouped into the categories described in the Section~\ref{sec:systematics}.}
\label{tab:had_sys_impact}
% \begin{center}
%   \begin{tabular}{%
%       @{}l%
...
Upper limits on the branching ratios $\BR(t\to qH)$ (\DIFdelbegin \DIFdel{$q=u,c$}\DIFdelend \DIFaddbegin \DIFadd{$q=u,\, c$}\DIFaddend ) can be translated into upper limits on the dimension-6 (D6) operator Wilson coefficients appearing in the effective field theory Lagrangian for the $tqH$ interaction~\cite{fcnc_production_theory}:
%
\begin{equation}
...
\end{equation}
%
where the subscript \DIFdelbegin \DIFdel{i= 1, 2 }\DIFdelend \DIFaddbegin \DIFadd{$i=1,\, 2$ }\DIFaddend represents the generation of the light quark fields (\DIFdelbegin \DIFdel{$q=u, c$}\DIFdelend \DIFaddbegin \DIFadd{$q=u,\, c$}\DIFaddend ).
The branching ratio $\BR(t\to qH)$ is estimated as the ratio of its partial width to the SM $t \to Wb$ partial width including \DIFdelbegin \DIFdel{next-to-leading-order QCD correctionsand the }\DIFdelend \DIFaddbegin \DIFadd{next-to-leading order QCD corrections. The }\DIFaddend coefficients can be extracted as $C_{q\phi} = \sqrt{1946.6~\BR(t\to qH)}$~\cite{fcnc_production_theory}. The $C_{q\phi}$ coefficient corresponds to the sum in quadrature of the coefficients relative to the two possible chirality combinations of the quark fields,
...
results }\DIFaddend are $C_{c\phi}<1.35\,(0.97)$ and $C_{u\phi}<1.16\,(0.82)$ for a new physics scale $\Lambda=1$~TeV. 
...
%-------------------------------------------------------------------------------
A search for flavour-changing neutral current processes involving a top quark, \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{another }\DIFaddend up-type quark ($q=u, c$), and a SM Higgs boson is presented. FCNC $tqH$ interaction is \DIFdelbegin \DIFdel{searched for }\DIFdelend \DIFaddbegin \DIFadd{sought }\DIFaddend both in the $t\bar{t}$ decay mode, where one top quark decays \DIFdelbegin \DIFdel{according to the }\DIFdelend \DIFaddbegin \DIFadd{via }\DIFaddend SM processes and the other one decays through $t\rightarrow qH$, and in the production mode ($pp\rightarrow tH$) with a single top quark produced via \DIFaddbegin \DIFadd{the
}\DIFaddend FCNC interaction where $t\to Wb$.
...
A slight excess of data is observed above background with a significance of 2.3$\sigma$. \DIFaddbegin \DIFadd{Upper limits at the
...
\section{Tau fake scale factor calibration in the \DIFdelbegin \DIFdel{ttCRs}\DIFdelend \DIFaddbegin \DIFadd{CRtt}\DIFaddend }
...
\DIFdelbegin \DIFdel{Table}\DIFdelend \DIFaddbegin \DIFadd{Tables}\DIFaddend ~\ref{tab:ff1_summary} and~\ref{tab:ff2_summary} summarize the fake scale factors of 1-prong and 3-prong taus
derived from the \DIFdelbegin \DIFdel{ttCRs }\DIFdelend \DIFaddbegin \DIFadd{CRtt }\DIFaddend including both statistical and systematic uncertainties. 
...
(d) & (e) & (f)\\
\end{tabular}
\caption{Leading $\thad$ $\pt$  distributions obtained after the fit to data (``Post-Fit'') for the $\thad$ fake scale factors in the following \DIFdelbeginFL \DIFdelFL{ttCRs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CRtt}\DIFaddendFL :
