...
\begin{table}
\centering
\caption{Overview of the signal regions (SR), validation region (VR), and the $t\bar{t}$ control regions (CRtt) used for fake tau scale factor derivation in leptonic channels. Leptons are required to have either same-sign (SS) or opposite-sign (OS) charges in each region.}
\label{tab:srcr}
\begin{tabular}[h]{c|c|c|c|c|c|c}
\hline \hline
...
Events should contain 4 jets, with the one at the smallest angular distance to the visible decay of the di-tau being labelled as the q-jet since the FCNC top decay products are likely boosted closer together than other jets. 
...
These kinds of events will fall into $t_hH$ final state.
...
are estimated using a kinematic fit and collinear approximation for the tau decay products.
The fit is done by minimising a $\chi^2$ function based on the Gaussian constraints on the Higgs boson mass and the
measured $\met$ within their expected resolutions. The resolution on the Higgs boson mass is estimated to be $20~\gev$ from the MC simulation
of the $tqH$ signals, while the resolution on the measured $\met$ is parameterised as a linear function of 
