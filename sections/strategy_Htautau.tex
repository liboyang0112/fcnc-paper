...
\item $M(\tau\tau qjet,min)$ is the visible mass of the di-tau candidates (include leptonic tau) and the light-flavor jet, minimized by choosing different jet, reflects the invariant mass of the visible FCNC top decaying product, an alternative of variable $m_{t,FCNC,vis}$.
...
\item $\Delta R(\tau,qjet)$ is the angular distance between the tau and the reconstructed $q$-jet. If there are two taus in the event, the leading one is used.
...
The expected signals (dashed) are also shown after scaled up for the shape comparison of the distributions.
...
%\put(-70, 70){\textbf{(a3)}}
\\
\includegraphics[page=7,width=0.33\textwidth]{\FCNCFigures/tthML/showFake/faketau/postfit/NOMINAL_fancpaper/reg1l1tau1b2j_os_vetobtagwp70_highmet/tau_pt_0.pdf}
\put(-40, 90){\small\textbf{(b1)}}
\includegraphics[page=7,width=0.33\textwidth]{\FCNCFigures/tthML/showFake/faketau/postfit/NOMINAL_fancpaper/reg1l1tau1b3j_os_vetobtagwp70_highmet/tau_pt_0.pdf}
