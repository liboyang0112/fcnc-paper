...
and $\theta$, a set of nuisance parameters that encode the effect of systematic uncertainties of the signal and background expectations. 
...
The fitting procedure is initially validated through extensive studies using mock data, defined as the sum of all predicted backgrounds 
...
when varying assumptions on some of the leading sources of uncertainty. 
...
nuisance parameters before and after removal of the blinding requirements, which are found to be consistent. In addition, it is verified that the 
...
A related statistic is used to determine the compatibility of the observed data with the background-only hypothesis (the so-called discovery test)  
...
In the case of the data being compatible with the background-only hypothesis, upper limits on $\mu$, and thus on 
