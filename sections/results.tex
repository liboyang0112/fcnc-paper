...
A slight excess of data is observed above background with a significance of 2.6 $\sigma$, which is mainly in the $t_l\thadhad$ channel in the high BDT score region.
...
affecting the normalization and factorization scales, $b$-tagging, the choice of parton shower and hadronization for $t\bar t$ modelling, and the fake $\had$ background estimation in the hadronic channels. Their relative impacts on the signal strength are summarized in Tables~\ref{tab:had_sys_impact}-~\ref{tab:lep_sys_impact}.
...
$C_{q\phi} =\sqrt{(C^{i3}_{q\phi})^2 + (C^{3i}_{q\phi})^2}$~\cite{fcnc_production_theory}. The observed (expected) upper limits on the D6 Wilson coefficients from the combination of the searches are $C_{c\phi}<1.38\,(0.97)$ and $C_{u\phi}<1.18\,(0.83)$. 
