...
A track-based lepton isolation criterion is defined by calculating the quantity $p_R = \sum \pt^{\textrm{trk}}$ and $E_R = \sum E^{\textrm{clst}}$, where
...
$\pt >20~\gev$ and $|\eta|<2.5$ in simulated $\ttbar$ events.  (TBD: check DL1r description)
...
with a total charge of $\pm 1$. Candidates are required to have \pt 5 GeV higher than the trigger threshold, and $|\eta|<2.5$, excluding the EM calorimeter's transition region.
A Recurrent Neural Network (RNN) ~\cite{Breiman:1984jka,Friedman:2002we,Freund:1997xna} (citation to be changed) using calorimeter- and tracking-based variables is used to identify $\had$ candidates and reject jet backgrounds. Three working points labelled loose, medium and tight are defined, and correspond to different $\had$ identification efficiency values, with the efficiency designed to be independent of $\pt$. The $\Htautau$ search uses the medium
...
Any $\had$ candidate that is also $b$-tagged is rejected. (TBD: check RNN description and update then numbers)
