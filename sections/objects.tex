...
the sum includes all tracks (excluding the lepton candidate itself) within the cone defined by $\Delta R<R_{\textrm{cut}}$ about the %axis
...
Hadronically decaying $\tau$-lepton candidates ($\tauhad$) are reconstructed from clusters in the calorimeters and
...
with a total charge of 1. Candidates are required to have $\pt > 25~\gev$ and $|\eta|<2.5$, excluding the EM calorimeter's transition region.
...
working point for the nominal selection, while the loose working point is used for the purpose of background estimation.
...
Overlaps between candidate objects are removed sequentially, following this order: muons, electron, taus (only for the $\Htautau$ search) and jets. 
