...
and the Higgs boson is forced to decay via $H\to \tau\tau$ with branching ratio calculated using \textsc{Hdecay}~\cite{Djouadi:1997yw}.  
...
In the leptonic channels, the events with a prompt electron or muon and fake taus are modelled by calibrating the MC with scale factors (SF)
...
$b$-jets, as is summarized in Table~\ref{tab:srcr}. The control regions are defined similarly to the signal regions but with an additional $b$-jet or light lepton. 	
...
Type-2 $\tau_{W}$'s with same-sign (SS) charge relative to the light lepton; Type-3 fake taus originating from $b$-hadron decays; Type-4 fake taus from light-flavour hadron decays.
...
regions ($t_{\ell}t_h2b\thad$-2jOS and $t_{\ell}t_h2b\thad$-3jOS), where the $\thad$ and light lepton have opposite charges, are used to calibrate Type-1 fake taus.
...
A simultaneous fit to data is made to derive the scale factors for fake taus in the MC. These consist of a total of 24 parameters
depending on four types of fake taus, three $\pt$ bins, and two bins for 1- and 3-prong taus separately. The post-fit distributions of the
leading $\thad$ $\pt$ and measured tau scale factors are presented in Appendix~\ref{sec:tauFF_appendix}. 
The values of these scale factors range from 0.3 to 1.28 and are then used to correct the MC-estimated fakes in the corresponding  signal regions with a single $b$-jet.
...
The central value of scale factors vary according to their uncertainties in the final fit. A closure test is made for the fake tau estimates using the same procedure
...
that the subleading tau candidate is required to fail the medium tau identification while still passing a loose requirement.
The contribution of fakes with subleading tau candidates can be calculated by rescaling the templates of loose taus in the CR
with the FFs.
The templates are produced by subtracting all MC background contributions with real subleading taus from the data.
...
%We have compared
FFs obtained from the $W$+jets and from the same-sign $\thadhad$ control regions are compared and the difference is treated as a systematic uncertainty.
...
low $\met$ region. The contributions of prompt leptons and calibrated fake taus to the number of tight and loose leptons are subtracted using MC simulation.
A closure test is made for the background estimates in the signal depleted low BDT ($<-0.6$) regions, defined in Section~\ref{sec:tmva}.
