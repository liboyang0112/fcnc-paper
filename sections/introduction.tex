...
decay of $W$ boson hadronically and leptonically from the $t\rightarrow wb$ decay referred as $t_h$ and $t_l$. The $H\rightarrow \tau\tau$ decay detected to be in
either $\tlhad$ or $thadhad$ final states when the top decays hadronically ($t_h$) and only in $\thadhad$ when the top decay leptonically ($t_l$). 
Events with two hadronically decaying $\tau$-lepton candidate (\tauhad) without electron or muon are selected for hadronic channels. Events with at least one \tauhad with an additional electron or muon are selected from leptonic channels. More regions are exploited compared to the FCNC $\Htautau$ search conducted using partial Run~2 data.
