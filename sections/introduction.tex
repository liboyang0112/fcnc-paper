...
It has been nearly a decade since the observation of Higgs boson by the ATLAS and CMS experiments~\cite{Aad:2012tfa,Chatrchyan:2012ufa} at
the Large Hadron Collider (LHC), a comprehensive programme of measurements of its properties was conducted, by which no significant sign of deviation from Standard Model (SM) was observed until today. But there is still chance finding new physics with searches already attempted but with higher sensitivity. An interesting possibility is the presence of flavour-changing neutral-current (FCNC) interactions 
...
This paper is focussed on the tau decay mode of the Higgs boson with the complete dataset collected during the Run~2 from 2016 to 2018. The corresponding integrated luminosity is 139.6 fb$^{-1}$. Both $\ttbar \to WbHq$ decay and $pp \to tH$ production are targeted. Events with two hadronically decaying $\tau$-lepton candidate (\tauhad) are selected for hadronic channels. Events with at least one \tauhad with an additional electron or muon are selected from leptonic channels, which exploits more regions compared to the search conducted using partial Run~2 data.
