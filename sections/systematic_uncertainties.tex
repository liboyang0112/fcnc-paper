...
The following sections describe the systematic uncertainties considered in these searches.
...
the sample with increased radiation (referred to as ``radHi'') has the renormalisation and factorisation scales decreased by a factor of two, 
the $h_{\textrm{damp}}$ parameter doubled, and uses the Var3c upward variation of the A14 parameter set; 
the sample with decreased radiation (referred to as ``radLow'') has the scales increased by a factor of two 
and uses the Var3c downward variation of the A14 set~\cite{ATL-PHYS-PUB-2016-004}.
...
regions have a sufficiently large number of events of $\ttbin$ background, its normalisation is completely 
...
\textbf{Update discussion on small bkgs systs for tautau (recently updated).}
...
between $t\bar{t}$ and $Wt$ at NLO~\cite{Frixione:2008yi} is assessed by comparing
the nominal sample, which uses the so-called ``diagram removal'' scheme, with an alternative sample
using the ``diagram subtraction'' scheme.
...
(we assume already two jets come from the $W/Z$ decay, as in $WW/WZ \to \ell \nu jj$). 
Therefore, the total normalisation uncertainty is $5\% \oplus \sqrt{N-2}\times 24\%$, where $N$ is the desired jet multiplicity,  
...
Uncertainties on the data-driven multijet background in the $\Hbb$ search estimate receive
...
and alternative $\ttbar \to WbWb$ background samples (see discussion above) and applied to $\Hq$ signal. 
