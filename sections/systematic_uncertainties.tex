...
Table~\ref{tab:had_sys_impact} shows a summary of the dominant systematic uncertainties on the measured $\BR(t\to qH)$ branching ratio
resulting from the fits to the data on the signal regions as described in Section~\ref{sec:result}.
...
in data and in MC is represented by one nuisance parameter (NP). It is applied to the MC by smearing the jet $\pt$ within the prescribed uncertainty.
...
The uncertainties in these scale factors include a total of 44 independent sources affecting $b$-jets, 19 sources affecting $c$-jets, and 19 sources affecting light-jets. 
...
An uncertainty on these reweighting scale factors is estimated by varying the reweighting upon the profile in data within its uncertainties.
...
\powheg~v2, is set to 1.5$m_t$ in the $t\bar t$ sample. Alternative sample is generated with $h_{\mathrm{damp}}$=3$m_t$. The
difference between two samples is treated as a systematic called ``$t\bar{t}$ $h_{\mathrm{damp}}$''. 
...
The uncertainty on the modelling of additional radiation from the PS is assessed by
...
Another significant background to the hadronic channel stems from the $Z\rightarrow \tau\tau$ samples. Several sources of uncertainty are
considered for these samples: the PDF variation, which is evaluated by considering the standard deviation
produced by 100 NNPDF replicas event weights of NNPDF3.0nnlo~\cite{Ball:2015NNPDF} PDF set used in Sherpa,
renormalisation ($\mu_{R}$) and factorisation scales ($\mu_{F}$), jet-to-parton matching uncertainty, resummation scale uncertainty,
variation in the choice of $\alpha_{S}$, and alternative PDF use, which is evaluated by comparing predictions from NNPDF3.0nnlo PDF set (nominal)
with those of MMHT2014nnlo68cl and CT14nnlo~\cite{Lai:2010vv,Gao:2013xoa} PDF sets.
...
This is mainly driven by the disagreement between MC simulations in the $V$+jets regions with a high multiplicity of jets.  
...
Additional uncertainties associated with the parton shower, hadronisation and ISR/FSR radiations are also considered by using the same procedure
as for $t\bar t$. Uncertainties on the diboson background normalisation include variations of the renormalization and factorization scales, NNPDF3.0nnlo variations and $\alpha_{S}$ variations. Uncertainties of the $\ttbar V$ and $\ttbar H$ cross sections are 12\% and [$+5.8\%/-9.2\%$], respectively,
from the uncertainties on their respective NLO theoretical cross sections~\cite{ttZRun2}. 
...
The uncertainty on the ABCD method is applied to the normalisation factors for muons and electrons,including both
its statistical fluctuation and the differences in the normalisation factors between the leptonic channels.
The uncertainties on the fake factor method applied to the hadronic channel includes statistical uncertainties on each fake factor and
different sets of fake factors derived from different signal-depleted CRs.
...
Uncertainties on the Higgs boson branching ratios are taken
...
The uncertainty due to ISR, FSR, scale and PDF are considered and are treated as fully correlated in all SRs. The parton shower uncertainties are estimated by comparing
the nominal sample to an alternative sample interfaced with {\herwig7}.
