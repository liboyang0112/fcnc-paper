...
is applied as an uncertainty on the prediction in the nominal SR.
In addition, a 30\% uncertainty is applied to the fraction of $\ttbar$ events with a fake $\had$ candidate from the simulation that are added to the fake $\had$ template in the $\hadhad$ channel as part of the fake $\had$ background estimation procedure (see Section~\ref{sec:faketaus}). This uncertainty, associated with the modelling of the fake $\had$ rate by the simulation, is estimated by comparing data and simulation in a sample enriched in $\ttbar$ dilepton events plus a fake $\had$ candidate. 
