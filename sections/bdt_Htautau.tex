...
A comparison between data and the predicted background for the leading $\had$ $\pt$  and visible di-tau mass distribution in the SRs, after applying fake factors, 
%some of these variables in three main decay final states($t_{\ell}\hadhad$, $t_h\hadhad$, and $t_h\lephad$)
is shown in Figure~\ref{fig:taupt_prefit} and Figure~\ref{fig:mtt}, respectively.
...
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l2tau1bnj_os_log_.pdf} &
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b1j_ss_log_.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b2j_ss_log_.pdf}\\
(a) & (b) & (c) \\
%(a1) \pT(\tauhad) in $t_{\ell}\thadhad$ & (a2) \pT(\tauhad) in  $t_{\ell}\tauhad$-1j& (a3) \pT(\tauhad) in $t_{\ell}\tauhad$-2j\\
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b2j_os_log_.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l1tau1b3j_os_log_.pdf} &
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg1l2tau1bnj_ss_log_.pdf} \\
(d) & (e) & (f)\\
%(b1) \pT(\tauhad) in $t_h\tlhad$-2j & (b2) \pT(\tauhad) in  $t_h\tlhad$-3j & (b3) \pT(\tauhad) in $t_h\thadhad$-2j \\
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b2jos_log_.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b3jos_log_.pdf}&
\includegraphics[page=1,width=0.29\textwidth]{figures/new_pt/log_or_nolog/tuH_reg2mtau1b3jss_log_.pdf}\\
(g) & (h) & (i)\\
...
%(c1) \pT(\tauhad) in $t_h\thadhad$-3j\\
\end{tabular}
\caption{Visible di-tau mass  distributions obtained before the fit to data (``Pre-Fit'') showing 
  the expected background and $tuH$ signals after applying fake factors in the following SRs: $t_{\ell}\thadhad$ (a),
 $t_{\ell}\tauhad$-1j (b),  $t_{\ell}\tauhad$-2j (c), $t_h\tlhad$-2j (d), $t_h\tlhad$-3j (e), $t_{\ell}\thadhad$ same sign CR (f), $t_h\thadhad$-2j (g), $t_h\thadhad$-3j (h) and $t_h\thadhad$-3j same sign CR (i).
 The total size of the statistical and systematic uncertainties of the background prediction is indicated by the hatched band.
 %The uncertainty band includes both the statistical and systematic uncertainties in the background prediction.
...
%(c1) \pT(\tauhad) in $t_h\thadhad$-3j\\
\end{tabular}
\caption{Leading $\thad$ $\pt$  distributions obtained the fake $\thad$ calibration method after applying fake factors in the following CRs: 
